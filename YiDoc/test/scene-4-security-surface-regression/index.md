...
| D1 — 基线文件手动维护易出错 | 首次运行时自动扫描生成基线，后续运行与基线对比 |
| D2 — CDN URL 可能通过子资源完整性（SRI）验证 | 基线中增加 integrity 哈希字段，验证 CDN 资源未被篡改 |
| D3 — 未检查协议降级攻击 | 检测 http:// URL（非 https），标记为 WARN 并建议升级 |
| D4 — 外部域名信誉未评估 | 集成域名信誉 API（如 Google Safe Browsing），检查 CDN 域名安全性 |
| D5 — 缺少 CSP（内容安全策略）分析 | 分析项目是否可从 CDN 引用中自动生成合适的 CSP 头 |
| D6 — 仅关注 HTML 文件 | CSS 中的 `@import url()` 和 `url()` 外部引用也应纳入扫描 |
| D7 — 报告缺少修复建议 | 对每个 ALERT 输出具体修复步骤（如 "请将 http:// 升级为 https://"） |
| D8 — 无历史趋势分析 | 记录每次扫描结果，生成安全面变化趋势图 |
| D9 — 新依赖审查流程未定义 | 建立新外部依赖的审批流程：提出 → 安全审查 → 更新基线 → 合入 |
