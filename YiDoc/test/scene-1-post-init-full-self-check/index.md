...
项目在全新 `rui-init` 初始化后，通过一次完整自检验证所有静态资源的完整性。该场景覆盖 31 个 HTML 页面、35 个 CSS 样式表和 62 个 JS 脚本的存在性校验，以及跨文件的资源引用（`<link>`、`<script>`、`<img>`）有效性检查。

预期效果：自检全部通过（31/31 HTML、35/35 CSS、62/62 JS 存在且引用有效），0 个断链，0 个缺失资源。

```mermaid
graph TD
    A[rui-init 初始化完成] --> B[遍历 Websites/ 目录树]
    B --> C{文件计数校验}
