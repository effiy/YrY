...
- **关键架构决策**
  ...
  - **排除 CDN 引用**：对 `http://`、`https://` 开头的 `<script>` / `<link>` 引用不进行本地文件解析（Vue CDN 已迁移至 YiPet/cdn/vendor/vue.global.prod.js 自托管），但记录在报告中供安全审查（参见 scene-4）
...
| D1 — 新网站加入时文件计数硬编码 | 将预期文件数（31/35/62）提取为配置变量，从 data.js stats 动态读取 |
| D2 — 缺少增量检查能力 | 引入文件哈希缓存（manifest.sha256），支持仅检查变更文件 |
| D3 — CDN 引用无版本锁定 | 在自检报告中标记所有 CDN 引用及其当前版本，供 scene-4 使用 |
| D4 — 报告格式不统一 | 统一使用 JSON 报告格式，与 CI 流水线集成 |
| D5 — 未验证图片文件格式合法性 | 增加图片魔术字节（magic bytes）校验，防止损坏的图片文件 |
| D6 — 字体文件未纳入检查范围 | 将 41 个字体文件（.woff/.woff2/.ttf/.eot/.svg/.otf）纳入存在性检查 |
| D7 — 并行扫描性能优化 | 对大型目录树使用并行文件遍历，减少全量扫描耗时 |
| D8 — 无自动修复能力 | 考虑在 FAIL 时自动生成修复建议（如缺失文件的预期路径） |
| D9 — 跨平台路径兼容性 | 确保路径比较在 Windows 反斜杠和 Unix 正斜杠之间正常工作 |
