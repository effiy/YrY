...
**Why it matters**: YiPet 的 26 个 CDN 组件（12 个 rui-*、10 个 Yi*、4 个 business）构成了项目的 UI 基础设施。Markdown 渲染器（10 个插件）和 Mermaid 渲染器（5 个插件）都采用了可扩展的插件架构。不理解这套体系，就无法正确地新增或修改 UI 组件。

```mermaid
graph TD
    subgraph 组件注册
        CI[customElements.define] --> RC[rui-scene-card]
        CI --> RS[rui-stats-grid]
        CI --> RB[rui-breadcrumb]
        CI --> RP[rui-panel-hub]
        CI --> YM[YiModal]
        CI --> YI[YiInput]
    end
...
**Expected**: 每个 rui-* / Yi* 组件都有对应的 custom element 定义，且继承自 `HTMLElement`
**File**: `cdn/components/index.js`

## Step 2: 验证组件三件套完整性
**Action**: 遍历 `cdn/components/` 下所有子目录，确认每个组件包含 `index.js` + `index.css` + `index.html` 三个文件（`rui-loader` 例外，仅 `index.js`）
**Expected**: 25/26 组件满足三件套结构，`rui-loader` 为纯 JS 组件
**File**: `cdn/components/`
...
| `cdn/components/rui-*/` | dirs | 12 个 rui-* 组件（dashboard UI 套件） |
...
- `rui-loader` 不遵循三件套结构，是唯一的纯 JS 组件——需要在文档中注明
