...
**What this scene demonstrates**: The documentation generated by the rui-init pipeline (CLAUDE.md, README.md, data.js, and the 11 arch/test scene index.md files) must accurately reflect the actual code on disk. This scene verifies that every claim made in the documentation — website count, file paths, dependency versions, module responsibilities — can be validated against the source files. Consistency is critical: if the docs say there are 14 websites but one was deleted, or if `data.js` lists Bootstrap as a dependency of `Arter` but the CSS file is missing, downstream consumers of the documentation (developers, AI agents) will make incorrect decisions.
...
- Replace hardcoded file counts (282, 39, 89) with a dynamic count that is recalculated on each rui-init pipeline run.
