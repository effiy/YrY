...
- The `panelHub.urls` in `data.js` point to `arch/index.html` and `test/index.html`, but these index.html files are generated by a separate process (the yry-html-cdn dashboard). If that process hasn't run yet, the directories exist but the index.html files do not — this is a valid intermediate state.
