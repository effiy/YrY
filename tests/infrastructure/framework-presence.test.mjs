...
describe('第三方测试框架 — vitest 安装与配置', () => {
  describe('package.json', () => {
    ...
    it('test:legacy 脚本保留原运行器', () => {
      const pkg = JSON.parse(readFileSync(resolve(PROJECT_ROOT, 'package.json'), 'utf-8'));
      expect(pkg.scripts['test:legacy']).toBe('node tests/run.mjs');
    });
  });
  ...
  describe('vitest.config.mjs', () => {
    ...
    it('配置文件包含 test include 模式', () => {
      ...
      expect(config).toContain("tests/infrastructure/**/*.test.mjs");
    });
  ...
  describe('适配层存在', () => {
    it('vitest-adapter.mjs 已创建', () => {
      expect(existsSync(resolve(PROJECT_ROOT, 'tests/lib/vitest-adapter.mjs'))).toBe(true);
    });
  ...
  describe('遗留测试基础设施保留', () => {
    it('test-harness.mjs 未改动', () => {
      expect(existsSync(resolve(PROJECT_ROOT, 'tests/lib/test-harness.mjs'))).toBe(true);
    });

    it('run.mjs 未改动', () => {
      expect(existsSync(resolve(PROJECT_ROOT, 'tests/run.mjs'))).toBe(true);
    });

    it('helpers.mjs 未改动', () => {
      expect(existsSync(resolve(PROJECT_ROOT, 'tests/lib/helpers.mjs'))).toBe(true);
    });

    it('遗留运行器仍可执行', () => {
      const runContent = readFileSync(resolve(PROJECT_ROOT, 'tests/run.mjs'), 'utf-8');
