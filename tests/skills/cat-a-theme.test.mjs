 ...
 *
 * 验证 theme-mono/index.css 的特有样式规则和组件。
 *
 * 对应场景文档:
...
const FONTS_CSS = 'cdn/fonts\/index\.css';

describe('Cat A 主题 — theme-mono/index.css', () => {
  // ── Font dependency ──────────────────────────────────────────────
  describe('字体依赖', () => {
    it('fonts/index.css 存在', () => assert.ok(fileExists(FONTS_CSS)));
    it('fonts/index.css 定义了 JetBrains Mono', () => {
      const content = readFile(FONTS_CSS);
      assert.ok(content.includes('JetBrains Mono'), '应定义 JetBrains Mono 字体');
    });
    it('fonts/index.css 定义了 4 个 font-face', () => {
      const content = readFile(FONTS_CSS);
