...
describe("TC4 — 字体降级 (Google Fonts 不可达 → monospace 回退)", () => {
  it("theme-mono.css 定义 monospace 回退字体栈", () => {
    const css = readFile(resolve(CDN, "theme-mono.css"));
    assert.notEqual(css, null, "theme-mono.css not found");
    assert.equal(css.includes("monospace"), true,
      "theme-mono.css should include monospace fallback in font-family");
  });

  it("fonts.css 自托管字体使用 font-display: swap", () => {
    const css = readFile(resolve(CDN, "fonts.css"));
...
describe("TC5 — JS 降级 (shared.js 不可用 → HTML/CSS 仍正常渲染)", () => {
  it("shared.css 不含 display:none 依赖 JS 移除", () => {
    const css = readFile(resolve(CDN, "shared.css"));
  ...
  it("theme.css 不含必须 JS 才能显示的组件", () => {
    const css = readFile(resolve(CDN, "theme.css"));
  ...
  it("所有消费者页面有内联样式确保无 JS 时基本可读", () => {
    ...
    assert.equal(css.includes("scroll-behavior"), true, "shared.css provides base rendering");
  });

  it("shared.js Toast 降级: toast 元素不存在时 YrY.toast 创建新元素", () => {
    const js = readFile(resolve(CDN, "shared.js"));
...
// ─── TC6: CSS 降级 — theme.css 不可用 → 浏览器默认样式 ─────────────
describe("TC6 — CSS 降级 (theme.css 不可用 → 浏览器默认样式)", () => {
  it("shared.css 提供基础排版 (Reset + 导航 + 动画)", () => {
    const css = readFile(resolve(CDN, "shared.css"));
    assert.equal(css.includes("margin: 0; padding: 0"), true, "missing CSS reset");
  });

  it("Cat B 页面内联 :root 变量作为 theme.css 的二阶回退", () => {
    ...
    assert.equal(html.includes("--bg:"), true, "Cat B page inline vars serve as theme.css fallback");
  });
