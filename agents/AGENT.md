...
| **不做** | 与核心定位无关、爆炸半径大于收益、投入产出倒挂 | 记录原因到 board.md，关闭故事 |
...
**唯一产出**: `docs/.pm/board.md` + `docs/.improvement/proposals.jsonl`
...
子项目PM未在 `agents/` 目录下定义时，根pm临时兼任，在 board.md 中标注 `⚠ 代理`。
...
| 做 | 不做 |
|---|------|
| C0 预检：双边影响分析，验证 P0 完整性 | P0 缺失不进入 C2；影响链未闭合不声称闭合 |
