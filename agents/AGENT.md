...
所有 Agent 遵守 [CLAUDE.md 铁律](../CLAUDE.md#铁律)：

| 铁律 | 含义 | Agent 应用 |
|------|------|-----------|
| **验先于称** | 未运行验证命令不得声称完成/通过/修复 | 任何"已通过/已完成"声称必须有新鲜验证输出 |
| **溯先于修** | 未找到根因不得提出修复方案 | 修复前必向后追溯调用链至原始触发点 |
| **清先于进** | 模块 P0 未清零不得进入下一模块 | coder 逐模块、tester Gate A/B、reporter 三报告闭合 |
| **表达优先** | 文档必须 图 → 结构化文本 → 表，不可降级 | 所有 Agent 写入 docs/ 遵循此顺序 |
...
以下念头是你在合理化——停下来，回到 [CLAUDE.md 铁律](../CLAUDE.md#铁律)：
