...
>
> 铁律应用：**清先于进** — P0 未清零不进下一模块。**溯先于修** — 任何 bug 修复前先根因追溯。**验先于称** — 分支隔离必须在 Edit/Write 前验证，P0 清零必须有审查记录。
...
| 实施报告逐场景映射使用场景，编号一一对应 | 实施报告 §2 场景数 = 使用场景 §2 场景数 |
| 每场景深度探查全部关联源文件（路径 + 内容 + 依赖） | 实施报告 §2.1 + §2.2，文件通过 Read/Grep 独立验证 |
| 每模块审查记录留痕，P0 清零可追溯 | 实施报告 §2.3 逐场景 P0 审查 + §2.N 全模块汇总 |
| 实施报告偏差表完整记录「评审 vs 实际」 | 每场景 §2.4 偏差表，有原因 + 影响 + 优先级 |
| 源码依赖图覆盖全故事所有场景的文件级关系 | 实施报告 §3 mermaid + 依赖矩阵 |
| 测试文件按场景组织，覆盖→源文件映射可追溯 | 实施报告 §4 测试文件表 + 测试→源文件映射 |
| 效果验证逐场景可独立复现 | 实施报告 §5 每场景含 curl 或截图 |
