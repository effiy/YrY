...
```mermaid
flowchart TB
    DOC["04-测试用例评审<br/>pm 委派后产出"]:::doc --> GA{"Gate A<br/>测试先行"}
    ...
    GB -->|"✅ 通过"| DONE["交接 reporter<br/>写测试报告 07"]:::done
...
|------|------|---------|---------|------|
| **Gate A** | 编码前 | 04-测试用例评审.md 不存在 | `skip-gate-a` | 单行 CSS/文案变更 |
...
| 6 | 无测试覆盖不通过 Gate A | 空 04-测试用例评审.md 直接通过 |
...
flowchart LR
    subgraph in["归 tester"]
        I1["AC 设计与执行"]:::in
        I2["测试评审 04<br/>+ 测试报告 07"]:::in
        I3["用例 / 数据准备"]:::in
    end
    subgraph out["不归 tester"]
        O1["业务规则定义<br/>→ pm"]:::out
        O2["实施报告 05/06<br/>→ coder"]:::out
