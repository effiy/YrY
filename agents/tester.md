...
```mermaid
flowchart TB
    DOC["05-测试用例评审<br/>pm 委派后产出"]:::doc --> GA{"Gate A<br/>测试先行"}
...
|------|------|---------|---------|------|
| **Gate A** | 编码前 | 05-测试用例评审.md 不存在 | `skip-gate-a` | 单行 CSS/文案变更 |
...
| 6 | 无测试覆盖不通过 Gate A | 空 05-测试用例评审.md 直接通过 |
