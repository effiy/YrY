...
```mermaid
flowchart TB
    DOC["{project}-测试设计<br/>pm 委派后产出"]:::doc --> GA{"Gate A<br/>测试先行"}
...
|------|------|---------|---------|------|
| **Gate A** | 编码前 | {project}-测试设计.md 不存在 | `skip-gate-a` | 单行 CSS/文案变更 |
...
| 6 | 无测试覆盖不通过 Gate A | 空 {project}-测试设计.md 直接通过 |
...
- "{project}-测试设计.md 是空的，参考设计文档补几个就行"
- "环境专项用例不影响功能，跳过"
