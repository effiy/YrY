...
>
> 铁律应用：**清先于进** — Gate A 未通过不编码，Gate B > 2 轮阻断。**验先于称** — 测试执行必须有新鲜输出，不可依赖"上次运行"。**溯先于修** — 失败用例修复前必须定位根因。
...
|------|------|---------|---------|------|
| **Gate A** | 编码前 | 测试设计.md 不存在 | `skip-gate-a` | 单行 CSS/文案变更 |
...
| 6 | 无测试覆盖不通过 Gate A | 空 测试设计.md 直接通过 |
...
flowchart LR
    ...
    S2 --> S3["测试报告逐场景<br/>映射使用场景 §2"]:::sig
    S3 --> S4["测试→源文件映射<br/>可交叉引用实施报告依赖图"]:::sig
    S4 --> S5["Gate B 判定<br/>基于逐场景数据"]:::sig
...
| 测试报告逐场景映射使用场景 §2 | 测试报告 §2 场景数 = 使用场景 §2 场景数，编号一一对应 | 补缺失场景测试 |
| 测试→源文件映射交叉引用实施报告依赖图 | 每个源文件有对应测试覆盖（直接/间接/声明跳过） | 补源文件覆盖 |
| Gate B 判定基于逐场景数据 | 逐场景通过率 + P0 清零 + 修复 ≤2 轮 | 退回 coder 修复 |
...
| "测试设计文档是空的，我临时补几个" | 空的 Gate A = skip-gate-a 阻断。tester 补用例是 Gate A 的前置条件。 |
