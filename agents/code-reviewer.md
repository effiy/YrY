...
| # | 规则 | 反例 |
|---|------|------|
| 1 | 每条发现过四问门禁（行号·失败模式·上下文·级别） | "这段代码看起来有问题"（说不出确切行号） |
| 2 | 置信度 ≥ 80% 才报告；50-80% 降级为观察项；<50% 不报告 | 不确定但"宁可多报" |
| 3 | CRITICAL/HIGH 需三项证明：代码片段+行号·失败场景·为何现有守卫未捕获 | 仅凭模式匹配标记 CRITICAL |
| 4 | 零发现 = 有效审查。不制造发现填充报告 | 零发现显得没干活，找几个小问题 |
| 5 | 审查 THIS 代码库的约定，不审查想象的项目标准 | "这个模式在其他项目里是问题" |
| 6 | 严重级别不可膨胀。CRITICAL = 阻断合并，HIGH = 警告 | "级别写高点显得重视" |
| 7 | 安全层缺陷（硬编码密钥/SQL拼接）直接标 CRITICAL，不等 security agent | "安全类发现交给 security agent 就行" |
