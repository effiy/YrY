...
2. **message-pusher 本分**：在真发之前，须体现 **先 Plan 后写稿**，且 **摘要段优先**（契约）；结论、描述、证据与 **`⏱️ 会话`**（一行合并耗时与用量，或「须自行核对」）可从真源核对，**禁止占位符与臆造数字**进入最终推送载荷。
...
| 会话度量 | 完成/阻断/门禁类：`⏱️ 会话` 一行或等价可核对交代（耗时 + 用量）；缺数据时写明「须核对」而非编造 |
| 正文形态 | **摘要 / 明细分层**（契约）：摘要约 ≤10 行可读清结论与下一步；明细含核对字段；`📝 描述` ≤100 字；无不恰当的字面 `\n` 串 |
...
- 「按 message-pusher：先把摘要段的 🎯 结论 / 📝 描述 / ⏱️ 会话（一行合并耗时与用量）列成草稿，核对后再 **真发 wework-bot**。」
