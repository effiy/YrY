...
**故事**：作为交付负责人，希望 implement-code 严格执行“先测后写、写后自动冒烟”：写代码前必须在真实场景完成最小可用测试，写完后必须由 AI 自动跑通主流程冒烟，任何一步缺失都应阻断。
