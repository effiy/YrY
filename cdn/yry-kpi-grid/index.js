...
(function () {
  ...
  if (!window.Vue) {
    console.warn('[YryKpiGrid] Vue 3 未加载,组件已跳过注册。请先引入 vue.global.prod.js');
    return;
  }
  ...
  if (!script || !script.src) {
    console.warn('[YryKpiGrid] 无法获取当前脚本 URL,组件已跳过注册');
    return;
  }
  var scriptUrl;
  try {
    ...
    console.warn('[YryKpiGrid] 解析脚本 URL 失败:', e);
    return;
  }
  var templateUrl = new URL('index.html', scriptUrl).href;

  function buildComponent(templateHTML) {
    return name: 'YryKpiGrid',
      props: { kpis: { type: String, default: '[]' }, cols: { type: [String, Number], default: 6 } },
      computed: { parsedKpis: function () { try { return JSON.parse(this.kpis); } catch (e) { return []; } } },
      template: templateHTML;
  }

  function fireReady() {
    document.dispatchEvent(new CustomEvent(READY_EVENT, { detail: { component: 'YryKpiGrid' } }));
  }

  var timedOut = false;
  var timeoutId = setTimeout(function () {
    timedOut = true;
    console.error('[YryKpiGrid] 模板加载超时 (' + LOAD_TIMEOUT_MS + 'ms):', templateUrl);
  }, LOAD_TIMEOUT_MS);
  ...
  fetch(templateUrl, { credentials: 'same-origin' })
    ...
    .then(function (htmlText) {
      ...
      window.YryKpiGrid = buildComponent(templateHTML);
    ...
    .catch(function (err) {
      if (timedOut) return;
      clearTimeout(timeoutId);
      console.error('[YryKpiGrid] 模板加载失败:', err, '· URL:', templateUrl);
    });
})();