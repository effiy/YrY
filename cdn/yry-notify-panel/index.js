...
(function () {
  ...
  if (!window.Vue) {
    console.error('[YryNotifyPanel] Vue 3 未加载,请先引入 vue.global.prod.js');
    return;
  }
  ...
  if (!script || !script.src) {
    console.error('[YryNotifyPanel] 无法获取当前脚本 URL,组件已跳过注册');
    return;
  }
    ...
    return {
      ...
      computed: {
        ...
        healthSparkline: function () {
          ...
          return {
            bars: bars, trendIcon: trendIcon, trendClr: trendClr,
            diff: Math.abs(lastS - firstS),
            from: lastN[0].date, to: lastN[lastN.length - 1].date, count: lastN.length
          };
        },
        dateGroups: function () {
          var list = this.filteredItems;
        ...
        counts: function () {
          return {
            ...
            health: this.allItems.filter(function (x) { return x.type === 'health'; }).length,
            loop: this.allItems.filter(function (x) { return x.type === 'loop'; }).length,
            trend: this.allItems.filter(function (x) { return x.type === 'trend'; }).length,
            analysis: this.allItems.filter(function (x) { return x.type === 'analysis'; }).length
          };
        },
      ...
      methods: {
        ...
        escHtml: function (s) {
          return window.PanelHub ? window.PanelHub.escHtml(s) : (window.YrY && window.YrY.escHtml ? window.YrY.escHtml(s) : String(s));
        },
        ...
        renderTimeTitle: function (item) {
          if (!item.date) return '';
          if (item.type === 'health' && item.timeText) return this.escHtml(item.date + ' ' + item.timeText);
        ...
        healthDimScores: function (m) {
          ...
          return entries.slice(0, 8).map(function (dn) {
            return {
              name: this.escHtml(dn.name), score: dn.score,
        ...
        trendKeywords: function (item) {
          ...
          return item.keywords.slice(0, 8).map(function (k) { return this.escHtml(k); }.bind(this));
        },
        ...
        analysisIssues: function (m) {
          ...
          return m.issues.slice(0, 3).map(function (iss) {
            return { text: this.escHtml(iss.msg), level: iss.level === 'fail' ? 'fail' : iss.level === 'warn' ? 'warn' : 'info' };
          }.bind(this));
        },
        ...
        registerWithHub: function () {
          var self = this;
          if (window.PanelHub) {
            window.PanelHub.register('notify', 'notifyPanel', 'notifyOverlay', function () {
        ...
        fetchAll: async function () {
          ...
          try {
            ...
            var healthData = results[0], loopData = results[1], trendData = results[2], analysisData = results[3];
            var unified = [];
            ...
            loopData.forEach(function (lr) {
              unified.push({
                type: 'loop', href: lr.file, basePath: './自循环报告/', label: lr.label || '',
                date: lr.date || '', timeId: lr.timeId || '', meta: lr.meta || null
              });
            });
            trendData.forEach(function (tr) {
              unified.push({
                type: 'trend', href: tr.file, basePath: './趋势报告/', label: tr.label || '',
                date: tr.date || '', source: tr.source || '',
                keywords: tr.keywords || [], meta: tr.meta || null
              });
            });
            analysisData.forEach(function (ar) {
              unified.push({
                type: 'analysis', href: ar.file, basePath: './项目分析/', label: ar.label || '',
            ...
            var healthItems = this.allItems.filter(function (x) { return x.type === 'health'; });
        ...
        fetchHealthReports: async function () {
          try {
            ...
            return deduped;
          } catch (e) { console.warn('[YryNotifyPanel] 健康报告 fetch 失败: ' + e.message); return []; }
        },
        fetchLoopReports: async function () {
          ...
          } catch (e) { console.warn('[YryNotifyPanel] 自循环报告 fetch 失败: ' + e.message); return []; }
        },
        fetchTrendReports: async function () {
          try {
            ...
            return data.map(function (r) {
              var src = r.source || '';
              return {
                file: r.file, date: r.date, source: r.source,
                label: (iconMap[src] || '📡') + ' ' + (src || 'trend') + ' · ' + r.date,
                meta: { ok: r.ok, trend: r.trend, items: r.items }
              };
            });
          } catch (e) { console.warn('[YryNotifyPanel] 趋势报告 fetch 失败: ' + e.message); return []; }
        },
        fetchAnalysisReports: async function () {
          try {
            ...
            return data.map(function (r) {
              return {
                file: r.file, date: r.date, source: 'proj-analysis',
          ...
          } catch (e) { console.warn('[YryNotifyPanel] 项目分析 fetch 失败: ' + e.message); return []; }
        }
      },
      mounted: function () {
        var self = this;
  ...
  fetch(templateUrl, { credentials: 'same-origin' })
    ...
    .then(function (htmlText) {
      ...
      if (typeof window.Vue.defineCustomElement !== 'function') {
        console.error('[YryNotifyPanel] Vue.defineCustomElement 不可用,跳过注册');
        return;
      }
      var CE = window.Vue.defineCustomElement(component, { shadowRoot: false });
      customElements.define(TAG_NAME, CE);

      document.dispatchEvent(new CustomEvent(READY_EVENT, { detail: { component: 'YryNotifyPanel' } }));
    })
    .catch(function (err) {
      if (timedOut) return;
      clearTimeout(timeoutId);
