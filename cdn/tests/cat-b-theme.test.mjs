 ...
 *
 * 验证 theme/index.css 的特有样式规则和 14 个组件。
 *
 * 对应场景文档:
...
describe('Cat B 主题 — theme/index.css', () => {
  ...
  describe('B 类组件覆盖', () => {
    ...
    for (const comp of B_COMPONENTS) {
      it(`包含 .yry-${comp} 样式`, () => {
        assert.ok(content.includes(`yry-${comp}`), `theme/index.css 应包含 ${comp} 组件样式`);
      });
    }
  });
  ...
  describe('动画', () => {
    it('包含 fadeInUp 关键帧', () => {
      // Animations are inline in HTML, but theme/index.css may have some
      const content = readFile(THEME_CSS);
      assert.ok(content.length > 2000, 'theme/index.css 应有足够内容');
    });
  });
});
