...
(function () {
  ...
  if (!window.Vue) {
    console.warn('[YryVerifyReportFoot] Vue 3 未加载,组件已跳过注册。请先引入 vue.global.prod.js');
    return;
  }
  ...
  if (!script || !script.src) {
    console.warn('[YryVerifyReportFoot] 无法获取当前脚本 URL,组件已跳过注册');
    return;
  }
  var scriptUrl;
  try {
  ...
  function fireReady() {
    document.dispatchEvent(new CustomEvent(READY_EVENT, { detail: { component: 'YryVerifyReportFoot' } }));
  }

  var timedOut = false;
  var timeoutId = setTimeout(function () {
    timedOut = true;
    console.error('[YryVerifyReportFoot] 模板加载超时 (' + LOAD_TIMEOUT_MS + 'ms):', templateUrl);
  }, LOAD_TIMEOUT_MS);
  ...
  fetch(templateUrl, { credentials: 'same-origin' })
    ...
    .then(function (htmlText) {
      ...
      window.YryVerifyReportFoot = buildComponent(templateHTML);
    ...
    .catch(function (err) {
      if (timedOut) return;
      clearTimeout(timeoutId);
      console.error('[YryVerifyReportFoot] 模板加载失败:', err, '· URL:', templateUrl);
    });
})();