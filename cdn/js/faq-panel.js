...
(function() {
  ...
  var app = Vue.createApp({
    ...
    template: /* html */`<div style="padding:10px 20px;font-size:.64rem;color:var(--text-muted);border-bottom:1px solid rgba(255,255,255,.04);line-height:1.6"><strong>YrY 知识库</strong> — ${faqs.length} 个常见问题，覆盖：系统概念 · 命令使用 · 面板关系 · 健康检查 · 设计原则 · 版本管理。<br>点击问题展开答案。彩色链接可快速打开对应面板。</div><ul class="faq-list"><li v-for="(item, i) in faqs" :key="i" class="faq-item"><div class="faq-q" :data-index="i"><span class="faq-q-icon">Q:</span>{{ item.q }}</div><div class="faq-a" v-html="item.a"></div></li></ul>`
  });

  function mountApp() {
    function doMount() {
      if (faqPanelBody) app.mount(faqPanelBody);
      H.register('faq', 'faqPanel', 'faqOverlay', null);
      /* ── Accordion ──────────────────────────── */
