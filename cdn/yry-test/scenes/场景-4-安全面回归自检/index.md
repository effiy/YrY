...
| 诊断 | 触发? | 证据 | 说明 |
|------|-------|------|------|
| D0 基线偏离 | 否 | 四条不可妥协安全底线（密钥不落盘/认证不可绕过/输入必校验/规约完整性）每次检查 | 基线稳定 |
| D1 效率退化 | 否 | 基于 grep 模式匹配的轻量级扫描，秒级完成全量检查 | 性能可控 |
| D2 质量热点 | 否 | 每条安全发现附精确文件位置 + 被匹配模式，可独立复核 | 证据确凿 |
| D3 复杂度增长 | 否 | 六面安全检查独立执行、独立判定，一面阻断不影响其余 | 模块解耦 |
| D4 流程退化 | 否 | 安全扫描嵌入日常变更流程，不依赖安全专家手动审计 | 自动化 |
| D5 依赖退化 | 否 | 纯 grep + 文件系统扫描，零外部安全工具依赖 | 自包含 |
| D6 文档过时 | 否 | 安全约束基线文件与代码同步版本控制 | 基线可追溯 |
| D7 配置漂移 | 否 | 安全扫描模式列表可配置化，变更受 git 版本控制 | 配置一致 |

### §4.2 改进清单

| # | 改进项 | 优先级 | 状态 |
|---|--------|--------|:--:|
| 1 | 建立安全约束基线快照 `tests/fixtures/security-baseline.json` | P1 | 规划中 |
| 2 | 扩展密钥扫描模式（AWS/GCP/Azure 云凭据） | P2 | 待评估 |
| 3 | 添加 `tests/run.mjs --security` 专用筛选参数 | P2 | 待评估 |
| 4 | commit hook 安全扫描阻断（硬编码密钥阻止提交） | P1 | 规划中 |
| 5 | 安全扫描结果集成至健康报告仪表板 | P2 | 待评估 |
