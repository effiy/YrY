...
(function () {
  ...
  /* ── Path resolver (兼容 PanelHub.PATHS / YRY_PATHS) ─────────────── */
  ...
  /* ── Cron parsing helpers (纯函数,与 UI 解耦) ────────────────────── */
  function cronDescription(expr) {
    ...
    if (m.indexOf(',') !== -1 && h === '*' && dom === '*' && mon === '*' && dow === '*') return '每小时第 ' + m + ' 分';
    if (m.indexOf('/') === 1 && h === '*' && dom === '*' && mon === '*' && dow === '*') {
      var n = parseInt(m.substring(2), 10);
      if (n) return '每 ' + n + ' 分钟';
    }
    if (/^\d+$/.test(m) && h === '*' && dom === '*' && mon === '*' && dow === '*') return '每小时第 ' + m + ' 分';
    if (/^\d+$/.test(m) && /^\d+$/.test(h) && dom === '*' && mon === '*' && dow === '*') return '每天 ' + h + ':' + m.padStart(2, '0');
    if (/^\d+$/.test(m) && /^\d+$/.test(h) && /^\d+$/.test(dom) && /^\d+$/.test(mon) && dow === '*') return mon + '月' + dom + '日 ' + h + ':' + m.padStart(2, '0');
  ...
  function cronStatus(item) {
    ...
    try {
      var parts = String(item.cron || '').trim().split(/\s+/);
      ...
      if (/^\d+$/.test(parts[0]) && /^\d+$/.test(parts[1]) && parts[2] === '*' && diff < 86400000) return 'active';
    ...
    return {
      ...
      methods: {
        ...
        _syncExternalCounters: function () {
          ...
          if (taskCount) taskCount.textContent = this.cronData.length > 0 ? this.cronData.length + ' 个任务' : '0 个任务';
          if (badge) badge.textContent = this.cronData.length > 0 ? String(this.cronData.length) : '';
        },
        onRefresh: function () {
          this.fetchCron();
        },
      ...
      mounted: function () {
        ...
        document.dispatchEvent(new CustomEvent(READY_EVENT, { detail: { component: 'YryCronPanel' } }));
      }
    };
  }
  ...
  fetch(templateUrl, { credentials: 'same-origin' })
    ...
    .then(function (htmlText) {
      ...
      if (typeof window.Vue.defineCustomElement !== 'function') {
        console.error('[YryCronPanel] Vue.defineCustomElement 不可用,跳过注册');
        return;
      }
