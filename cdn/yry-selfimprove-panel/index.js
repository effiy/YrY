...
(function () {
  ...
  function escHtml(s) {
    if (window.PanelHub && typeof window.PanelHub.escHtml === 'function') return window.PanelHub.escHtml(s);
    ...
    return {
      ...
      computed: {
        ...
        hero: function () {
          ...
          if (this.siRaw.length > 1) {
            ...
            spark = sampled.map(function (e, i) {
              ...
              return { color: GRADE_COLORS[e.grade] || '#555', size: last ? '10px' : '5px',
          ...
          return {
            score: s, grade: g, label: g, color: info.color, bg: info.bg,
            trend: trend, spark: spark,
            diagCount: diagCount,
        ...
        summary: function () {
          ...
          var sorted = d.dimSummary.slice().sort(function (a, b) { return a.avgScore - b.avgScore; });
          ...
          return {
            overall: overall, overallIcon: overallIcon, diagCount: diagCount,
            dimPass: pass, dimWarn: warn, dimFail: fail,
        ...
        recommendations: function () {
          ...
          if (d.signals) {
            d.signals.forEach(function (s) {
              recs.push({
                source: s.type === 'regression' ? '维度退化' : s.type === 'warning' ? '诊断告警' : '改进信号',
        ...
        testScore: function () {
          ...
          var testDims = d.dimSummary.filter(function (dim) { return dim.dim && (dim.dim.startsWith('em_') || dim.dim === 'comp_qual'); });
        ...
        coreDims: function () {
          ...
          var items = dims.map(function (dim) {
            ...
            return {
              icon: s >= 80 ? '●' : s >= 60 ? '◐' : '○',
              label: escHtml(dim.label), weight: w, score: s,
        ...
        dimMatrix: function () {
          ...
          return d.dimSummary.map(function (dim) {
            ...
            var trendClr = dim.trend > 3 ? '#22c55e' : dim.trend < -3 ? '#ef4444' : '';
            return {
              label: escHtml(dim.label), score: s, color: scoreColor(s),
              cls: mxCls(s), trendIcon: trendIcon, trendClr: trendClr,
              trendAbs: trendIcon ? Math.abs(dim.trend) : 0,
              title: escHtml(dim.label) + ' · 均分' + s + ' · 最近' + dim.recentAvg + ' · ' + dim.entries + '次'
            };
          });
        },
        /* ── Active diagnostics ────────────────── */
        activeDiags: function () {
          ...
          return active.map(function (diag) {
            ...
            var info = DIAG_INFO[diag.id] || {};
            return {
              id: diag.id, label: escHtml(label), count: diag.count, rate: diag.rate,
        ...
        branches: function () {
          ...
          return d.branchSummary.map(function (b) {
            return {
              name: escHtml(b.name), score: b.avgScore, color: scoreColor(b.avgScore),
        ...
        componentHealth: function () {
          ...
          types.forEach(function (t) {
            ...
            items.push({
              icon: t.icon, label: t.label, score: data.avgScore || 0,
              barW: Math.max(3, data.avgScore || 0), count: data.count,
              color: scoreColor(data.avgScore || 0)
            });
          });
          return {
            items: items, overallAvg: ch.overallAvg, totalComponents: ch.totalComponents || 0,
        ...
        periodData: function () {
          ...
          if (prev && current.avgScore !== prev.avgScore) {
            ...
            delta = { text: dSign + (current.avgScore - prev.avgScore) + ' 分 vs 上期', up: current.avgScore > prev.avgScore };
          }
          var gradeDist = [];
          ...
          return {
            empty: false, score: current.avgScore, grade: current.topGrade, color: scoreColor(current.avgScore),
        ...
        periodDims: function () {
          ...
          var buckets = this.siPeriod === 'daily' ? d.daily : this.siPeriod === 'weekly' ? d.weekly : d.monthly;
          if (!buckets || buckets.length === 0) return [];
          var current = buckets[buckets.length - 1];
          if (!current.dimScores) return [];
          var entries = Object.keys(current.dimScores).map(function (k) { return { name: k, score: current.dimScores[k] }; });
          entries.sort(function (a, b) { return a.score - b.score; });
          return entries.slice(0, 9).map(function (dn) {
            return {
              label: escHtml(dn.name), score: dn.score,
              barW: Math.max(3, dn.score), color: scoreColor(dn.score),
              trendIcon: '', trendAbs: null, trendClr: ''
            };
          });
        },
        /* ── Period diagnostics ────────────────── */
        ...
        todayEntries: function () {
          ...
          return this.siRaw.filter(function (e) { return e.timestamp && e.timestamp.slice(0, 10) === today; })
            ...
            .map(function (e) {
              ...
              return {
                time: t, score: e.composite, color: scoreColor(e.composite),
        ...
        periodBranches: function () {
          ...
          return pd.current.branches.map(function (b) {
            return { name: escHtml(typeof b === 'string' ? b : b.name || ''),
        ...
        periodHistory: function () {
          ...
          return buckets.slice(0, buckets.length - 1).reverse().slice(0, maxBuckets).map(function (b) {
            ...
            var topDiags = (b.topDiags || []).map(function (diag) {
              return { id: diag.id, label: escHtml(diag.label), count: diag.count, hot: diag.rate >= 50 };
            });
            return { label: escHtml(label), score: b.avgScore, grade: b.topGrade || '', entries: b.entries,
                     deltaText: deltaText, topDiags: topDiags };
          });
        }
      },
      methods: {
        setPeriod: function (p) { this.siPeriod = p; },
        ...
        fetchData: async function () {
          ...
          try {
            var results = await Promise.all([
              fetch(paths.summaryJson).then(function (r) { return r.ok ? r.json() : null; }),
              fetch(paths.healthTrend).then(function (r) { return r.ok ? r.text() : ''; }).catch(function () { return ''; })
            ]);
            var data = results[0];
            if (!data) throw new Error('summary.json 不可用');

            var jsonlText = results[1] || '';
            this.siRaw = jsonlText.trim().split('\n').filter(Boolean).map(function (line) {
        ...
        _syncExternalCounters: function () {
          ...
          if (d && d.latest && badge) {
            ...
            badge.title = '最新评分: ' + latest.composite + '分 ' + latest.grade + '级 | 共 ' + (d.totalEntries || 0) + ' 条记录';
          } else if (badge && d && d.totalEntries) {
            badge.textContent = d.totalEntries > 99 ? '99+' : String(d.totalEntries);
            badge.className = 'badge';
          }
          if (live && d && d.latest) {
            var lg = d.latest, lGrade = lg.grade || '', lScore = lg.composite;
            live.textContent = lScore + ' ' + lGrade;
            live.className = 'si-live-grade ' + (lGrade ? lGrade : 'loading');
          }
          if (count) count.textContent = (d && d.dateRange) ? (d.dateRange.from + ' → ' + d.dateRange.to) : ((d && d.totalEntries) || 0) + ' 条';
          if (footer) {
            var now = new Date();
            footer.textContent = '更新 ' + now.getHours().toString().padStart(2, '0') + ':' + now.getMinutes().toString().padStart(2, '0');
          }
        },
        onRefresh: function () {
          this.fetchData();
        ...
        registerWithHub: function () {
          if (window.PanelHub) {
            var self = this;
            window.PanelHub.register('selfimprove', null, 'selfimprovePanel', 'selfimproveOverlay', function () {
  ...
  fetch(templateUrl, { credentials: 'same-origin' })
    ...
    .then(function (htmlText) {
      ...
      if (typeof window.Vue.defineCustomElement !== 'function') {
        console.error('[YrySelfImprovePanel] Vue.defineCustomElement 不可用,跳过注册');
        return;
      }
