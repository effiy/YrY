...
> | v5.4.0 | 2026-06-10 | deepseek-v4-pro | 🌿 feat/yry-self-improve | 📎 [CLAUDE.md](../../../CLAUDE.md) |
...
YrY 的自改进机制已经沉淀为两份规约（`skills/*/rules/self-improve.md` + `skills/rui/AGENT.md`（§self-improve 段）），定义了四段闭环（观察→诊断→改进→评估）、九级诊断规则（D0-D8）、四级效果评估（E1-E4）、五种提案类型和两条经验升级路径。但这份机制目前散落在规约文本中——诊断规则与数据采集的契约关系未成形、提案路由的触发条件缺少端到端场景覆盖、效果评估的参照基准尚未从历史数据中抽取、经验技能化的升级路径缺少逐阶段可追踪的决策记录。
...
- 🎯 **诊断可判定** — D0-D8 每条规则都有明确的信号阈值和基线依据，可脚本化校验
...
作为 系统演进者，我想要 一条完整的自改进闭环——从执行记忆中自动采集数据、按 D0-D8 规则诊断偏差、为每条诊断生成可追踪的提案、用量化的 E1-E4 指标评估效果，以便 每个故事走完管线后自动产出改进建议，同一模式反复出现时自动升级为项目规则或 skill，使系统在运行中自我纠偏而非依赖人工复盘。

优先级 **P0**。范围边界：覆盖自改进管线的全部四个阶段（数据采集→诊断→提案→评估），含诊断决策表、提案类型路由、效果评估矩阵和经验技能化升级路径。不涉及各诊断规则的具体算法实现细节，不涉及外部监控系统。依赖：执行记忆数据源（`execution-memory.jsonl` · `rui-state.json` · `proposals.jsonl` · Git diff）、基线规约（`CLAUDE.md` · `skills/*/rules/` · `skills/rui/AGENT.md`）。
...
flowchart TD
    subgraph 输入["数据源"]
        ...
        BASE["基线<br/>CLAUDE.md · skills/*/rules/*.md · skills/rui/AGENT.md"]:::src
    end
...
| 2 | 触发诊断分析 | 故事管线完成后，用户需要了解是否有改进信号 | 查阅 D0-D8 诊断决策表 → 对照当前数据与基线阈值 → 识别触发的诊断项 | 获得完整的诊断判定结果和触发的改进信号列表 |
...
- **FP2 诊断引擎** — 实现 D0-D8 九级诊断规则的信号判定。每条规则标注信号阈值、基线依据（引用具体规约文件）、置信度条件和误报风险。输入：采集数据 + 基线规约。输出：诊断决策表（诊断项 · 信号 · 阈值 · 基线引用 · 判定结果 · 置信度）。阻断：任一诊断项无基线引用或阈值定义。
...
- **FP5 经验技能化** — 检测同类型提案连续触发次数，达到阈值后触发升级流程。定义从提案到规则/skill 的五条升级路径（process → skills/*/rules/code-pipeline.md · quality → skills/rui/AGENT.md §tester/§coder 段 · refactor → skills/*/rules/code-pipeline.md · security → skills/rui/AGENT.md §security 段 · skill → skills/ 或 rules/ 新条目）。输入：proposals.jsonl 历史。输出：目标规约文件增量更新 + 来源提案 ID 标注。阻断：目标文件已含类似规则时更新现有规则而非重复创建。
...
| R3 | 每条诊断必须引用基线文件作为判定依据（CLAUDE.md / skills/*/rules/*.md / skills/rui/AGENT.md） | 逐条诊断检查 baseline_ref 字段引用有效文件路径 | A |
...
flowchart TD
    DATA["数据采集<br/>5 类数据源"]:::src --> DIAG{"D0-D8 诊断<br/>信号 vs 基线阈值"}
...
- D0-D8 九级诊断规则的信号判定与基线锚定 (FP2) — 含阈值定义、基线引用、置信度评估
...
| AC2 | 采集数据就绪，基线文件可读取 | 系统执行 D0-D8 诊断 | 输出诊断决策表，每条判定有信号值、阈值、基线引用和判定结果 | Gate A |
...
- 基线文件（CLAUDE.md / skills/*/rules/*.md / skills/rui/AGENT.md）保持更新 → 诊断前重新加载基线文件验证引用有效性 (FP2)
...
| Story 1 FP2 | D0-D8 诊断规则和基线锚定 | 场景-2-诊断引擎/index.md | 九级诊断的信号判定 · 阈值定义 · 基线引用 · 诊断决策表 | 待生成 |
...
| 关联故事 | 关系类型 | 说明 |
|---------|---------|------|
| `yry-arch` | 数据消费 | 自改进诊断需要引用基线文件（CLAUDE.md / skills/*/rules/*.md / skills/rui/AGENT.md），yry-arch 提供的模块拓扑和管线阶段定义为诊断引用提供验证依据 |
...
| 维度 | 指标 | 公式 | 阈值 | 数据源 |
|------|------|------|:---:|------|
| 诊断精度 | 触发准确率 | `correct_triggers / total_triggers` | ≥ 0.8 | 人工评审 |
| 提案质量 | 闭合率 | `closed / total_proposals` | ≥ 0.7 | proposals.jsonl |
| 改进效果 | E1 改善幅度 | `after - before` | > 0 | 诊断引擎 |
| 响应时效 | 平均闭合时长 | `Σ(closed_at - created_at) / N` | ≤ 7d | proposals.jsonl |
| 经验固化 | 升级率 | `promoted / total_patterns` | ≥ 0.2 | skills/*/rules/ 变更 |
| 记忆命中 | 注入有效率 | `used_injections / total_injections` | ≥ 0.5 | .memory/ |
...
flowchart LR
    OBSERVE["观察<br/>场景-1"]:::s --> DIAGNOSE["诊断<br/>场景-2"]:::s
    DIAGNOSE --> PROPOSE["提案<br/>场景-3"]:::s
    PROPOSE --> EVALUATE["评估<br/>场景-4"]:::s
    EVALUATE --> PROMOTE["升级<br/>场景-5"]:::s
    PROMOTE -.->|"固化经验"| OBSERVE
...
| 阶段 | 耗时 | 频率 | 内存 | 输出 |
|------|:---:|:---:|:---:|:---:|
| 观察 | ≤ 200ms | 每故事 | ≤ 21MB | 采集报告 |
| 诊断 | ≤ 500ms | 每故事 | ≤ 25MB | 决策表 |
| 提案 | ≤ 100ms | 触发时 | ≤ 5MB | proposals.jsonl |
| 评估 | ≤ 2s | 闭合时 | ≤ 50MB | 评估报告 |
| 升级 | ≤ 1s | 月级 | ≤ 10MB | 规则文件 |
...
| 文件 | 类型 | 增长率 | 治理策略 | 保留期 |
|------|------|------|------|:---:|
| `execution-memory.jsonl` | append-only | 每故事 +1 | 滚动 100 条 | 30 天 |
| `rui-state.json` | 覆盖 | 每阶段 +1 | 自动覆盖 | 当前 |
| `proposals.jsonl` | append-only | 每提案 +1 | 归档分片 | 永久 |
| `health-trend.jsonl` | append-only | 每日 +1 | 滚动 365 天 | 1 年 |
| `drift-trend.jsonl` | append-only | 每日 +1 | 滚动 90 天 | 90 天 |
| `summary.md` | 覆盖 | 每故事 +1 | 自动压缩 | 当前 |

### 自改进成熟度模型

| 级别 | 特征 | 度量 | 目标 |
|:---:|------|------|------|
| L1 反应式 | 人工触发改进 | 自动化率 0% | 启动 |
| L2 半自动 | 诊断自动 + 提案人工 | 自动化率 50% | 当前 |
| L3 全自动 | 诊断 + 提案 + 评估自动 | 自动化率 80% | 目标 |
| L4 自优化 | 经验自动升级为规则 | 自动化率 95% | 远期 |
| L5 自进化 | 跨项目知识迁移 | 自动化率 100% | 愿景 |

---

> **回溯链**
>
> - 来源：本故事由 `/rui 添加自改进机制的任务故事及各个场景内容` 触发。自改进机制的定义来源于 `skills/*/rules/self-improve.md`（诊断规则 D0-D8、效果评估 E1-E4、经验技能化、记忆压缩与注入）和 `skills/rui/AGENT.md`（§self-improve 段 · 四段闭环、提案矩阵、跨会话记忆注入、自循环）。
...
> - 角色契约：[持续改进](../../../skills/rui-yry/self-improve.md)（自改进管线的核心执行角色）
> - 治理约束：[自改进流程](../../../skills/rui-yry/rules/self-improve.md)（诊断基准、提案路由、评估标准）· [代码变更治理](../../../skills/rui-code/rules/code-pipeline.md)（自改进阶段的管线位置）· [交付收口](../../../skills/rui/rules/delivery-gate.md)（自改进完成后交付三步）
> - 文档公式：[公式规约](../../../skills/rui/formulas.md) — 定义故事文档的结构化产出规范
>
> **证据标注说明**：本 Story 文档的断言基于 skills/*/rules/self-improve.md 和 skills/rui/AGENT.md（§self-improve 段）的分析推导（证据级别 B）。D0-D8 诊断数量、E1-E4 评估维度、五种提案类型的定义直接来源于规约文件（证据级别 A）。

### 变更记录

| 日期 | 版本 | 变更内容 | 触发 | 证据 |
|------|------|---------|------|------|
| 2026-06-10 | 1.0.0 | 初始化，从自改进机制需求创建：Story 1 四段闭环 + Story 2 跨会话记忆注入 | `/rui 添加自改进机制的任务故事及各个场景内容` | skills/*/rules/self-improve.md + agents/self-improve.md 分析 |
