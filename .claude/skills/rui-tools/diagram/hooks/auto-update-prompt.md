...
9. **Apply `.understandignore` exclusions** (same semantics as `/understand` Step 2.5 in `agents/project-scanner.md`).
   ...
   3. Resolve `$PLUGIN_ROOT`:
      ...
      - Validate the chosen candidate by checking `$candidate/core/dist/ignore-filter.js` exists (or legacy `$candidate/packages/core/dist/ignore-filter.js`).
   ...
   4. Write `$PROJECT_ROOT/.understand-anything/intermediate/ignore-filter.mjs`:
      ...
      const coreDir = existsSync(path.join(PLUGIN_ROOT, 'core')) ? 'core' : 'packages/core';
      const modUrl = pathToFileURL(
        path.join(PLUGIN_ROOT, coreDir, 'dist/ignore-filter.js'),
      ).href;
