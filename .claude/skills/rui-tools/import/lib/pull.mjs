...
export async function recommendPullMode(/** @type {string} */ apiUrl) {
  console.error("# rui-import pull mode -- 远端可同步故事\n");
  ...
  if (storyMap.size === 0) {
    console.error("远端无story-task-panel文件");
    return;
  }
  ...
  for (const name of storyMap.keys()) {
    console.error(`   node .claude/rui-import/sync.mjs dir=docs/story-task-panel/${name}/ mode=pull`);
  }
}
