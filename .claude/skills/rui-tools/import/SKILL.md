...
- Honors a manual trigger contract — never auto-overwrites remote without
  human intent; safe to delegate from `/rui` delivery step or `rui-claude`
  sync.

## What this skill does NOT do

- Does NOT generate document content (that's [rui-doc] · [rui-html]).
- Does NOT push notifications (that's [rui-bot]).
...
2. **Run** `node .claude/rui-import/sync.mjs [options]` — the script is the
...
- [sync.mjs](./sync.mjs) — executable entry: `node .claude/rui-import/sync.mjs [options]`
