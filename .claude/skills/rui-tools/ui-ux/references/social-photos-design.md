...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates `rui-tools-ui-ux`, `brand`, `design-system`, and `chrome-devtools` skills.
...
3. **Randomly invoke ONE of:** `/ck:rui-tools-ui-ux` OR `/ck:frontend-design` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
