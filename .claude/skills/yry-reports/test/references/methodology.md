...
> Per-facet measurement methodology for yry-report-test. The
...
Mirrors `yry-reports/files` Stage 1 exactly. The walk is shared
...
and the yry-init scene tree stay aligned. The markdown body is
regenerated; the §0–§4 headers are byte-stable.

## Loader contract

The page uses the same shared loader as `yry-reports/files`:
