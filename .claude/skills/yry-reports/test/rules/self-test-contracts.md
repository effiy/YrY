---
description: "Analyzer ↔ page data contract for yry-report-test. Invariants the analyzer MUST respect."
---

# test Contracts

> Invariants the implementing agent MUST respect while running
> `yry-report-test`. Violations are bugs in the execution, not
> the spec.
...
Inherited from `yry-reports/files` (single source of truth):
...
These caps match `yry-reports/files` Stage 1 and Stage 3.
...
report and the yry-init scene tree stay aligned. The
`scene-N-<slug>/index.md` filename and the `# §0..§4` headers
are byte-stable; the body is regenerated.

`yry-init` step 04-arch checks for §0–§4 headers (not body
content), so re-running `yry-init` after this skill is safe —
...
| `yry-init` not installed | Page still renders; markdown mirror step is skipped |
