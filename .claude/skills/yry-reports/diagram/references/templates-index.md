...
| 18 | **Clean browser console** | The yry-tools headless probe (in the yry-report-diagram workspace) reports `consoleMessages: []` and `requestFailures: []`. Three historical offenders to watch for: (a) `yry-toast` losing `callerSrc` — fixed in iteration-1 by snapshotting `document.currentScript.src`; (b) `yry-toast` `defaultConfig` missing top-level `templateId` — also iteration-1; (c) Vue 3 multi-root templates make `$el` a DocumentFragment with no `querySelector` — use template refs in `mounted()`. |
