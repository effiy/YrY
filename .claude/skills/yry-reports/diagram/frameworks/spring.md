...
- **Spring Cloud for microservices**: `@EnableDiscoveryClient`, `@EnableFeignClients`, `@EnableCircuitBreaker` enable service discovery (Eureka/Consul), declarative HTTP clients (Feign/OpenFeign), and circuit breakers (Resilience4j). Each Feign client interface → `depends_on` edge to the remote service
- **Spring Batch for data processing**: `@EnableBatchProcessing` + `Job`/`Step`/`ItemReader`/`ItemProcessor`/`ItemWriter` form a chunk-oriented processing pipeline. Each step's reader→processor→writer chain → `transforms` edges between stages
- **Event-driven with Spring Events**: `ApplicationEventPublisher.publishEvent()` and `@EventListener` / `@TransactionalEventListener` create in-process event bus. Publish sites → `publishes` edges; listener methods → `subscribes` edges
- **Spring Data REST**: `@RepositoryRestResource` auto-exposes repositories as REST endpoints. Each exported repository → `endpoint` nodes with `serves` edges from the repository
- **Reactive stack (WebFlux)**: `RouterFunction` + `HandlerFunction` replace `@RestController` for reactive, non-blocking APIs. `Mono<T>` and `Flux<T>` return types indicate reactive pipelines — create `depends_on` edges through reactive chains
- **Testing pyramid with Spring Test**: `@SpringBootTest` (integration), `@WebMvcTest` (controller slice), `@DataJpaTest` (repository slice) — each test annotation scopes which beans are loaded. `@MockBean` replaces real beans with mocks → `tested_by` edges from real bean to test
