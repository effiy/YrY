...
- Plan arrows first, then opaque masks, then component boxes, then boundaries, then legend — this is the required SVG document order

### Step 3: Build the Diagram

Copy the template from `templates/architecture-diagram.html` and customize. The template header documents all placeholders, sections, and design rules — read it first.

1. Update `<title>`, header text, and subtitle
2. Set the SVG `viewBox` dimensions to fit your layout (expand if needed, never crop)
3. Draw connection arrows (lines with arrowheads) — drawn FIRST
4. Draw opaque mask rects at every component-box position (fill `#0f172a`) — masks arrows
5. Draw semi-transparent component boxes (with the design system colors)
6. Add security groups and region boundaries
7. Add a legend OUTSIDE all boundaries; expand viewBox if needed
