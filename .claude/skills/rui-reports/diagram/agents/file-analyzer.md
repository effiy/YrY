...
- `tags` — Use localized tags when natural (e.g., translated equivalents of "entry-point", "utility") or keep English tags for universal technical terms (e.g., "middleware", "api-handler", "test")
...
```bash
cat > $OUTPUT_DIR/tmp/ua-file-analyzer-input-<batchIndex>.json << 'ENDJSON'
{
...
```bash
node <SKILL_DIR>/scripts/extract-structure.mjs \
  $OUTPUT_DIR/tmp/ua-file-analyzer-input-<batchIndex>.json \
  $OUTPUT_DIR/tmp/ua-file-extract-results-<batchIndex>.json
```
...
After the script returns, verify the output file exists and is non-empty (e.g. `test -s $OUTPUT_DIR/tmp/ua-file-extract-results-<batchIndex>.json`). Exit 0 with a missing output file means the bundled script silently no-opped — report this as a hard failure rather than proceeding to Step 3.

### Step 3 — Read the extraction results

Read `$OUTPUT_DIR/tmp/ua-file-extract-results-<batchIndex>.json`. The output format is:
...
After the script completes, read `$OUTPUT_DIR/tmp/ua-file-extract-results-<batchIndex>.json`. Use these structured results as the foundation for your analysis. Do NOT re-read the source files unless the script skipped a file or you need to understand a specific pattern that the script could not capture.
...
**Step B — Decide split.**
- If `nodeCount ≤ 60` AND `edgeCount ≤ 120`: write ONE file to `<OUTPUT_DIR>/intermediate/batch-<batchIndex>.json`. Done. Skip to Step F.
...
Write part `k` (1-indexed) to `<OUTPUT_DIR>/intermediate/batch-<batchIndex>-part-<k>.json`. Each part is a valid GraphFragment: `{ "nodes": [...], "edges": [...] }`.
