...
- Tour `title` — Write in the specified language (e.g., "Project Overview", "Application Entry", "Database Schema")
...
```bash
cat > $OUTPUT_DIR/tmp/ua-tour-input.json << 'ENDJSON'
{
...
```bash
node $OUTPUT_DIR/tmp/ua-tour-analyze.js $OUTPUT_DIR/tmp/ua-tour-input.json $OUTPUT_DIR/tmp/ua-tour-results.json
```
...
After the script completes, read `$OUTPUT_DIR/tmp/ua-tour-results.json`. Use the structural analysis as your primary guide for designing the tour. Do NOT re-read source files or re-analyze the graph -- trust the script's results entirely.
...
1. Write the JSON array to: `<OUTPUT_DIR>/intermediate/tour.json`
