...
- Layer `name` — Translate to the specified language (e.g., "API Layer", "Service Layer", "Infrastructure Layer")
...
```bash
cat > $OUTPUT_DIR/tmp/ua-arch-input.json << 'ENDJSON'
{
...
```bash
node $OUTPUT_DIR/tmp/ua-arch-analyze.js $OUTPUT_DIR/tmp/ua-arch-input.json $OUTPUT_DIR/tmp/ua-arch-results.json
```
...
After the script completes, read `$OUTPUT_DIR/tmp/ua-arch-results.json`. Use the structural analysis as the primary input for your layer decisions. Do NOT re-read source files or re-analyze imports -- trust the script's results entirely.
...
1. Write the JSON array to: `<OUTPUT_DIR>/intermediate/layers.json`
