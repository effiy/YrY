...
import { buildFileAnalysisPrompt, buildProjectSummaryPrompt, parseFileAnalysisResponse, parseProjectSummaryResponse, } from "./llm-analyzer.js";
describe("LLM Analyzer", () => {
    describe("buildFileAnalysisPrompt", () => {
        it("should include file path and content in the prompt", () => {
            const prompt = buildFileAnalysisPrompt("src/utils.ts", "export function add(a: number, b: number) { return a + b; }", "A math utility library");
        ...
        it("should include project context", () => {
            const prompt = buildFileAnalysisPrompt("app.py", "print('hello')", "A Python web server");
...
\`\`\``;
    ...
    describe("buildProjectSummaryPrompt", () => {
        ...
        it("should include sample file contents when provided", () => {
            const prompt = buildProjectSummaryPrompt(["src/app.ts"], [{ path: "src/app.ts", content: "const app = express();" }]);
