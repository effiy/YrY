    ...
    describe("Non-code file support", () => {
        ...
        it("adds non-code child nodes (definitions)", () => {
            ...
            builder.addNonCodeFileWithAnalysis("schema.sql", {
                ...
                complexity: "moderate",
                definitions: [
                    { name: "users", kind: "table", lineRange: [1, 20], fields: ["id", "name", "email"] },
                ],
            });
            const graph = builder.build();
        ...
        it("adds endpoint child nodes", () => {
            ...
            builder.addNonCodeFileWithAnalysis("schema.graphql", {
                ...
                complexity: "moderate",
                endpoints: [
                    { method: "Query", path: "users", lineRange: [5, 5] },
                ],
            });
            const graph = builder.build();
            expect(graph.nodes).toHaveLength(2);
        ...
        it("adds resource child nodes", () => {
            ...
            builder.addNonCodeFileWithAnalysis("main.tf", {
                ...
                complexity: "moderate",
                resources: [
                    { name: "aws_s3_bucket.main", kind: "aws_s3_bucket", lineRange: [1, 10] },
                ],
            });
            const graph = builder.build();
            expect(graph.nodes).toHaveLength(2);
        ...
        it("mapKindToNodeType falls back to concept for unknown kinds and warns", () => {
            ...
            builder.addNonCodeFileWithAnalysis("schema.sql", {
                ...
                complexity: "simple",
                definitions: [
                    { name: "doStuff", kind: "procedure", lineRange: [1, 10], fields: [] },
                ],
            });
            const graph = builder.build();
            ...
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Unknown definition kind "procedure"'));
            warnSpy.mockRestore();
        });
        it("skips duplicate node IDs in addNonCodeFileWithAnalysis and warns", () => {
            ...
            builder.addNonCodeFileWithAnalysis("schema.sql", {
                ...
                definitions: [
                    ...
                    { name: "users", kind: "table", lineRange: [12, 20], fields: ["id", "name"] },
                ],
            });
            const graph = builder.build();
            ...
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Duplicate node ID "table:schema.sql:users"'));
            warnSpy.mockRestore();
        });
