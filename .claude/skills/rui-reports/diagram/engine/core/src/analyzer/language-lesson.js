 ...
 */
function buildConceptPatterns(langConfig) {
 ...
 */
export function detectLanguageConcepts(node, language, langConfig) {
    const text = [
        ...node.tags,
    ...
    for (const [concept, keywords] of Object.entries(patterns)) {
        const found = keywords.some((keyword) => text.toLowerCase().includes(keyword.toLowerCase()));
        if (found) {
 ...
 */
export function getLanguageDisplayName(language, langConfig) {
 ...
 */
export function buildLanguageLessonPrompt(node, edges, language, langConfig) {
 ...
 */
export function parseLanguageLessonResponse(response) {
    try {
