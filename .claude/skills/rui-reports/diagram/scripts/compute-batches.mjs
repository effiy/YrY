 ...
 *
 * Usage:
 *   node compute-batches.mjs <project-root> <output-dir> [--changed-files=<path>]
 *
...
try {
  ...
  core = await import(pathToFileURL(resolve(PLUGIN_ROOT, 'engine/core/src/index.js')).href);
}
...
async function main() {
  ...
  if (!projectRoot || !outputDir) {
    process.stderr.write('Usage: node compute-batches.mjs <project-root> <output-dir> [--changed-files=<path>]\n');
    process.exit(1);
  }
  ...
  const scanPath = join(outputDir, 'intermediate', 'scan-result.json');
  ...
  const outPath = join(outputDir, 'intermediate', 'batches.json');
