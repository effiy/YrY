...
// ---------------------------------------------------------------------------
// Resolve @rui-reports/diagram
//
// Two-step resolution: try the workspace-linked package first, fall back to
// the local engine source. pathToFileURL() is required on Windows because
// dynamic import() of raw "C:\..." paths throws
...
// engine/core/src/languages/configs/* and the
...
/**
 * Determine whether `projectRoot` has any user .understandignore file.
 * When no such file exists, the combined and defaults-only filters are
 * identical, so we can skip the dual-filter accounting entirely.
 */
function hasUserIgnoreFile(projectRoot) {
  return existsSync(join(projectRoot, '.understandignore'));
}
