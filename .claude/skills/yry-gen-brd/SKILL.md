...
| # | Step | Responsibility |
|---|------|----------------|
| ① | **Read context** | Load `YiKnowledge/resources/templates/brd.md` (BRD template), `YiKnowledge/resources/prompts/brd-generation-prompt.md` (generation rules), and `YiVad/src/views/brd/meta-schemas.ts` (field definitions) |
...
| File | Purpose |
|------|---------|
| `YiKnowledge/resources/templates/brd.md` | Bilingual BRD template — 7 sections: Document Info, Background & Objectives, Business Scenarios, Countries/Modules, Rules & Constraints, Timeline, Acceptance Criteria, Attachments, Approvals |
| `YiKnowledge/resources/prompts/brd-generation-prompt.md` | BRD chapter generation prompt — system prompt rules, input variables, output format expectations, temperature/token settings, and YiAi integration notes |
...
```
1. Read brd/domains/{domain}.md     → 获取领域 KPI、痛点、角色、法规、术语
2. Read brd/scenarios/              → 匹配合适的场景模式作为 As-Is/To-Be 参考
3. Read brd/terminology/            → 准确使用领域和行业术语
4. Read brd/reference/countries.md   → 匹配目标市场特征
5. Read brd/reference/regulations.md → 匹配适用法规框架
6. Read brd/examples/               → 参考示例的输出格式和详细程度
7. Read YiKnowledge BRD template    → 确认章节结构完整性
8. Read YiVad meta-schemas.ts       → 确认所有必填字段
```
...
| 2 | If user input lacks information for a required field, write `[待补充 / TBC]` instead of fabricating |
...
```
Read YiKnowledge/resources/templates/brd.md → understand the BRD structure
Read YiKnowledge/resources/prompts/brd-generation-prompt.md → understand generation rules
...
- **Honest about gaps** — use `[待补充 / TBC]` for information not derivable from user input
...
| `YiKnowledge/resources/templates/brd.md` | read (legacy template) |
| `YiKnowledge/resources/prompts/brd-generation-prompt.md` | read (generation rules) |
...
### Code & Templates
- [BRD Template](../../../YiKnowledge/resources/templates/brd.md) — Bilingual BRD document template
- [BRD Generation Prompt](../../../YiKnowledge/resources/prompts/brd-generation-prompt.md) — YiAi BRD agent chapter generation prompt
