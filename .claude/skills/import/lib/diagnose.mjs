...
export async function recommendMode(root, workspaceName, opts, apiUrl) {
  ...
  console.log("# yry-import 状态检测与推荐\n");

  if (!API_X_TOKEN) {
    console.log("⚠️  API_X_TOKEN: 缺失");
    console.log("   → 推荐: 配置 token 后执行 `/yry-import workspace=true` 全量导入\n");
  } else {
  ...
  if (API_X_TOKEN) {
    try {
      ...
      console.log("   → 推荐: 检查网络或 API 地址later, retry\n");
    }
  }

  console.log(`📋 待sync file: ${files.length} 个`);
  if (files.length > 0) {
  ...
  console.log("\n## 推荐任务\n");
  if (!API_X_TOKEN) {
    console.log("1. [凭据缺失] 设置 API_X_TOKEN Environment variables");
  }
  console.log("2. [全量导入] `/yry-import workspace=true` 扫描并上传全部文件");
  console.log("3. [增量同步] `/yry-import workspace=true exclude=...` 跳过指定目录");
  console.log("4. [预览检查] `/yry-import workspace=true mode=list` 仅列出不上传");
