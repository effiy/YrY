...
Given a `failures[]` array from rui-init-verify, identify root causes (not just symptoms), group related failures, and produce a prioritized fix plan. Read-only.
...
|---------|-----------|-----|
| Checks 1+2+4 all fail | generate didn't run or crashed | Re-run rui-init-generate |
| Checks 5+6+7 all fail | arch didn't run or crashed | Re-run rui-init-arch |
| Only check 3 fails | Domain language too thin | Manually add term definitions |
| Only check 4 fails | docs file deleted | Re-run rui-init-generate |
...
```json
{
  "failures_in": 3,
  "root_causes": [
    {"cause": "generate_step_incomplete", "affected_checks": [1, 2, 4], "fix": "Re-run rui-init-generate", "effort": "automated"}
  ],
  "plan": [
    {"step": 1, "action": "Re-run rui-init-generate", "fixes_checks": [1, 2, 4], "command": "/rui-init --from generate"}
  ],
