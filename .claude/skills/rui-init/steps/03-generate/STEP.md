...
description: >
  ...
  home is emitted from the rui-init standalone dashboard template
  (`rui-init/templates/`) — the generator does not redefine the
  dashboard layout, only the data model. Run this skill after
  rui-init-explore and before rui-init-arch.
---
...
> shell, CSS, Vue 3 mount, theme tokens) is the rui-init standalone
> dashboard template at `rui-init/templates/`; this skill only
> produces the data model that the template consumes. It does not
> read source code (that is rui-init-explore's job), and it does not
> produce arch stories (that is rui-init-arch's job).
>
...
> data model. The template files in `rui-init/templates/` are the
...
| `index.html` | `<cwd>/docs/index.html` | Standalone dashboard shell copied from `rui-init/templates/index.html` and rewritten to the project root (`../rui-html-cdn/...` paths) |
| `index.css` | `<cwd>/docs/index.css` | Standalone dashboard styles copied from `rui-init/templates/index.css` |
| `index.js` | `<cwd>/docs/index.js` | Standalone dashboard Vue 3 mount copied from `rui-init/templates/index.js`; registers `<rui-scene-card>` + `<rui-tag-chip>` + `<rui-back-top>`, mounts from `window.HELP_CONFIG`, and wires reading-progress + teardown via `window.__ruiInitTeardown()` |
...
| `theme.css` | `<cwd>/docs/theme.css` | `--rui-*` design tokens copied from `rui-init/templates/theme.css` (Code Dark) |
...
that match the rui-init standalone dashboard template. The
**layout** (HTML shell, CSS, Vue 3 mount, theme tokens) is the
single source of truth at `rui-init/templates/`; this skill
**copies** the layout files into `<cwd>/docs/` (rewriting CDN
paths from `../../rui-html-cdn/...` to `../rui-html-cdn/...`) and
**emits** the data model (`data.js`) that the template consumes.

| File | Generation rule |
|------|-----------------|
| `docs/theme.css` | Copied verbatim from `rui-init/templates/theme.css` (Code Dark `--rui-*` tokens) |
| `docs/index.html` | Copied from `rui-init/templates/index.html`; rewrite CDN paths to `../rui-html-cdn/...`; rewrite page title to `<project profile> · Documentation Center`; rewrite body class to `rui-doc dashboard-page` |
| `docs/index.css` | Copied verbatim from `rui-init/templates/index.css` (dashboard chrome, `--dashboard-*` token bridge, `.items-grid` / `.stories-grid` / `.scenes-grid` wrappers, `.rui-scene-card` print overrides) |
| `docs/index.js` | Copied verbatim from `rui-init/templates/index.js` (Vue 3 mount, `<rui-scene-card>` / `<rui-tag-chip>` / `<rui-back-top>` registration, `sceneCardFor(group)` per-kind dispatch, `window.__ruiInitTeardown()`) |
...
| `stats` | Top-line counts from `profile` + `exploration`: runtime-dep count, dev-dep count, story count, source-file count (rendered by `<rui-stats-grid>`) |
...
- The generated docs home must preserve the dashboard layout
  (5-file shape) from `rui-init/templates/`. Do **not** edit the
...
| `docs/index.html` / `docs/index.css` / `docs/index.js` / `docs/theme.css` | **Copied from `rui-init/templates/`** with the documented path and title rewrites | The template is the single source of truth for the dashboard layout |
...
| `docs/index.html` body class is `rui-doc dashboard-page` | grep | Template layout adopted |
| `docs/index.html` references `../rui-html-cdn/...` CDN scripts | grep | CDN paths rewritten for the project root |
