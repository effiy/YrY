...
description: >
  ...
  emit a module map. Run this skill after rui-init-detect, before any
...
> `exploration` object passed to rui-init-generate and rui-init-arch.
>
> Triggered by the parent pipeline (rui-init), right after
> rui-init-detect.
>
> **Input**: the `profile` object emitted by rui-init-detect.
...
The module map is the canonical input for rui-init-arch — it is what
...
The corrected `securitySurface` is what rui-init-verify sees.
...
| Architecture pattern cannot be confirmed | Set `architecture.pattern = 'unknown'`; let the parent pipeline surface this to rui-init-verify |
...
|--------|--------------|-------------------|
| `exploration.moduleMap` is a non-empty array | `Array.isArray && length > 0` | Pipeline may proceed; empty array triggers rui-init-verify fallback |
