...
(function () {
  ...
  // <yry-scene-card> props. Returns an array of card descriptors; the
  ...
  // template reference <yry-scene-card> directly with no extra wrapper.
    ...
    if (group.kind === 'items') {
      return group.items.map(function (it, i) {
        return {
          ...
          // <span class="accent">) on separate fields. yry-scene-card exposes
          ...
          links: it.demoHref
            ...
            : [],
          // Staggered card entrance delay (yry-scene-card reads --card-delay).
    ...
    if (group.kind === 'stories') {
      return group.items.map(function (st) {
        return {
          ...
          // stories: prepend the emoji icon to the name (yry-scene-card has
          // no separate icon slot; the emoji gives a quick visual cue).
          name: (st.icon ? st.icon + ' ' : '') + st.title,
          badge: st.badge,
          desc: st.description,
          // stories: sceneLinks use {label, href}, but yry-tag-chip wants
          // {text, href, modifier}. Normalize the shape here so yry-scene-card
    ...
    if (group.kind === 'scenes') {
      return group.items.map(function (sc) {
        return {
          ...
          // (renders as the footer link row in yry-scene-card).
  ...
  /* ── Resolve the stats shape for <yry-stats-grid> ───────────────────────
  ...
  window.__vueLoadPromise.then(function () {
    if (typeof Vue === 'undefined') {
      console.error('[yry-init] Vue global not present after load promise — page will not mount');
      return;
    }
    ...
    app = Vue.createApp({
      data: function () {
        return {
          ...
          // <yry-panel-hub>. `reportTabs` is derived once (cadence key +
      ...
      methods: {
        ...
        // Cross-report navigation: <yry-panel-hub> dispatches
        ...
        // shared/components/yry-panel-hub/index.js.
        //
        // The component dispatches a native CustomEvent on its root
        // element, so Vue 3 passes the event object to the
        // @panel-hub-select handler. Unwrap detail.panel here so the
        // rest of the handler can treat `panel` as a plain string. The
        // "Reports" button intentionally omits a URL — it opens the
        // floating reports panel (daily / weekly / monthly tabs) via
        // Vue state instead of navigating away.
        ...
        // Reading-progress only; back-to-top visibility is owned by yry-back-top.
