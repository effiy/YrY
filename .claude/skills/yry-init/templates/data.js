/**
 * yry-init · Dashboard data template (shape reference)
 * --------------------------------------------------------------------------
 * This file is the **shape template** for the dashboard data model.
 * yry-init-generate (step ③ of the yry-init pipeline) reads the
 ...
 *   stats         — high-level KPIs; rendered by <yry-stats-grid> (CDN)
 *   panelHub      — cross-report navigation toolbar; rendered by
 *                   <yry-panel-hub> (CDN). `urls` is keyed by `panel`
 *   sections      — content sections; each contains `groups[]` whose
 *                   `kind` selects the yry-scene-card shape (see below)
 ...
 *
 * Stat modifier vocabulary (consumed by <yry-stats-grid>):
...
window.HELP_CONFIG = {
  titleIcon:     "★",
  title:         "Rui Init",
  tagline:       "Five-step pipeline · detect → explore → generate → arch → verify · full rebuild on every run · /yry-init",
  backTopLabel:  "Back to Top",
  footerNote:    "Refactored from docs/index.html into a yry-html-vue style standalone dashboard template",

  // Top-of-page context bar — delegated to <yry-breadcrumb> (CDN).
  breadcrumb: [
    { label: "Rui Init", href: "../index.html" },
    { label: "Dashboard" }
  ],

  // High-level KPIs — delegated to <yry-stats-grid> (CDN). The CDN
  ...
  // yry-panel-hub · cross-report navigation toolbar. The component
  ...
  // new tab. Resolution order inside <yry-panel-hub>:
  ...
  panelHub: {
    ...
    buttons: [
        {icon: '🏛', name: 'Architecture', desc: 'Markdown scenes', color: 'var(--yry-accent)', panel: 'arch'},
        {icon: '📁', name: 'Files Report', desc: 'Codebase analysis', color: 'var(--yry-accent)', panel: 'files'},
        // Per-button override: this one reverts to same-tab even though
        // panelHub.targetBlank is true.
        {icon: '✅', name: 'test', desc: 'Self-check strategy', color: 'var(--yry-cyan)', panel: 'test', targetBlank: false},
    ],
  ...
  // <yry-panel-hub>. The panel is owned by index.html + index.js; this
  // block is the per-cadence list it reads from. The list shape is
  // mirror-imaged across daily / weekly / monthly, so the same
  // `<yry-scene-card>`-style four-row item renders for every cadence
  ...
  // reports once the yry-reports/daily, yry-reports/weekly and
  // yry-reports/monthly commands start emitting them.
  reportsList: {
    daily: [
      {
        date:    'YYYY-MM-DD',
        ...
        status:  'shipped',
        summary: 'Per-day health check: commits landed, file touches, lint / scene regressions, and any sub-skill upgrade that cleared impact review. Replace this stub with the yry-reports/daily output once it lands.',
        metrics: [
    ...
    weekly: [
      // Populated by the yry-reports/weekly command; empty until the
      // first weekly report lands under `weekly/`.
    ],
    monthly: [
      // Populated by the yry-reports/monthly command; empty until the
