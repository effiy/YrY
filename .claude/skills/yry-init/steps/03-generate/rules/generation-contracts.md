...
`yry-init-generate` produces `CLAUDE.md`, `README.md`, and `docs/` home entry files from `profile` + `exploration`. These are the hard contracts.
...
| Template-driven layout | `docs/` layout files copied from `yry-init/templates/` with path rewrites |
...
1. Never modify `yry-init/templates/` — those are the source of truth.
