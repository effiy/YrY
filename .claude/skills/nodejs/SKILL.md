...
# nodejs — Curated Node.js Best-Practices Navigator
...
| Node concern | Recommended approach |
|--------------|---------------------|
| Vitest setup in a Node project | Configure `vitest.config.ts` with Node environment |
| ESM vs CJS module resolution in tests | Use `server.deps.inline` for CJS deps |
| `vi.mock` for native modules (`fs`, `child_process`) | Use `vi.mock` with mock interop |
| Fake timers for `setTimeout` / `setInterval` | Use `vi.useFakeTimers()` |
