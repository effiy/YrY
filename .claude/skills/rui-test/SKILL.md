...
description: >
  ...
  reactivity / slots / etc — see `rui-html-vue`), CSS / styling
...
> One skill, twelve topics. The twelve `rui-html-test-*` source
...
- Does NOT cover production code architecture (props / emits / reactivity / slots) — defer to `rui-html-vue`.
...
└── evals/
    └── evals.json                          # test prompts (created by rui-tools eval step)
```

The twelve `topics/<slug>/` directories are the canonical content
for the formerly-separate sibling skills
(`rui-html-test-{vitest-setup,runner-choice,e2e-playwright,fixture,async-flush,async-component,suspense,composable-wrapper,pinia-setup,teleport,blackbox,no-snapshot}`);
...
| 12 sibling skills still exist | `ls .claude/skills/rui-html-test-* \| wc -l` → 12 | Source skills preserved (not deleted) |
...
| User wants the previous 12-skill split | The 12 source skills still exist under `rui-html-test-*` and can be installed individually. This skill supersedes them for daily use. |
