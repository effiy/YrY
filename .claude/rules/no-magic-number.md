...
# No Magic Number Rules

> **口诀：数字不说话，语义在说话。** 文档里裸出现的编号（`00`、`01–04`、`05/06`）若被用作判定条件或状态描述，必须先绑定语义；只在「编号区间」「文件清单」里允许保留裸数字。

服务公理：CLAUDE.md「**释义** 说清楚」——人看不懂，正确也没意义。状态用 `00 存在` 表达，读者必须脑补"00 = 索引"才能解码，已是退化信号。
...
要求：编号区间出现的**段落首次提及**必须有完整文件名作为锚点，避免读者首次遇到 `01–04` 时无从对应。
...
- 技术日志 / JSON 字段值：状态机内部值可使用 `complete` / `partial` 等枚举，不在本规则约束内
