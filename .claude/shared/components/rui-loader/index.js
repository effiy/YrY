/* ═══════════════════════════════════════════════════════════════════════════
   rui HTML CDN — rui-loader · bootstrap framework loader (thin bridge)

   This file bridges the template contract {{CDN_ROOT}}/rui-loader/index.js
...
(function () {
  ...
  // From shared/components/rui-loader/index.js → ../../loader.js → shared/loader.js
  var LOADER_PATH = SELF_SRC.replace(/\/components\/rui-loader\/index\.js(\?.*)?$/, '/loader.js');

  if (!LOADER_PATH || LOADER_PATH === SELF_SRC) {
    console.warn('[rui-loader] cannot resolve loader.js path from ' + SELF_SRC);
    return;
  }
  ...
  s.async = false;
  s.onerror = function () {
    console.error('[rui-loader] failed to load loader.js from ' + LOADER_PATH);
  };
