...
1. **Base URL** → `VITE_API_URL` env var; configured in `RequestHttp` constructor
2. **API modules** → `src/api/modules/login.ts`, `user.ts`, `upload.ts` — each exports a function that calls `RequestHttp` methods
