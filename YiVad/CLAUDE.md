...
### 2026-08-24 — Cross-page navigation & pipeline closed-loop

- **`src/components/EntityBreadcrumb/EntityBreadcrumb.vue`** (new): Reusable breadcrumb showing entity hierarchy (Project → Cycle → Release → current). Fetches parent names lazily; renders clickable links with entity-type icons. Registered in `src/components/index.ts`.
- **`src/views/cycle/detail.vue`**: Breadcrumb + linked releases with progress bars (`releaseProgressPct`).
- **`src/views/release/detail.vue`**: Breadcrumb + contributing cycles with progress bars (`releaseCycleProgressPct`).
- **`src/views/issue/detail.vue`**: Breadcrumb (Project → Cycle → Release → Issue).
- **`src/views/bug/detail.vue`**: Breadcrumb (Project → Bug).
- **`src/views/module/detail.vue`**: Breadcrumb (Project → Module).
- **`src/views/project/detail.vue`**: Delivery Pipeline section — 5-stage visual flow (Requirements → Active Cycles → In Progress → Pending Releases → Released), each stage clickable.
- **`src/views/issue/index.vue`**: Card view shows cross-entity link chips (project/cycle/release) below meta row.
- **`src/views/kanban/index.vue`**: Cards show project/release link chips. Added `projectName`, `releaseName`, `goProject`, `goRelease` helpers; `loadNames()`.

### 2026-08-08 — Queued `/followup` messages reflect in the chat transcript (pi Agent.followUp)

- **`src/stores/modules/aiChat.ts`** `sendMessage` `/followup` branch: a successfully queued follow-up is now appended to the conversation as a **`followup`-typed bubble** — the user's deferred intent stays visible in the transcript instead of living only in a transient toast (inconsistent with the auto-steer path, which reflects its message as a bubble). The dedicated type keeps it **out of the request history** (`runStream`'s `aiMessages` filter only passes `user` / non-empty `pet`), so a follow-up the loop already consumed is never re-sent on a later run — which would re-execute a done task. Matches pi's `Agent.followUp` semantics: the message waits until the agent would otherwise stop, then triggers a new turn.
- **`src/views/aiChat/components/MessageBubble.vue`**: `followup` bubbles render on the **user side** (`isUser` includes them) with a distinct "⏱ Follow-up queued — runs when the agent finishes" pill (`isFollowUp`), so a queued instruction reads as deferred, not executed. `Clock` icon added to the import; `.mb-followup-tag` style added.
- **`src/api/interface/yiweb.ts`**: `ChatMessage.type` union grows `"followup"`. Transcript/export labels (`aiChat.ts` markdown + HTML, `MessageBubble.vue` branch transcript) map it to "Follow-up (queued)" instead of the generic "AI".
- Type-check: 0 new errors (18 pre-existing, all in unrelated files). The backend drain the bubble reflects is proven by `/tmp/test_followup_drain.py` (deterministic: external `/agent/follow-up` store is drained at the first no-tool boundary and executed as a new turn — 4 LLM calls, 2 confirmations, 2 `db_create` executions, store empty) and live `/tmp/e2e_followup.py` (qwen3-coder: follow-up queued at `agent_start`, both main and follow-up menus confirmed and created, `stop=completed`; a second run hit the known pre-existing recon-only stall where the model never proposes a write — unrelated to follow-ups).
...
- **`src/stores/modules/aiChat.ts`** `runStream`: resume-by-session is now gated on a **genuine continuation**. When the previous run hit `max_turns`, only `resume: true` for `继续`-class messages (which send just the user's continuation; the server restores the persisted tool trajectory); a **new task sent after max_turns goes as a fresh run** (`resume: false`, full history) — the known-good path. Before, ANY next send resumed, and the backend's `[RESUME]` merge injected a "db_* 请勿重复执行" handoff note meant for 继续, which told the model to skip the writes a new task needed (observed: a new "create" after max_turns never ran its `db_create`). Uses `lastUserMsg` (already in `runStream` scope) as the message to classify — `content` lives in `sendMessage`, not `runStream`. Type-check: 0 new errors (18 pre-existing, all unrelated).
- **Verified live** (`/tmp/e2e_resume.py`, qwen3-coder): run 1 (`max_turns=2`) persisted the trajectory under the session id; run 2 with `messages:[{user:"继续"}], resume:true` on the SAME session restored it and completed the full lifecycle — `db_update` + `db_delete`, **no `db_create` re-run** (the duplicate the fix prevents), `stop=completed`. No leftover probe menus (0 `/e2e-` paths).

### 2026-08-08 — Chat confirmation answers (pi natural-language permission)

- **`src/utils/confirmationAnswer.ts`** (new, pure/dependency-free): `confirmationAnswerFor(text)` classifies a plain chat message as a confirmation answer — `{ action: "approve" | "reject", bare: boolean } | null`. Affirmative keywords (`可以/好/行/同意/批准/确认/yes/ok/…`) → approve; rejective (`不要/不行/取消/拒绝/不/no/stop/…`) → reject; multi-token phrase prefixes (`go ahead`, `好的`, `不要删除`…) → the same action with `bare:false` (carries a correction). Exact keyword = `bare:true` (no correction). Empty/ambiguous/`now`/`not`/task text → `null` (falls through to steering). Unit-tested 51/51 by compiling the module with `tsc` and asserting in node.
- **`src/stores/modules/aiChat.ts`** `sendMessage`: while a confirmation is pending, a plain chat message answers it instead of only steering — **`可以`/`yes` approves the pending write, `不要`/`no` rejects it, no mouse needed**. A reject with extra text (`bare:false`, e.g. "不要删除，改成更新 title") also steers the whole message into the running loop so the model hears the correction, not just the rejection. Previously the user had to click the banner button; typing "可以" merely queued a steer the loop only read after the 120s wait timed out. Slash commands, images, and non-answer text keep their prior behavior (auto-steer).
- **`src/views/aiChat/components/MessageList.vue`**: the confirmation banner now hints the chat affordance — "或直接在输入框回复 — 可以/好/yes 批准，不要/取消/no 拒绝". Type-check: 0 new errors (18 pre-existing, all unrelated).
- **Verified live** (`/tmp/e2e_confirmation_chat.py`, qwen3-coder): the exact frontend sequence — while the first `db_create` confirmation is pending, answer in chat with reject (`POST /agent/confirm approve:false`) then steer a read-only alternative (`POST /agent/steer "不要创建菜单 /x，改为执行一次 db_list"`). Result: `confirm_count=1` (the rejected write was never re-prompted — rejection memory held), `created=False` (never executed), `list_done=True` (the steered alternative actually ran), `stop=completed`. No leftover probe menus (68 docs, 0 `/e2e-` paths).
...
| [src/api/index.ts](./src/api/index.ts) | RequestHttp class — Axios wrapper |
| [src/stores/modules/aiChat.ts](./src/stores/modules/aiChat.ts) | aiChat store — SSE streaming, agent loop, confirmation |
| [YiKnowledge/projects/YiVad/onboarding.md](../YiKnowledge/projects/YiVad/onboarding.md) | YiVad onboarding guide |
