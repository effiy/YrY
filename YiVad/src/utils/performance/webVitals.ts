...
function observeLCP(): void {
  ...
  ["pointerdown", "keydown", "scroll"].forEach(evt => addEventListener(evt, finalize, { once: true, capture: true }));
...
function observeTTFB(): void {
  ...
  if (performance.getEntriesByType("navigation").length > 0) {
    ...
    const po = new PerformanceObserver(list => {
      if (list.getEntriesByType("navigation").length > 0) {
        report();
        po.disconnect();
      }
    });
...
function observeINP(): void {
  ...
  const report = () => {
    if (inpMaxDuration > 0) recordMetric("INP", inpMaxDuration);
  };
  document.addEventListener("visibilitychange", () => {
...
export function getPerformanceScore(): {
  ...
  const weights: Record<string, number> = { LCP: 0.25, INP: 0.25, CLS: 0.2, FCP: 0.15, TTFB: 0.15 };
  let totalScore = 0;
  ...
  for (const [name, weight] of Object.entries(weights)) {
    ...
    if (!metric) continue;
    const [good, poor] = thresholds[name] || [0, Infinity];
    const sub = poor > good ? Math.max(0, Math.min(100, 100 - ((metric.value - good) / (poor - good)) * 100)) : 100;
...
export function getMetricsHistory(): VitalMetric[] {
