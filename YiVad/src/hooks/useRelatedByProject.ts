...
export type RelatedDomain = "tl" | "bug" | "cr" | "story";
...
export const RELATED_TOPICS: TopicConfig[] = [
  { domain: "tl", topic: "architecture", label: "Architecture", cname: "tech_architecture", route: "leaderArchitectureDetail" },
  { domain: "tl", topic: "capacity", label: "Capacity", cname: "tech_capacity", route: "leaderCapacityDetail" },
  { domain: "tl", topic: "decisions", label: "Decisions", cname: "tech_decisions", route: "leaderDecisionsDetail" },
  { domain: "tl", topic: "risk", label: "Risk", cname: "tech_risk", route: "leaderRiskDetail" },
  { domain: "tl", topic: "roadmap", label: "Roadmap", cname: "tech_roadmap", route: "leaderRoadmapDetail" },
...
export const RELATED_DOMAIN_ORDER: RelatedDomain[] = ["tl", "cr", "bug", "story"];
...
function fetchAllForProject(project: string): Promise<(RelatedEntry & { topic: string; domain: RelatedDomain })[]> {
  // Normalize to lowercase for cross-domain matching
...
export function useRelatedByProject(project: () => string | undefined, opts: UseRelatedByProjectOptions = {}): UseRelatedByProject {
  ...
  const perDomainCount = computed(() => {
    const out: Record<RelatedDomain, number> = { tl: 0, cr: 0, bug: 0, story: 0 };
  ...
  async function load() {
    ...
    try {
      ...
      error.value = e instanceof Error ? e.message : "Failed to load related entries";
    } finally {
      loading.value = false;
    }
  }
