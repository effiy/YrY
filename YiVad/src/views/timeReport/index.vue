    ...
    <div v-loading="loading" class="time-report__body">
      ...
      <div v-if="groupedData.length" class="time-report__table-wrap">
        <table>
          ...
          <tbody>
            <tr v-for="item in groupedData" :key="item.key">
              <td class="time-report__title-cell" @click="router.push(`/issue/${item.key}`)">{{ item.title }}</td>
              <td>
                <el-button link size="small" type="primary" @click.stop="goProject(item.projectKey)">{{ item.projectName }}</el-button>
              </td>
              <td>{{ item.assignee || "-" }}</td>
...
const groupedData = computed(() => {
  return issues.value
    ...
    .map(i => ({
      ...
      projectName: projectNames.value.get(i.project_key) || i.project_key,
      assignee: i.assignee,
...
onMounted(async () => {
  ...
  try {
    const projRes = await getProjectList({ pageSize: 500 });
    projectNames.value = new Map(((projRes.data?.list as Project[]) ?? []).map(p => [p.key, p.name]));
  } catch { /* ignore */ }
  loadData();
});
</script>
...
.time-report__title-cell { cursor:pointer;color:var(--el-color-primary);font-weight:500; &:hover { text-decoration:underline; } }
</style>