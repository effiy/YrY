...
const props = withDefaults(defineProps<{ scopeFiles?: string[]; derivedScope?: string; hasScope?: boolean }>(), {
  scopeFiles: () => [],
  derivedScope: "",
  hasScope: false
});
...
const compareDelta = computed(() => {
  const [a, b] = compareRecords.value;
  if (!a || !b) return null;
  const ta = tokenBudget(a.sources),
    tb = tokenBudget(b.sources);
  return {
...
async function copyRecordSources(r: RagQueryRecord) {
  try {
    ...
    setTimeout(() => {
      if (copiedRecSrcId.value === r.id) copiedRecSrcId.value = null;
    }, 1800);
  } catch {
    /* */
  }
...
async function copyQuestion(id: string, text: string) {
  ...
  try {
    ...
    copiedQId.value = id;
    setTimeout(() => {
      if (copiedQId.value === id) copiedQId.value = null;
    }, 1800);
  } catch {
    /* */
  }
...
async function copyChatAnswer(t: RagChatTurnRecord) {
  ...
  try {
    ...
    setTimeout(() => {
      if (chatAnswerCopiedId.value === t.id) chatAnswerCopiedId.value = null;
    }, 1800);
  } catch {
    /* */
  }
...
async function copyChatTurnSources(t: RagChatTurnRecord) {
  try {
    ...
    setTimeout(() => {
      if (copiedChatSrcId.value === t.id) copiedChatSrcId.value = null;
    }, 1800);
  } catch {
    /* */
  }
  ...
  const rows = records.map(r => {
    if (isRetrieval) {
      ...
      return [
        ...
        rr.top_k,
        rr.result_count,
        (rr.top_score * 100).toFixed(1) + "%",
        (rr.avg_score * 100).toFixed(1) + "%",
        rr.latency_ms,
        rr.config?.hybrid ? "yes" : "no",
        rr.config?.rerank ? "yes" : "no",
        rr.config?.citations ? "yes" : "no",
        rr.config?.num_queries ?? 1,
        rr.config?.category ?? "",
        (rr.config?.tags ?? []).join("|")
      ]
        .map(csvField)
    ...
    return [
      ...
      tt.chat_mode,
      tt.source_count,
      (tt.top_score * 100).toFixed(1) + "%",
      (tt.avg_score * 100).toFixed(1) + "%",
      tt.latency_ms
    ]
      .map(csvField)
      .join(",");
  ...
  <div class="rc-body">
    ...
    <div class="rc-hd-row">
      <div class="rc-hist-subtabs">
        <button class="rc-hist-subtab" :class="{ active: historyView === 'retrieval' }" @click="switchHistoryView('retrieval')">
          Retrieval <span v-if="historyRecords.length" class="rc-tab-count">{{ historyRecords.length }}</span>
        </button>
        <button class="rc-hist-subtab" :class="{ active: historyView === 'chat' }" @click="switchHistoryView('chat')">
          Chat <span v-if="chatTurns.length" class="rc-tab-count">{{ chatTurns.length }}</span>
        </button>
      </div>
      <div class="rc-hd-actions">
        <template v-if="historyView === 'retrieval'">
          <el-button
            size="small"
            :icon="Download"
            :disabled="!analytics.filteredHistoryRecords.value.length"
        ...
        <template v-else>
          <el-button
            size="small"
            :icon="Download"
            :disabled="!analytics.filteredChatTurns.value.length"
            @click="exportHistoryJSON"
            >JSON</el-button
          >
          <el-button size="small" :icon="Download" :disabled="!analytics.filteredChatTurns.value.length" @click="exportHistoryCSV"
            >CSV</el-button
          >
    ...
    <template v-if="historyView === 'retrieval'">
      ...
      <div v-if="compareRecords.length" class="rc-cmp">
        <div class="rc-cmp-hd">
          <span class="rc-hist-spark-label">side-by-side ({{ compareRecords.length }}/2)</span
        ...
        <div class="rc-cmp-grid" :class="{ 'is-pair': compareRecords.length === 2 }">
          <div
            v-for="(r, idx) in compareRecords"
            :key="r.id"
            class="rc-cmp-cell"
            :class="{ 'is-winner': compareWinnerSide === (idx === 0 ? 'left' : 'right') }"
          >
            ...
            <div class="rc-cmp-cell-stats">
              <div class="rc-cmp-stat">
                <span class="rc-cmp-k">latency</span><span class="rc-cmp-v">{{ r.latency_ms }}ms</span>
              </div>
              <div class="rc-cmp-stat">
                <span class="rc-cmp-k">top</span
              ...
              <div class="rc-cmp-stat">
                <span class="rc-cmp-k">src</span><span class="rc-cmp-v">{{ r.result_count }}</span>
              </div>
              <div v-if="tokenBudget(r.sources)" class="rc-cmp-stat">
                <span class="rc-cmp-k">tok</span><span class="rc-cmp-v">~{{ tokenBudget(r.sources)!.toLocaleString() }}</span>
              </div>
              <div class="rc-cmp-stat">
                <span class="rc-cmp-k">top-k</span><span class="rc-cmp-v">{{ r.top_k }}</span>
              </div>
            </div>
        ...
        <div v-if="compareDelta" class="rc-cmp-delta">
          ...
          <div class="rc-cmp-delta-row">
            ...
            <span
              ...
              >top {{ compareDelta.top_score_pct > 0 ? "+" : "" }}{{ compareDelta.top_score_pct.toFixed(0) }}%</span
            >
            <span
              class="rc-cmp-delta-chip"
            ...
            <span
              ...
              >src {{ compareDelta.sources > 0 ? "+" : "" }}{{ compareDelta.sources }}</span
            >
          </div>
        </div>
      </div>
      ...
      <div class="rc-hist-filter-row">
        <span v-if="historyRecords.length" class="rc-results-stat"
          >{{ analytics.filteredHistoryRecords.value.length }}/{{ historyRecords.length }} shown · max {{ historyMax }}</span
        >
        <span
          v-if="analytics.meanTopScoreRetrieval.value != null"
          class="rc-results-stat rc-results-score"
          :style="{ color: scoreColor(analytics.meanTopScoreRetrieval.value / 100) }"
          >μ top {{ analytics.meanTopScoreRetrieval.value }}%</span
        >
        <span
          v-if="analytics.meanLatencyRetrieval.value != null"
          class="rc-results-stat"
          :style="{ color: latencyBucket(analytics.meanLatencyRetrieval.value).color }"
          >μ {{ analytics.meanLatencyRetrieval.value }}ms</span
        >
        <span v-if="analytics.staleRateRetrieval.value?.stale" class="rc-results-stat rc-results-stale"
          >⚠ {{ analytics.staleRateRetrieval.value.stale }}/{{ analytics.staleRateRetrieval.value.total }} stale</span
        >
        <el-select v-model="historyDateRange" size="small" class="rc-hist-range-filter">
          <el-option label="24h" value="24h" /><el-option label="7d" value="7d" /><el-option label="30d" value="30d" /><el-option
            label="all"
            value="all"
          />
        </el-select>
        ...
        <el-select v-model="retrievalConfigFilter" size="small" clearable placeholder="All configs" class="rc-hist-mode-filter">
          <el-option label="hybrid" value="hybrid" /><el-option label="rerank" value="rerank" /><el-option
            label="citations"
            value="citations"
      ...
      <div v-if="historyLoading && !historyRecords.length" class="rc-empty">
        <el-icon :size="40"><Clock /></el-icon><span>Loading…</span>
      </div>
      <div v-else-if="!historyRecords.length" class="rc-empty">
        <el-icon :size="40"><Clock /></el-icon><span>No retrieval history yet.</span>
      </div>
      ...
      <div v-else class="rc-hist-list">
        ...
        <div v-if="analytics.gradeBreakdownRetrieval.value" class="rc-hist-grades">
          ...
          <div class="rc-hist-grades-track">
            <div
              v-for="g in analytics.gradeBreakdownRetrieval.value.buckets"
              :key="g.grade"
        ...
        <div v-if="analytics.bestWorstQuestionRetrieval.value" class="rc-hist-bwq">
          <div class="rc-hist-bwq-row">
            <span
              class="rc-hist-bwq-lbl"
              :style="{ color: scoreColor(analytics.bestWorstQuestionRetrieval.value.best.topScore / 100) }"
              >best</span
            ><span
              class="rc-hist-bwq-q"
              @click="reuseHistoryQuestion(analytics.bestWorstQuestionRetrieval.value.best.question)"
              >{{ analytics.bestWorstQuestionRetrieval.value.best.question }}</span
            ><span
              class="rc-hist-bwq-score"
              :style="{ color: scoreColor(analytics.bestWorstQuestionRetrieval.value.best.topScore / 100) }"
              >{{ analytics.bestWorstQuestionRetrieval.value.best.topScore }}%</span
            >
          </div>
          <div class="rc-hist-bwq-row">
            <span
              class="rc-hist-bwq-lbl"
              :style="{ color: scoreColor(analytics.bestWorstQuestionRetrieval.value.worst.topScore / 100) }"
              >worst</span
            ><span
              class="rc-hist-bwq-q"
              @click="reuseHistoryQuestion(analytics.bestWorstQuestionRetrieval.value.worst.question)"
              >{{ analytics.bestWorstQuestionRetrieval.value.worst.question }}</span
            ><span
              class="rc-hist-bwq-score"
              :style="{ color: scoreColor(analytics.bestWorstQuestionRetrieval.value.worst.topScore / 100) }"
              >{{ analytics.bestWorstQuestionRetrieval.value.worst.topScore }}%</span
            >
          </div>
        </div>
        ...
        <div v-if="analytics.configCost.value" class="rc-hist-cost">
          ...
          <div class="rc-hist-cost-row">
            <span
              v-for="e in analytics.configCost.value.entries"
              :key="e.key"
            ...
            >
              ...
              <span
                ...
                :style="{ color: e.delta > 0 ? 'var(--el-color-danger)' : 'var(--el-color-success)' }"
        ...
        <div v-if="analytics.latencySparkRetrieval.value || analytics.scatterRetrieval.value" class="rc-hist-viz">
          <div v-if="analytics.latencySparkRetrieval.value" class="rc-hist-spark">
            <svg
              :viewBox="`0 0 ${analytics.latencySparkRetrieval.value.W} ${analytics.latencySparkRetrieval.value.H}`"
              preserveAspectRatio="none"
              class="rc-hist-spark-svg"
              aria-hidden="true"
            >
              <polyline
                :points="analytics.latencySparkRetrieval.value.pts"
                fill="none"
          ...
          <div v-if="analytics.scoreSparkRetrieval.value" class="rc-hist-spark">
            <svg
              :viewBox="`0 0 ${analytics.scoreSparkRetrieval.value.W} ${analytics.scoreSparkRetrieval.value.H}`"
              preserveAspectRatio="none"
              class="rc-hist-spark-svg"
              aria-hidden="true"
            >
              <polyline
                :points="analytics.scoreSparkRetrieval.value.pts"
                fill="none"
          ...
          <div v-if="analytics.scatterRetrieval.value" class="rc-hist-scatter">
            ...
            <svg
              :viewBox="`0 0 ${analytics.scatterRetrieval.value.W} ${analytics.scatterRetrieval.value.H}`"
              preserveAspectRatio="xMidYMid meet"
              class="rc-hist-scatter-svg"
              aria-hidden="true"
            >
              <line
                :x1="analytics.scatterRetrieval.value.pad"
                :y1="analytics.scatterRetrieval.value.H - analytics.scatterRetrieval.value.pad"
                :x2="analytics.scatterRetrieval.value.W - analytics.scatterRetrieval.value.pad"
                :y2="analytics.scatterRetrieval.value.H - analytics.scatterRetrieval.value.pad"
                stroke="var(--el-border-color)"
                stroke-width="0.5"
              />
              <line
                :x1="analytics.scatterRetrieval.value.pad"
                :y1="analytics.scatterRetrieval.value.pad"
                :x2="analytics.scatterRetrieval.value.pad"
                :y2="analytics.scatterRetrieval.value.H - analytics.scatterRetrieval.value.pad"
                stroke="var(--el-border-color)"
                stroke-width="0.5"
              />
              <circle
                v-for="d in analytics.scatterRetrieval.value.dots"
                :key="d.id"
                :cx="d.cx"
                :cy="d.cy"
                :r="d.r"
                :class="[
                  'rc-hist-scatter-dot',
                  { 'is-hybrid': d.hybrid, 'is-rerank': d.rerank, 'is-active': selectedHistoryId === d.id }
                ]"
                @click="selectHistoryRecord(d.id)"
              >
                <title>
                  {{ d.lat }}ms · {{ Math.round(d.score * 100) }}%{{ d.hybrid ? " · hybrid" : ""
                  }}{{ d.rerank ? " · rerank" : "" }}
                </title>
              </circle>
            </svg>
        ...
        <!-- Record cards -->
        <div
          v-for="r in analytics.filteredHistoryRecords.value"
          :key="r.id"
          :id="`rc-hist-rec-${r.id}`"
          class="rc-hist-item"
        ...
        >
          <div class="rc-hist-hd">
            ...
            <span class="rc-hist-stat rc-hist-stat--latency" :style="{ color: latencyBucket(r.latency_ms).color }"
            ...
            <span class="rc-hist-stat" :style="{ color: scoreColor(r.top_score) }"
              >top {{ (r.top_score * 100).toFixed(0) }}%</span
            >
          ...
          <div v-if="expandedSourcesRId === r.id" class="rc-hist-recsrc">
            <div class="rc-hist-recsrc-toolbar">
              <span class="rc-hist-recsrc-summary"
                >{{ r.sources.length }} chunks · Σ
                {{ r.sources.reduce((a, s) => a + (s.metadata?.token_estimate ?? 0), 0).toLocaleString() }} tok</span
              >
              <el-tooltip :content="copiedRecSrcId === r.id ? 'Copied!' : 'Copy chunks as JSON'" placement="top"
            ...
            <div v-for="(s, i) in r.sources" :key="i" class="rc-hist-recsrc-item">
              <div class="rc-hist-recsrc-hd">
                ...
                <span class="rc-hist-recsrc-score" :style="{ color: scoreColor(s.score) }"
                  >{{ (s.score * 100).toFixed(0) }}%</span
                >
                <span class="rc-hist-recsrc-path" :title="s.file_path" @click="emit('open-file', s.file_path)"
                  ><el-icon :size="12"><Document /></el-icon>{{ s.file_path }}</span
                >
              </div>
    ...
    <template v-else>
      <!-- Chat filter row -->
      <div class="rc-hist-filter-row">
        <span v-if="chatTurns.length" class="rc-results-stat"
          >{{ analytics.filteredChatTurns.value.length }}/{{ chatTurns.length }} shown · max {{ chatTurnsMax }}</span
        >
        <span
          v-if="analytics.meanTopScoreChat.value != null"
          class="rc-results-stat rc-results-score"
          :style="{ color: scoreColor(analytics.meanTopScoreChat.value / 100) }"
          >μ top {{ analytics.meanTopScoreChat.value }}%</span
        >
        <span
          v-if="analytics.meanLatencyChat.value != null"
          class="rc-results-stat"
          :style="{ color: latencyBucket(analytics.meanLatencyChat.value).color }"
          >μ {{ analytics.meanLatencyChat.value }}ms</span
        >
        <span v-if="analytics.staleRateChat.value?.stale" class="rc-results-stat rc-results-stale"
          >⚠ {{ analytics.staleRateChat.value.stale }}/{{ analytics.staleRateChat.value.total }} stale</span
        >
        <el-input
          v-model="historyFilterText"
          size="small"
      ...
      <div v-else-if="!chatTurns.length" class="rc-empty">
        <el-icon :size="40"><Clock /></el-icon><span>No chat history yet.</span>
      </div>
      ...
      <template v-else>
        ...
        <div v-if="analytics.latencySparkChat.value || analytics.scatterChat.value" class="rc-hist-viz">
          <div v-if="analytics.latencySparkChat.value" class="rc-hist-spark">
            <svg
              :viewBox="`0 0 ${analytics.latencySparkChat.value.W} ${analytics.latencySparkChat.value.H}`"
              preserveAspectRatio="none"
              class="rc-hist-spark-svg"
              aria-hidden="true"
            >
              <polyline
                :points="analytics.latencySparkChat.value.pts"
                fill="none"
                stroke="var(--el-color-primary)"
                stroke-width="1.5"
                stroke-linejoin="round"
          ...
          <div v-if="analytics.scatterChat.value" class="rc-hist-scatter">
            <div class="rc-hist-scatter-title">
              <span class="rc-hist-spark-label">latency × score</span>
              <span class="rc-hist-scatter-legend">
                <span class="rc-hist-scatter-legend-item"
                  ><i class="dot" :style="{ background: 'var(--el-color-primary)' }" /> condense+ctx</span
                >
                <span class="rc-hist-scatter-legend-item"
                  ><i class="dot" :style="{ background: 'var(--el-color-success)' }" /> condense_q</span
                >
                <span class="rc-hist-scatter-legend-item"
                  ><i class="dot" :style="{ background: 'var(--el-color-warning)' }" /> context</span
                >
                <span class="rc-hist-scatter-legend-item"
                  ><i class="dot" :style="{ background: 'var(--el-text-color-secondary)' }" /> simple</span
                >
              </span>
            </div>
            <svg
              :viewBox="`0 0 ${analytics.scatterChat.value.W} ${analytics.scatterChat.value.H}`"
              preserveAspectRatio="xMidYMid meet"
              class="rc-hist-scatter-svg"
              aria-hidden="true"
            >
              <line
                :x1="analytics.scatterChat.value.pad"
                :y1="analytics.scatterChat.value.H - analytics.scatterChat.value.pad"
                :x2="analytics.scatterChat.value.W - analytics.scatterChat.value.pad"
                :y2="analytics.scatterChat.value.H - analytics.scatterChat.value.pad"
                stroke="var(--el-border-color)"
                stroke-width="0.5"
              />
              <line
                :x1="analytics.scatterChat.value.pad"
                :y1="analytics.scatterChat.value.pad"
                :x2="analytics.scatterChat.value.pad"
                :y2="analytics.scatterChat.value.H - analytics.scatterChat.value.pad"
                stroke="var(--el-border-color)"
                stroke-width="0.5"
              />
              <circle
                v-for="d in analytics.scatterChat.value.dots"
                :key="d.id"
                :cx="d.cx"
                :cy="d.cy"
                :r="d.r"
        ...
        <div class="rc-hist-list">
          <div
            v-for="t in analytics.filteredChatTurns.value"
            :key="t.id"
          ...
          >
            <div class="rc-hist-hd">
              ...
              <span class="rc-hist-stat rc-hist-stat--latency" :style="{ color: latencyBucket(t.latency_ms).color }"
                >{{ latencyBucket(t.latency_ms).label }} · {{ t.latency_ms }}ms</span
              >
              <span class="rc-hist-stat">{{ t.source_count }} src</span>
              <span class="rc-hist-stat" :style="{ color: scoreColor(t.top_score) }"
                >top {{ (t.top_score * 100).toFixed(0) }}%</span
              >
              ...
              <el-tooltip :content="chatAnswerCopiedId === t.id ? 'Copied!' : 'Copy answer'" placement="top"
            ...
            <div v-else-if="!chatCompactMode" class="rc-hist-chat-answer" :title="t.answer">
              {{ t.answer.length > 280 ? t.answer.slice(0, 280) + "…" : t.answer }}
            </div>
