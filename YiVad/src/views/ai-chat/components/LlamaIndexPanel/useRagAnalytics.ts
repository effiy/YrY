...
import { sparklineData, pearsonCorr, gradeBreakdown, tokenBudget, metaFreshness } from "./ragFormat";
...
export function useRagAnalytics(
  ...
  const meanSourceCountRetrieval = computed(() =>
    meanOf(filteredHistoryRecords.value, r => r.result_count ?? r.sources?.length ?? 0)
  );
  ...
  const tokenLatencyCorrRetrieval = computed(() =>
    corrPairs(filteredHistoryRecords.value, r => [tokenBudget(r.sources), r.latency_ms ?? 0])
  );
  const scoreLatencyCorrRetrieval = computed(() =>
    corrPairs(filteredHistoryRecords.value, r => [r.top_score ?? 0, r.latency_ms ?? 0])
  );
  const qlenScoreCorrRetrieval = computed(() =>
    corrPairs(filteredHistoryRecords.value, r => [r.question?.length ?? 0, r.top_score ?? 0])
  );
  const tokenLatencyCorrChat = computed(() =>
    corrPairs(filteredChatTurns.value, t => [tokenBudget(t.sources), t.latency_ms ?? 0])
  );
  ...
  // ── Top source files ──
  const topSourceFilesRetrieval = computed(() =>
    topFiles(
      filteredHistoryRecords.value.map(r => r.sources),
      r => r.id
    )
  );
  const topSourceFilesChat = computed(() =>
    topFiles(
      filteredChatTurns.value.map(t => t.sources),
      t => t.id
    )
  );

  // ── Top retrieved tags ──
  const topTagsRetrieval = computed(() => topTags(filteredHistoryRecords.value.flatMap(r => r.sources)));
  const topTagsChat = computed(() => topTags(filteredChatTurns.value.flatMap(t => t.sources)));
  ...
  const bestWorstQuestionRetrieval = computed(() => {
    ...
    let best: RagQueryRecord | null = null,
      worst: RagQueryRecord | null = null;
    for (const r of recs) {
  ...
  const bestWorstQuestionChat = computed(() => {
    ...
    let best: RagChatTurnRecord | null = null,
      worst: RagChatTurnRecord | null = null;
  ...
  const configCost = computed(() => {
    ...
    const baselineP50 = groups["--"]
      ? pct(
          [...groups["--"].latencies].sort((a, b) => a - b),
          50
        )
      : null;
    const order = ["--", "H-", "-R", "HR"];
  ...
  const chatModeCost = computed(() => {
    ...
    const baselineP50 = groups["simple"]
      ? pct(
          [...groups["simple"].latencies].sort((a, b) => a - b),
          50
        )
      : null;
  ...
  const topStaleFiles = computed(() => {
    ...
    const entries = Array.from(counts.entries())
      ...
      .sort((a, b) => b.count * b.ageDays - a.count * a.ageDays)
      .slice(0, 5);
    return entries.length ? entries : null;
  });
  ...
  function computeCoverageGap(scopeFiles: string[]) {
    ...
    for (const p of scopeFiles) {
      ...
      if (!hit)
        for (const rp of retrieved)
          if (rp.endsWith(name) || rp.endsWith(p)) {
            hit = true;
            break;
          }
...
function buildRetrievalScatter(records: RagQueryRecord[]) {
  ...
  const latencies = pts.map(p => p.lat),
    scores = pts.map(p => p.score);
  const maxLat = Math.max(...latencies, 1),
    minLat = Math.min(...latencies);
