...
export function useSkillsMcp() {
  ...
  // Stub state for inline MCP tool execution and pin stats.
  // Updated by refreshMcpPinStats() and inline runner code paths.
  const args = ref<Record<string, unknown>>({});
  const schema = ref<Record<string, unknown> | undefined>(undefined);
  const required = ref<Set<string>>(new Set());
  const failed = ref(0);
  ...
  function persistPinSortMode(): void {
    try {
      localStorage.setItem(PIN_SORT_MODE_LS_KEY, pinSortMode.value);
    } catch {
      /* ignore */
    }
  }
  ...
  function persistPinned(): void {
    try {
      localStorage.setItem(PINNED_LS_KEY, JSON.stringify([...pinnedTools.value]));
    } catch {
      /* ignore */
    }
  }
  function isPinned(name: string): boolean {
    return pinnedTools.value.has(name);
  }
  ...
  function onGlobalSearchKeydown(e: Event): void {
    ...
    if (ke.key === "ArrowDown") {
      ...
      e.preventDefault();
      selectedToolIdx.value = selectedToolIdx.value <= 0 ? flatFilteredTools.value.length - 1 : selectedToolIdx.value - 1;
  ...
  function getToolProps(tool: McpTool): ToolProp[] {
    const schema = (tool.input_schema ?? tool.inputSchema) as
      { properties?: Record<string, any>; required?: string[] } | undefined;
    ...
    for (const e of events) {
      ...
      if (!prev) {
        ...
        if (typeof e.durationMs === "number") {
          prev.avgMs =
            prev.avgMs != null ? Math.round((prev.avgMs * (prev.count - 1) + e.durationMs) / prev.count) : e.durationMs;
