...
async function handleSaveFileToKB(node: ContextNode) {
  ...
  try {
    ...
    ElMessage.error(e instanceof Error ? e.message : "Failed to save to knowledge base");
  } finally {
...
async function ensureContents() {
  ...
  const results = await Promise.allSettled(files.map(f => readKnowledgeFile(f.path).catch(() => null)));
  results.forEach((r, i) => {
...
async function onSave() {
  ...
  try {
    ...
    if (mode.value === "new") {
      ...
      const tags = contextRoots.value.length ? [...buildCtxTags(contextRoots.value), ...collectAllTags(contextRoots.value)] : [];
      ...
      const tags = [...buildCtxTags(contextRoots.value), ...collectAllTags(contextRoots.value)];
  ...
  <div class="cfp-panel">
    ...
    <div class="cfp-body">
      ...
      <div class="cfp-field">
        ...
        <el-input v-model="sessionTitle" placeholder="Session title" size="small" clearable />
      </div>

      <!-- Search -->
      <div v-if="displayContexts.length > 0" class="cfp-field">
        <el-input v-model="contextSearch" placeholder="Filter files..." size="small" clearable :prefix-icon="Search" />
      </div>
      ...
      <div v-if="visibleContexts.length" class="cfp-list">
        ...
        >
          ...
          />
          <el-button size="small" text type="danger" :icon="Delete" title="Remove" @click="removeContextNode(item.node.key)" />
        </div>
      </div>
      <div v-else class="cfp-empty">
        {{
          mode === "new"
            ? "No knowledge files yet. Drag files from the left panel."
            : "No context files. Drag files from the knowledge panel on the left."
        }}
      </div>
    </div>
  </div>
