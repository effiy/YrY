...
const filtered = computed(() => {
  ...
  return contextFiles.value.filter(f => f.name.toLowerCase().includes(q) || f.path.toLowerCase().includes(q));
});
...
<template>
  <div v-if="visible" class="fmd-dropdown">
    ...
    <div v-else-if="!filtered.length" class="fmd-empty">No matching context files</div>
    <div
