  ...
  <div class="export-wizard">
    ...
    <el-card v-if="step === 1" shadow="never">
      ...
      <el-checkbox-group v-model="form.fields" class="field-grid">
        <el-checkbox v-for="f in currentFields" :key="f" :label="f" border />
      </el-checkbox-group>
      <div class="step-actions">
    ...
    <el-card v-if="step === 2" shadow="never">
      ...
      <div class="export-summary mt16">
        <p>
          Collection: <strong>{{ form.cname }}</strong>
        </p>
        <p>
          Fields: <strong>{{ form.fields.length }}</strong> selected
        </p>
        <p>
          Format: <strong>{{ form.format.toUpperCase() }}</strong>
        </p>
        <p>
          Max rows: <strong>{{ form.limit.toLocaleString() }}</strong>
        </p>
      </div>
...
<script setup lang="ts" name="exportWizard">
...
const form = ref({ cname: "", fields: [] as string[], format: "csv", limit: 10000 });
  ...
  try {
    ...
    if (filterJson.value.trim()) {
      try {
        ...
        ElMessage.warning("Invalid filter JSON");
        return;
      }
    }
