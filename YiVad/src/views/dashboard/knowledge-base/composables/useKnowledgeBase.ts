...
export function useKnowledgeBase() {
  ...
  const worstCategories = computed(() => qualityByCategory.value.filter(c => c.score < 80).slice(0, 5));
  ...
  const filteredModuleDrillData = computed(() => {
    ...
    return moduleDrillData.value.filter(m => m.name.toLowerCase().includes(q) || m.category.toLowerCase().includes(q));
  });
  ...
  const drillTableData = computed(() => {
    ...
    if (drillView.value === "stale") {
      return filteredFiles.value.filter(f => isStaleFile(f)).sort((a, b) => a.updated.localeCompare(b.updated));
    }
    return filteredFiles.value;
  });
  ...
  const staleFiles = computed(() => {
    return (knowledgeData.value?.files ?? []).filter(f => isStaleFile(f)).sort((a, b) => a.updated.localeCompare(b.updated));
  });
  ...
  const sameModuleCount = computed(() => {
    ...
    return (knowledgeData.value?.files ?? []).filter(
      f => f.category === selectedFile.value!.category && f.module === selectedFile.value!.module
    ).length;
  });
  ...
  const categoryComparisonData = computed(() => {
    ...
    return cats
      .map(cat => {
        ...
        const avgCoverage =
          totalFiles > 0 ? Math.round(catModules.reduce((s, m) => s + m.review_coverage_pct * m.count, 0) / totalFiles) : 0;
  ...
  const statDeltas = computed(() => {
    ...
    const allCov = allFiles.length > 0 ? Math.round((allFiles.filter(f => f.review_cycle).length / allFiles.length) * 100) : 0;
    const filtCov = filtered.length > 0 ? Math.round((filtered.filter(f => f.review_cycle).length / filtered.length) * 100) : 0;
    return {
  ...
  const reviewCycleDonutOption = computed<ECOption>(() => {
    ...
    const data = ctx ? countByField(ctx, "review_cycle").filter(d => d.name) : (knowledgeData.value?.review_cycles ?? []);
    const missing = ctx ? ctx.filter(f => !f.review_cycle).length : (knowledgeData.value?.health.no_review_cycle_count ?? 0);
  ...
  function addRecentlyViewed(row: KnowledgeFileSummary) {
    recentlyViewed.value = [row, ...recentlyViewed.value.filter(f => f.path !== row.path)].slice(0, MAX_RECENTLY_VIEWED);
