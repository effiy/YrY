 ...
 */
import type { KnowledgeFileSummary, KnowledgeModuleStats } from "@/api/interface/yiAi";
...
export function buildReviewCycleDonut(data: { name: string; count: number }[], missingCount: number): ECOption {
  const items = [
  ...
  return {
    tooltip: {
      trigger: "item",
      formatter: (p: any) => `${p.name === "__missing__" ? "No review" : p.name}: ${p.value} files (${p.percent}%)`
    },
    legend: {
      orient: "vertical",
      left: 0,
    ...
    series: [
      {
        ...
        barWidth: "65%",
        data: top
          .map(d => ({ value: d.count, itemStyle: { color: TYPE_COLORS[d.name] || "#5470c6", borderRadius: [0, 4, 4, 0] } }))
          .reverse()
      }
    ]
  };
    ...
    series: [
      {
        ...
        barWidth: "65%",
        data: filtered
          .map(d => ({ value: d.count, itemStyle: { color: STATUS_COLORS[d.name] || "#5470c6", borderRadius: [0, 4, 4, 0] } }))
          .reverse()
      }
    ]
  };
    ...
    series: [
      {
        ...
        barWidth: "65%",
        data: counts
          .map((d, i) => ({ value: d.count, itemStyle: { color: bucketColors[i], borderRadius: [0, 4, 4, 0] } }))
          .reverse()
      }
    ]
  };
    ...
    series: [
      {
        ...
        barWidth: "65%",
        data: buckets
          .map((b, i) => ({ value: counts[i], itemStyle: { color: AGE_COLORS[i], borderRadius: [0, 4, 4, 0] } }))
          .reverse()
      }
    ]
  };
    ...
    series: [
      {
        ...
        barWidth: "65%",
        data: filtered
          .map(d => ({ value: d.count, itemStyle: { color: LIFECYCLE_COLORS[d.name] || "#5470c6", borderRadius: [0, 4, 4, 0] } }))
          .reverse()
      }
    ]
  };
...
export function buildModuleBar(modules: KnowledgeModuleStats[], activeCategory: string | undefined, colors: string[]): ECOption {
    ...
    series: [
      {
        ...
        barWidth: "65%",
        data: top
          .map((d, i) => ({ value: d.count, itemStyle: { color: colors[i % colors.length], borderRadius: [0, 4, 4, 0] } }))
          .reverse()
      }
    ]
  };
    ...
    series: [
      {
        ...
        barWidth: "65%",
        data: top
          .map(d => ({ value: d.count, itemStyle: { color: CATEGORY_COLORS[d.name] || "#909399", borderRadius: [0, 4, 4, 0] } }))
          .reverse()
      }
    ]
  };
