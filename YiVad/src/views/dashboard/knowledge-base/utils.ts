...
export function getModuleClassSummary(files: KnowledgeFileSummary[]): ClassSummary {
  const countBy = (key: string) => {
    ...
    for (const f of files) {
      ...
      if (Array.isArray(v)) {
        for (const item of v) m.set(item, (m.get(item) || 0) + 1);
...
export function metaDisplayValue(
  val: string | undefined | null,
  field: string
): { label: string; isMissing: boolean; isUnknown: boolean } {
...
export function moduleHealthScore(m: { review_coverage_pct: number; stale_count: number; count: number }): number {
