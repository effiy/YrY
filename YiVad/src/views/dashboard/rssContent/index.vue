    ...
    <el-drawer v-model="detailVisible" :title="detail?.title ?? ''" size="56%" destroy-on-close>
      <template v-if="detail">
        ...
        <div class="detail-body" v-loading="detailLoading">
          ...
          <el-empty v-else description="No content — open the original article." />
        </div>
      </template>
    </el-drawer>
  </div>
...
import { Reading, Connection, Collection, Calendar, Refresh, Search, TopRight, CopyDocument, Setting, FolderOpened } from "@element-plus/icons-vue";
