...
import { PROJECT_DETAIL_KEY, useProjectDetail } from "@/views/project/types";
...
async function submitAddMember() {
  ...
  try {
    ...
    const updated = [...(project.value.members || []), member];
    await store.editProject(project.value.key, { members: updated });
    ElMessage.success(t("project.members.addSuccess", { username }));
...
async function handleRemove(m: ProjectMember) {
  ...
  const updated = (project.value.members || []).filter(x => x.user_id !== m.user_id);
  await store.editProject(project.value.key, { members: updated });
  ElMessage.success(t("project.members.removeSuccess", { username: m.username }));
}
</script>
