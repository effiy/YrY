...
/** Horizontal bar showing test count per layer, ordered bottom-up. */
export function buildTestLayerBar(rows: TestLayerRow[]): ECOption {
  const ordered = [...rows].reverse();
  return {
    ...
    tooltip: {
      trigger: "item",
      confine: true,
      formatter: (p: any) => `${p.name}: <b>${p.data.tests} tests</b> · ${p.data.passRate}% pass · ${p.data.dur}ms`
    },
    xAxis: { type: "value", show: false },
    yAxis: {
      type: "category",
    ...
    series: [
      {
        type: "bar",
        barWidth: "56%",
        label: { show: true, position: "right", fontSize: 10, color: AXIS_COLOR, formatter: "{c}" },
        itemStyle: { borderRadius: [0, 4, 4, 0] },
        data: ordered.map(r => ({
          value: r.tests,
          name: r.name,
...
/** Donut chart: passed vs failed tests. */
export function buildTestPassDonut(passed: number, failed: number): ECOption {
  return {
    tooltip: { trigger: "item", confine: true, formatter: "{b}: <b>{c}</b> ({d}%)" },
    legend: {
      bottom: 0,
    ...
    series: [
      {
        ...
        itemStyle: { borderWidth: 2, borderColor: "transparent" },
        data: [
          { value: passed, name: "Passed", itemStyle: { color: "#67c23a" } },
          { value: failed, name: "Failed", itemStyle: { color: "#f56c6c" } }
        ]
      }
    ]
  };
