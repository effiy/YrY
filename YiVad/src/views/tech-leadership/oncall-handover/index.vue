<script setup lang="ts" name="tlrOncallHandoverList">
import { ref } from "vue";
...
const schema = tlrMetaSchemas["oncall-handover"];
...
<template>
  <TopicListPage tree="tech-leadership" topic="oncall-handover" label="Oncall Handover" :meta-columns="schema.metaColumns" @meta-click="onMetaClick" />
  <el-drawer v-model="drawerOpen" :title="$t('brdRelated.title')" direction="rtl" size="520px" :destroy-on-close="true">
    <RelatedByProjectPanel v-if="drawerProject" :project="drawerProject" current-tree="tl" current-topic="oncall-handover" :current-key="drawerCurrentKey" />
  </el-drawer>
</template>
