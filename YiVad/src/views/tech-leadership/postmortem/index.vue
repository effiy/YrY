<script setup lang="ts" name="tlrPostmortemList">
import { ref } from "vue";
...
const schema = tlrMetaSchemas["postmortem"];
...
<template>
  <TopicListPage tree="tech-leadership" topic="postmortem" label="Postmortem" :meta-columns="schema.metaColumns" @meta-click="onMetaClick" />
  <el-drawer v-model="drawerOpen" :title="$t('brdRelated.title')" direction="rtl" size="520px" :destroy-on-close="true">
    <RelatedByProjectPanel v-if="drawerProject" :project="drawerProject" current-tree="tl" current-topic="postmortem" :current-key="drawerCurrentKey" />
  </el-drawer>
</template>
