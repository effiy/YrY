<script setup lang="ts" name="tlrProjectHandoffsList">
import { ref } from "vue";
...
const schema = tlrMetaSchemas["project-handoffs"];
...
<template>
  <TopicListPage tree="tech-leadership" topic="project-handoffs" label="Project Handoffs" :meta-columns="schema.metaColumns" @meta-click="onMetaClick" />
  <el-drawer v-model="drawerOpen" :title="$t('brdRelated.title')" direction="rtl" size="520px" :destroy-on-close="true">
    <RelatedByProjectPanel v-if="drawerProject" :project="drawerProject" current-tree="tl" current-topic="project-handoffs" :current-key="drawerCurrentKey" />
  </el-drawer>
</template>
