<script setup lang="ts" name="tlrCapacityCostList">
import { ref } from "vue";
...
const schema = tlrMetaSchemas["capacity-cost"];
...
<template>
  <TopicListPage tree="tech-leadership" topic="capacity-cost" label="Capacity & Cost (FinOps)" :meta-columns="schema.metaColumns" @meta-click="onMetaClick" />
  <el-drawer v-model="drawerOpen" :title="$t('brdRelated.title')" direction="rtl" size="520px" :destroy-on-close="true">
    <RelatedByProjectPanel v-if="drawerProject" :project="drawerProject" current-tree="tl" current-topic="capacity-cost" :current-key="drawerCurrentKey" />
  </el-drawer>
</template>