<script setup lang="ts" name="tlrRiskRegisterList">
import { ref } from "vue";
...
const schema = tlrMetaSchemas["risk-register"];
...
<template>
  <TopicListPage tree="tech-leadership" topic="risk-register" label="Risk Register" :meta-columns="schema.metaColumns" @meta-click="onMetaClick" />
  <el-drawer v-model="drawerOpen" :title="$t('brdRelated.title')" direction="rtl" size="520px" :destroy-on-close="true">
    <RelatedByProjectPanel v-if="drawerProject" :project="drawerProject" current-tree="tl" current-topic="risk-register" :current-key="drawerCurrentKey" />
  </el-drawer>
</template>
