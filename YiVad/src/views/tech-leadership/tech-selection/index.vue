...
const schema = tlrMetaSchemas["tech-selection"];
...
<template>
  <TopicListPage tree="tech-leadership" topic="tech-selection" label="Tech Selection" :meta-columns="schema.metaColumns" @meta-click="onMetaClick" />
  <el-drawer v-model="drawerOpen" :title="$t('brdRelated.title')" direction="rtl" size="520px" :destroy-on-close="true">
    <RelatedByProjectPanel v-if="drawerProject" :project="drawerProject" current-tree="tl" current-topic="tech-selection" :current-key="drawerCurrentKey" />
  </el-drawer>
</template>
