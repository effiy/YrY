<script setup lang="ts" name="tlrDoraMetricsList">
import { ref } from "vue";
...
const schema = tlrMetaSchemas["dora-metrics"];
...
<template>
  <TopicListPage tree="tech-leadership" topic="dora-metrics" label="DORA Metrics" :meta-columns="schema.metaColumns" @meta-click="onMetaClick" />
  <el-drawer v-model="drawerOpen" :title="$t('brdRelated.title')" direction="rtl" size="520px" :destroy-on-close="true">
    <RelatedByProjectPanel v-if="drawerProject" :project="drawerProject" current-tree="tl" current-topic="dora-metrics" :current-key="drawerCurrentKey" />
  </el-drawer>
</template>