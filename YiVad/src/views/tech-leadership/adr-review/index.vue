<script setup lang="ts" name="tlrAdrReviewList">
import { ref } from "vue";
...
const schema = tlrMetaSchemas["adr-review"];
...
<template>
  <TopicListPage tree="tech-leadership" topic="adr-review" label="Architecture Decision Records" :meta-columns="schema.metaColumns" @meta-click="onMetaClick" />
  <el-drawer v-model="drawerOpen" :title="$t('brdRelated.title')" direction="rtl" size="520px" :destroy-on-close="true">
    <RelatedByProjectPanel v-if="drawerProject" :project="drawerProject" current-tree="tl" current-topic="adr-review" :current-key="drawerCurrentKey" />
  </el-drawer>
</template>
