<script setup lang="ts" name="tlrMentorshipGrowthList">
import { ref } from "vue";
...
const schema = tlrMetaSchemas["mentorship-growth"];
...
<template>
  <TopicListPage tree="tech-leadership" topic="mentorship-growth" label="Mentorship & Growth" :meta-columns="schema.metaColumns" @meta-click="onMetaClick" />
  <el-drawer v-model="drawerOpen" :title="$t('brdRelated.title')" direction="rtl" size="520px" :destroy-on-close="true">
    <RelatedByProjectPanel v-if="drawerProject" :project="drawerProject" current-tree="tl" current-topic="mentorship-growth" :current-key="drawerCurrentKey" />
  </el-drawer>
</template>
