<script setup lang="ts" name="tlrProjectBootstrapList">
import { ref } from "vue";
...
const schema = tlrMetaSchemas["project-bootstrap"];
...
<template>
  <TopicListPage tree="tech-leadership" topic="project-bootstrap" label="Project Bootstrap" :meta-columns="schema.metaColumns" @meta-click="onMetaClick" />
  <el-drawer v-model="drawerOpen" :title="$t('brdRelated.title')" direction="rtl" size="520px" :destroy-on-close="true">
    <RelatedByProjectPanel v-if="drawerProject" :project="drawerProject" current-tree="tl" current-topic="project-bootstrap" :current-key="drawerCurrentKey" />
  </el-drawer>
</template>
