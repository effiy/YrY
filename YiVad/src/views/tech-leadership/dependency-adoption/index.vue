<script setup lang="ts" name="tlrDependencyAdoptionList">
import { ref } from "vue";
...
const schema = tlrMetaSchemas["dependency-adoption"];
...
<template>
  <TopicListPage tree="tech-leadership" topic="dependency-adoption" label="Dependency Adoption" :meta-columns="schema.metaColumns" @meta-click="onMetaClick" />
  <el-drawer v-model="drawerOpen" :title="$t('brdRelated.title')" direction="rtl" size="520px" :destroy-on-close="true">
    <RelatedByProjectPanel v-if="drawerProject" :project="drawerProject" current-tree="tl" current-topic="dependency-adoption" :current-key="drawerCurrentKey" />
  </el-drawer>
</template>
