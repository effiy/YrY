...
const phaseDetail = computed<string | null>(() => {
  if (!props.streaming || isUser.value) return null;
  ...
  if (store.streamingPhase === "thinking") return "Condensing context and preparing response…";
  if (store.streamingPhase === "streaming" && sourceCount.value) return `Generating answer grounded in ${sourceCount.value} source${sourceCount.value > 1 ? 's' : ''}…`;
  if (store.streamingPhase === "streaming") return "Generating response…";
  return null;
});
  ...
  >
    <div class="mb-content">
      ...
      <div v-else-if="showTyping" class="mb-typing">
        <span v-if="phaseDetail" class="mb-typing-detail">{{ phaseDetail }}</span>
        <span v-else-if="phaseLabel" class="mb-typing-phase">{{ phaseLabel }}</span>
      ...
      <div v-if="props.message.ragMeta" class="mb-rag-meta">
        ...
        <span class="mb-rag-meta-mode" :title="`llama_index ChatEngine: ${props.message.ragMeta.chatMode}`">{{ (props.message.ragMeta.chatMode || '').replace(/_/g, ' ') }}</span>
        <span v-if="props.message.ragMeta.hybrid" class="mb-rag-meta-chip mb-rag-meta-chip--on" title="Hybrid retrieval: vector + BM25">hybrid</span>
        <span v-if="props.message.ragMeta.rerank" class="mb-rag-meta-chip mb-rag-meta-chip--on" title="Cross-encoder reranking enabled">rerank</span>
        <span v-if="props.message.ragMeta.citations" class="mb-rag-meta-chip mb-rag-meta-chip--on" title="Inline citations enabled">cite</span>
        <span v-if="props.message.ragMeta.numQueries && props.message.ragMeta.numQueries > 1" class="mb-rag-meta-chip" title="QueryFusionRetriever with {{ props.message.ragMeta.numQueries }} variants">Q×{{ props.message.ragMeta.numQueries }}</span>
        ...
        >#{{ t }}</span>
        <code v-if="props.message.ragMeta.scope" class="mb-rag-meta-scope" :title="`Scope: ${props.message.ragMeta.scope}`">{{ props.message.ragMeta.scope.split('/').slice(-2).join('/') }}</code>
        <span
      ...
      <div v-if="ragContentSummary && !isUser" class="mb-rag-summary">
        <span
          v-if="retrievalGrade"
          class="mb-rag-summary-grade"
          :class="`mb-rag-summary-grade--${retrievalGrade.letter}`"
        >{{ retrievalGrade.letter }}</span>
        <span class="mb-rag-summary-text">
          检索到 <strong>{{ ragContentSummary.files }}</strong> 个文件中的 <strong>{{ ragContentSummary.count }}</strong> 个片段
          <template v-if="ragContentSummary.title">，最佳匹配：<em>{{ ragContentSummary.title }}</em></template>
        </span>
      </div>
      <!-- Per-message tool calls (Pi-inspired: tool timeline) -->
      <div v-if="toolCount && !isUser" class="mb-tools"> </div>
