<template>
  <div class="role-page">
    <header class="role-page__header">
      <h1>SRE</h1>
      <p>
        {{ totalFiles }} files across {{ subdirs.length }} problem domains — incident response,
        observability, and release management for production reliability.
      </p>
    </header>
...
const filesByDir = computed<Record<string, KnowledgeFileEntry[]>>(() => {
  ...
  for (const f of allFiles.value) {
    const dirName = f.path.replace(/^srer\//, "").split("/")[0];
...
function filePathHint(file: KnowledgeFileEntry): string {
  return file.path.replace(/^srer\//, '');
}

const STRUCTURAL_TAGS = new Set([
  'leaf', 'index', 'moc', 'summary', 'template',
  'incident-response', 'observability', 'release', 'srer',
]);
...
async function loadFiles() {
  try {
    const res = await scanKnowledge("srer");
