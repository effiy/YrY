<template>
  <div class="role-page">
    <header class="role-page__header">
      <h1>Curator</h1>
      <p>
        {{ totalFiles }} files across {{ subdirs.length }} domains — governance, templates, diagrams,
        and archive for the knowledge base lifecycle.
      </p>
    </header>
...
const subdirs = [
  ...
  { id: "templates", icon: "📝", label: "Templates", color: "#10b981", desc: "Standardized document templates for consistent knowledge creation — ADR, BRD, PRD, tech design, retrospectives, usability tests, and knowledge leaf specifications." },
...
const filesByDir = computed<Record<string, KnowledgeFileEntry[]>>(() => {
  ...
  for (const f of allFiles.value) {
    const dirName = f.path.replace(/^curator\//, "").split("/")[0];
...
function filePathHint(file: KnowledgeFileEntry): string {
  return file.path.replace(/^curator\//, '');
}

const STRUCTURAL_TAGS = new Set([
  'leaf', 'index', 'moc', 'summary', 'template',
  'governance', 'templates', 'diagrams', 'archive', 'curator',
]);
...
async function loadFiles() {
  try {
    const res = await scanKnowledge("curator");
