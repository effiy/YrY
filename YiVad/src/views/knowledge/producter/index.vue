<template>
  <div class="role-page">
    <header class="role-page__header">
      ...
      <p>
        {{ totalFiles }} files across {{ subdirs.length }} problem domains — frameworks,
        discovery, delivery, strategy, and projects for product management.
      </p>
    </header>
...
const subdirs = [
  ...
  { id: "strategy", icon: "🎯", label: "Strategy", color: "#ef4444", desc: "Product strategy and market positioning — AI after-sales case studies, RAG agent deployment cases, overseas BRD methodology, and competitive analysis." },
...
const filesByDir = computed<Record<string, KnowledgeFileEntry[]>>(() => {
  ...
  for (const f of allFiles.value) {
    const dirName = f.path.replace(/^producter\//, "").split("/")[0];
...
function filePathHint(file: KnowledgeFileEntry): string {
  return file.path.replace(/^producter\//, '');
}

const STRUCTURAL_TAGS = new Set([
  'leaf', 'index', 'moc', 'summary', 'template',
  'frameworks', 'discovery', 'delivery', 'strategy', 'projects', 'producter',
  'product', 'product-management', 'yivad', 'yiai', 'yipet',
]);
...
async function loadFiles() {
  try {
    const res = await scanKnowledge("producter");
