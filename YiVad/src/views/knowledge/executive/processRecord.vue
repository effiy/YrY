        ...
        <div class="process__section">
          ...
          <div class="process__section-body">
            ...
            <div v-if="filterLoopId" class="process__filter-banner">
              <span
                >Filtered: <strong>{{ filterLoopId }}</strong></span
              >
            ...
            <div v-if="filterGoalId" class="process__filter-banner">
              <span
                >Filtered goal: <strong>{{ filterGoalId }}</strong></span
              >
            ...
            <div v-if="!loading && !filteredLoopGroups.length" class="process__empty">
              ...
              <p class="process__empty-hint">
                Loop records are generated when AI completes a full lifecycle — from requirement review through launch. Check the
                OKR Dashboard for active tasks that may produce loop records.
              </p>
            </div>
            ...
            <div v-else-if="viewMode === 'card'" class="process__grid">
              <el-card
                ...
                :key="group.loopId"
                class="process__card"
                :class="{ 'is-highlighted': group.loopId === filterLoopId }"
                shadow="hover"
              >
                <template #header>
            ...
            <div v-else-if="viewMode === 'list'" class="process__list">
              <div v-for="group in filteredLoopGroups" :key="group.loopId" class="process__list-row">
                ...
                <div class="process__list-meta">
                  ...
                  <el-button v-if="group.summary" size="small" text type="primary" @click="openRecord(group.summary!.path)"
                    >Open</el-button
                  >
                </div>
              </div>
...
/** Loop filter from query param (e.g. /knowledge/executive/processRecord?loop=loop-001). */
const filterLoopId = computed(() => {
  const q = route.query.loop;
  return typeof q === "string" ? q : "";
});

/** Goal filter from query param (e.g. /knowledge/executive/processRecord?goal=exec-001) — shows all loops tracing that OKR goal. */
...
async function loadLoopRecords() {
  ...
  try {
    ...
    const records = files.map(recordFromFile).filter((r): r is LoopRecord => r !== null && r.stage in STAGE_ORDER);
    ...
    loopGroups.value = [...allLoopIds]
      .map(loopId => {
        const recs = (byLoop.get(loopId) || []).sort((a, b) => (STAGE_ORDER[a.stage] ?? 99) - (STAGE_ORDER[b.stage] ?? 99));
        ...
        if (summary) {
          for (const r of summary.roles) roleSet.add(r);
        }
        ...
        const goalId =
          recs.find(r => r.goalId)?.goalId || summary?.path?.split("/").find(seg => seg.startsWith("goal-")) || undefined;
...
const filteredRecordCount = computed(() => filteredLoopGroups.value.reduce((n, g) => n + g.records.length, 0));
...
function clearLoopFilter() {
  router.replace({ path: "/knowledge/executive/processRecord" });
}

function clearGoalFilter() {
  const query = { ...route.query };
  delete query.goal;
  router.replace({ path: "/knowledge/executive/processRecord", query });
}
...
/** 深链到该闭环所属的 OKR 目标（/knowledge/executive/okr?role=:role&goal=:goalId）。 */
...
.process__sticky-icon {
  flex-shrink: 0;
  margin-top: 2px;
  font-size: 28px;
}
.process__sticky-info {
  display: flex;
  flex-direction: column;
  gap: 3px;
  min-width: 0;
...
.process__stat-pill--accent {
  background: var(--el-color-primary-light-9);
}
.process__stat-pill-value {
  font-size: 18px;
  font-weight: 700;
  line-height: 1.1;
  color: var(--el-text-color-primary);
}
.process__stat-pill--accent .process__stat-pill-value {
  color: var(--el-color-primary);
}
.process__stat-pill-label {
  font-size: 10px;
  font-weight: 600;
  color: var(--el-text-color-secondary);
...
.process__sidebar-icon {
  flex-shrink: 0;
  font-size: 18px;
}
.process__sidebar-label {
  flex: 1;
  min-width: 0;
  overflow: hidden;
}
