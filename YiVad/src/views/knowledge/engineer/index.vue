<template>
  <div class="role-page">
    <header class="role-page__header">
      <h1>Engineer</h1>
      <p>
        {{ totalFiles }} files across {{ subdirs.length }} phases — Build, Ship, Run, Learn — covering the full
        design → deploy → operate → learn lifecycle for engineering teams.
      </p>
    </header>
...
const filesByDir = computed<Record<string, KnowledgeFileEntry[]>>(() => {
  ...
  for (const f of allFiles.value) {
    const dirName = f.path.replace(/^engineer\//, "").split("/")[0];
...
function filePathHint(file: KnowledgeFileEntry): string {
  return file.path.replace(/^engineer\//, '');
}

const STRUCTURAL_TAGS = new Set([
  'leaf', 'index', 'moc', 'summary', 'template',
  'build', 'ship', 'run', 'learn', 'engineer',
  'yivad', 'yiai', 'yipet',
]);
...
async function loadFiles() {
  try {
    const res = await scanKnowledge("engineer");
