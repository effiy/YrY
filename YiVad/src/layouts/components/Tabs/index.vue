      ...
      <el-tabs v-model="tabsMenuValue" type="card" @tab-click="tabClick" @tab-remove="tabRemove">
        <el-tab-pane v-for="item in tabsMenuList" :key="item.path" :label="translateTitle(item.name, item.title)" :name="item.path" :closable="item.close">
          <template #label>
            ...
            {{ translateTitle(item.name, item.title) }}
          </template>
        </el-tab-pane>
      </el-tabs>
      <MoreButton />
...
// Tabs drag sort
const tabsDrop = () => {
  // `.el-tabs__nav` is only in the DOM once el-tabs has rendered its header.
  // On the very first mount of the layout (before any tab has been pushed),
  // the nav node may not exist yet — Sortable would throw
  // "el must be an HTMLElement, not [object Null]". Guard and skip; the
  // watcher re-runs this after the first tab is added.
