      ...
      <div v-if="searchList.length" class="menu-list" ref="menuListRef">
        ...
        >
          <div class="menu-lf">
            ...
            <span class="menu-title">{{ translateTitle(item.name, item.meta.title) }}</span>
          </div>
    ...
    ? menuList.value.filter(
        item =>
          (item.path.toLowerCase().includes(searchMenu.value.toLowerCase()) ||
            ...
            translateTitle(item.name, item.meta.title).toLowerCase().includes(searchMenu.value.toLowerCase())) &&
          !item.meta?.isHide
      )
    : [];
...
const handleClickMenu = () => {
  ...
  if (!menu) return;
  if (menu.meta?.isLink) window.open(menu.meta.isLink, "_blank", "noopener,noreferrer");
