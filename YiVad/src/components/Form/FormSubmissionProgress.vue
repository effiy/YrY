      ...
      <div v-if="visible" class="form-submission-overlay">
        <div class="form-submission-progress">
          <div class="form-submission-progress__header">
            <el-icon :class="`status-${statusIcon}`" :size="24">
          ...
          <ul class="form-submission-progress__steps">
            <li v-for="step in steps" :key="step.id" class="form-submission-progress__step" :class="`is-${step.status}`">
              <el-icon :size="14">
          ...
          <div v-if="error" class="form-submission-progress__error">
            <p>{{ error }}</p>
            <p v-if="retryCount && maxRetries">已重试 {{ retryCount }}/{{ maxRetries }} 次</p>
          </div>

          <div class="form-submission-progress__actions">
            <el-button v-if="statusIcon === 'error'" type="primary" @click="emit('retry')"> 重试 </el-button>
            <el-button v-if="statusIcon === 'success'" type="primary" @click="emit('close')"> 完成 </el-button>
            <el-button v-if="statusIcon === 'loading'" text @click="emit('close')"> 后台提交 </el-button>
          </div>
        </div>
      </div>
    </Transition>
