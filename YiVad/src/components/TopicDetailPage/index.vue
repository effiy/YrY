...
const props = withDefaults(
  defineProps<{
    ...
    labelWidth?: string;
    /** i18n key prefix for translating meta-field labels. Falls back to raw label when omitted. */
    i18nPrefix?: string;
...
/**
 * Translates an option label using meta.options.<value> convention.
...
function translateOptionLabel(opt: { label: string; value: string }): string {
  const key = `meta.options.${opt.value}`;
...
async function loadEntry() {
  ...
  try {
    ...
    ElMessage.error(e instanceof Error ? e.message : t("topicDetail.loadFailed"));
  } finally {
    loading.value = false;
  }
}
...
async function handleSave(stay = false) {
  ...
  try {
    ...
    ElMessage.error(e instanceof Error ? e.message : t("topicDetail.saveFailed"));
  }
}
...
async function handleDelete() {
  ...
  try {
    ...
    ElMessage.error(e instanceof Error ? e.message : t("topicDetail.deleteFailed"));
  }
}
