    ...
    >
      ...
      <div v-if="item.metric" class="okr-card__metric" @click.stop="emit('openMetricPreview', item.metric)">
        ...
        <div class="okr-card__metric-body">
          ...
          <span class="okr-card__metric-val"
      ...
      <div v-if="item.kind === 'action' && item.status !== 'Done'" class="okr-card__progress">
        <el-progress
          ...
          :status="item.status === 'At Risk' ? 'exception' : ''"
          :stroke-width="6"
          :show-text="false"
        />
      </div>
      ...
      <div class="okr-card__dims">
        <span class="okr-card__dim" :class="`is-${item.roi}`"
          ><em>ROI</em><b>{{ levelLabel(item.roi) }}</b></span
        >
        <span class="okr-card__dim" :class="`is-${item.difficulty}`"
          ><em>难度</em><b>{{ levelLabel(item.difficulty) }}</b></span
        >
        <span class="okr-card__dim" :class="`is-${item.urgency}`"
          ><em>紧迫</em><b>{{ levelLabel(item.urgency) }}</b></span
        >
      </div>

      <div
        v-if="item.reason"
        class="okr-card__reason"
        :class="{ 'is-expanded': expandedCards.has(item.id) }"
        @click.stop="emit('toggleExpandCard', item.id)"
      >
        {{ item.reason }}
      </div>
