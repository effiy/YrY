import { onMounted, onUnmounted, watch, ref, type Ref, unref } from "vue";
...
export function useKeyboardShortcuts(shortcuts: Omit<ShortcutDefinition, "scope">[], options: UseKeyboardShortcutsOptions) {
  const scope = ref(typeof options.scope === "string" ? options.scope : unref(options.scope));
