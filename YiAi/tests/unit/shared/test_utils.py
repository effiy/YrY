...
class TestExtractJsonFromText:
    ...
    def test_markdown_code_with_invalid_json(self):
        # Invalid JSON inside markdown code block
        assert extract_json_from_text("```json\n{invalid}\n```") is None

    def test_bracket_fallback_array(self):
        result = extract_json_from_text("prefix [1, 2, 3] suffix")
        assert result == [1, 2, 3]

    def test_bracket_fallback_no_braces(self):
        # Only brackets, no braces
        result = extract_json_from_text("here is [1, 2] the end")
        assert result == [1, 2]

    def test_bracket_fallback_last_bracket(self):
        # Last bracket takes precedence for JSON ending
        result = extract_json_from_text("array: [1, 2] and more")
        assert result == [1, 2]
