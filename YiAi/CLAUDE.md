...
- **`domain/ai/agent.py`**: the no-write task-completion nudge fires once ("[TASK] … never actually invoked the write tool") — but if a weak model ignores it (observed live: qwen3.5 / qwen3-coder doing `db_schema` recon then narrating to completion without ever proposing `db_create`), the loop just ended `completed` with the write never done. New block after the no-write nudge: when `_task_nudged` is set, no write has executed, the user hasn't rejected or steered, no escalation yet, and a `model_fallback` exists → emit `MODEL_SWITCH`, swap `cfg.model` to the fallback, and inject a `[MODEL SWITCH]` takeover that names the exact failure ("only performed read-only reconnaissance … never invoked the write tool") so the "doer" model completes the task. Bounded by the same one-per-run `_model_escalated` flag as the narrate-and-stop path; mirrors that path's structure. A rejected write never re-arms; a user steer wins. (Known limitation: the gate is per-run `_write_executed`, so a *follow-up* write after the main write succeeded is not nudged/escalated — per-task write tracking is a future iteration.)
- **Verified**: deterministic `/tmp/test_nudge_escalation.py` (stub: narrate → `db_schema` recon → narrate-ignored → escalation → `db_create` + approve → summary) — 5 LLM calls, `model_switches=[('qwen3.5','qwen3-coder')]`, 1 confirmation, 1 `db_create` execution, `stop=completed`, probe menu self-cleaned. Live `/tmp/e2e_nudge_escalation.py` **2/2**: qwen3.5 stalled both runs, both escalated to qwen3-coder which executed `db_create` and ended `completed` — 0 leftover probes. Full regression: all 11 backend suites green (bounded/budget/count_gap/counts/mission/reject_memory/reject_memory_loop/spin/steer_checkpoint/skipped_tool_events/followup_drain). Frontend: no changes (vue-tsc 18 baseline unchanged).

### 2026-08-08 (later) — Surface skipped tool calls in the agent timeline (reject/timeout/blocked visibility)

- **`domain/ai/agent.py`**: a confirmation-gated tool that is **rejected, times out, or is auto-blocked** (rejection memory) is skipped by appending a blocked `ToolResult` — but previously it emitted **no** `tool_execution_start`/`tool_execution_end`, so the user saw the confirmation banner vanish and no trace of why the write never ran. The length-stop path already emitted start+end with the error for truncated calls; the confirmation-skip paths now match. New `_blocked_events(call, blocked)` helper emits `TOOL_EXECUTION_START` + `TOOL_EXECUTION_END` (with the error, e.g. "Rejected by user" / "Confirmation timed out — tool skipped" / "Blocked: identical call…") for every blocked call, wired into both the parallel and sequential execution loops. The model's context still carries the same `ToolResult` error — behavior unchanged, visibility added.
- **`YiVad/src/stores/modules/aiChat.ts`** `tool_execution_end` handler: when the ending tool matches the pending confirmation's tool, clear `pendingConfirmation` immediately — the confirmation is resolved the moment the backend surfaces the tool's end (executed OR skipped). Previously a timed-out confirmation's banner lingered up to the 120s frontend auto-reject timer, and the click was a no-op against an already-resolved id.
- **Verified**: deterministic `/tmp/test_skipped_tool_events.py` (stub LLM proposes `db_create`, confirmation stubbed "rejected") — `start_events=['db_create']`, `end_events=[('db_create','Rejected by user')]`, tool_result still carries the rejection, `stop=completed`. Live `/tmp/e2e_skipped_tool.py` (qwen3-coder, reject-first-confirmation): `db_create_starts=2` (the rejected call AND its rejection-memory auto-blocked re-issue both surfaced), both end events carry errors, 0 leftover probes. Full regression: all 10 backend suites green (bounded/budget/count_gap/counts/mission/reject_memory/reject_memory_loop/spin/steer_checkpoint + this one). vue-tsc: 0 new errors (18 baseline).
...
### 2026-08-08 (later) — Auto-steer plain messages into a running agent (pi Agent.steer)

- **Immediate steering drain (`domain/ai/agent.py`)**: the loop previously pulled the external steering store (`server.routes.agent.get_steering_messages`) into `steering_queue`, which was only drained at the *next* iteration's start — a one-turn delay that let the model act on stale instructions before ever seeing the steer. External steers now append **directly** to `agent_messages` (Pi's `Agent.steer` semantics: the direction lands in context immediately). `steering_queue`/`_drain_steering` are now vestigial (nothing feeds them) but left in place.
- **`_steering_consumed` flag (`domain/ai/agent.py`)**: a per-run flag set the moment any external steer lands in `agent_messages`. When set, all three task-completion checkpoints are suppressed — the no-write nudge, the completeness checkpoint, and the count-aware gap detector each gained `and not _steering_consumed`. Rationale: once a human steers mid-run ("actually use X instead"), their *direction* is the new ground truth and the loop must not second-guess it with heuristics built against the original task text (which would re-arm an abandoned write or demand an abandoned delete). The model now finishes the corrected task, not the original one.
- **Frontend auto-steer (`YiVad/src/stores/modules/aiChat.ts`)**: `sendMessage` previously dropped every message while the agent loop was running (`if (sending.value) return;`) — a user correcting a task mid-run ("改成 X，不是 Y") lost their words silently. A plain message typed while `sending && agentMode` now calls `steerAgent(convKey, msg)` (`POST /agent/steer`), reflects the text as a user bubble in the conversation, and toasts success/failure. Slash commands (`/steer`, `/followup`, `/stop`, …) and images keep their prior behavior.
- **Placeholder (`YiVad/src/views/aiChat/components/ChatInput.vue`)**: agent-mode placeholder now reads "Agent is running — type to redirect it, or /followup <msg> to queue after it finishes", surfacing the steer capability instead of hiding it behind `/steer` syntax.
- **Verified (2026-08-08)**: deterministic steer-checkpoint test (`/tmp/test_steer_checkpoint.py`) — RUN A steered mid-run: no delete re-armed, no nudge fired, `stop=completed`; RUN B baseline (no steer): checkpoint fires and the full create→update→delete task completes. All ~112 backend unit/integration checks green. Live e2e (`/tmp/e2e_steer_live.py`, qwen3-coder, auto-approve): a mid-run steer "title 改为 'Final'，不是 'Changed'" won over the original plan — `update_titles=['Changed','Final'] last_title='Final'`, no `db_delete`, `stop=completed`. Read-only query sanity unchanged (`mutated=False`). vue-tsc: 0 new errors (18 pre-existing, unrelated).

### 2026-08-08 (later) — Rejection memory: auto-block repeated rejected writes

- **Rejection ledger (`domain/ai/agent.py`)**: the confirmation gate now remembers what the user explicitly rejected, per session. Module-level `_session_rejections: Dict[session_id, List[str]]` stores canonical call signatures — `_call_signature` = `f"{name}|{json.dumps(arguments, sort_keys=True, ensure_ascii=False)}"` so two calls are "the same" iff same tool + same args, regardless of the provider's reused call id. When `_wait_for_confirmation` returns `"rejected"`, the preflight calls `_remember_rejection`; a `"timeout"` is deliberately NOT remembered (ambiguous — may warrant a later re-attempt after a resume). Bounded to the last `_MAX_SESSION_REJECTIONS = 20` per session (drop oldest).
- **Auto-block identical re-issues (`domain/ai/agent.py` preflight)**: before emitting `CONFIRMATION_REQUIRED`, the preflight checks `_is_rejected_call(session_id, call)` — if the user already rejected this exact call, it is **auto-blocked**: `_write_rejected = True` and a `ToolResult(error="Blocked: identical call was previously rejected by the user in this session. Do NOT retry it — change your approach or ask the user how to proceed.")` is fed back to the model with **no second prompt**. This closes the previously-documented accepted edge ("a model may still re-attempt a just-rejected write, popping a *new* confirmation the user can reject again") — the spin guard only caught it after 3 consecutive identical observations, meaning up to 2 redundant re-prompts.
- **Defensive fall-through**: an unexpected decision value from `_wait_for_confirmation` (anything but `approved`/`rejected`/`timeout`) is treated as a skip (`_write_rejected = True`, "Confirmation decision invalid — tool skipped") — it can NEVER fall through to execution. Data stays user-gated; the user just isn't asked the same question twice.
- **Verification (2026-08-08)**: (1) pure helpers 9/9 unit tests (`/tmp/test_reject_memory.py`: key-sorted args, per-session isolation, empty-session no-op, dedupe, bounded list, unicode stability); (2) deterministic loop integration test (`/tmp/test_reject_memory_loop.py`) — stub LLM proposes `db_create` then re-issues the **identical** call after rejection; the REAL preflight auto-blocked it: `wait_calls=1` (no re-prompt), `confirmations=1`, `blocked_seen=True`, `executed=False`, `stop_reason=completed`; (3) live e2e (`/tmp/e2e_reject_memory.py`, qwen3-coder, steer retry): `confirm_count=1 blocked_seen=True doc_created=False final_stop=completed` — the model's real re-issue was auto-blocked with the "Blocked: identical call" error and zero second prompt. All prior suites still green (20+15+15+9+17+9).

### 2026-08-08 (later) — Turn-budget awareness (pi: plan within max_turns)

- **Problem**: `turn_index`/`max_turns` only ever reach the frontend (SSE events) — the **model never sees its own turn budget**, so it over-plans and gets cut off mid-task at `max_turns_reached`. An agent at turn 7/10 happily starts a new 3-step plan it cannot finish.
- **`_budget_warning(turn_index, max_turns, warn_leftover=3)` (`domain/ai/agent.py`)**: pure helper returning a one-shot `[BUDGET]` note ("本次运行最多 N 轮，当前第 X 轮，还剩 R 轮。压缩非必要步骤；若不够完成，明确说还差哪些，方便用户回复「继续」") once the run is within `warn_leftover` turns of the limit. Injected at turn start, exactly once per run (`_budget_injected`), only near the wall — normal runs (which finish well before turn max_turns−3) never see it. The `[BUDGET]` prefix rides the existing `_last_user_text` `[`-skip convention, so the injected note can never shadow the user's real task for the completion checkpoints (verified: a write task past a trailing `[BUDGET]` note still reads as a write request).
- **Verified**: 15/15 unit tests (`/tmp/test_budget.py` — firing boundary at rem 3, last-turn rem 0, tight budget turn 1/2, custom `warn_leftover`, `[`-skip resilience). Live: the resume harness (run 1 `max_turns=2`) fires "Agent budget warning injected … turn 1/2, 1 left" in the server log, and the resume still completes the full lifecycle PASS. Regression: cycle e2e SUCCESS, all suites (bounded/spin/mission/counts/count_gap) pass.

### 2026-08-08 (later) — Repeated-observation spin guard (pi: break a stuck loop)

- **Problem**: a model can get stuck re-issuing the same tool call and observing the same result every turn — a read loop on an unchanged query, or re-attempting a just-rejected write. None of the existing guards catch it (narrate-guard needs *named but unexecuted* tools; escalation needs a narrate; nudges need a write gap), so a spinning model burns all of `max_turns` and ends `max_turns_reached` with the task undone.
- **Detection (`domain/ai/agent.py`)**: pure helpers `_turn_observation_signature(tool_results)` (name + effective result content per turn; errors render as `Error: …`) and `_advance_spin_state(prev, run, obs, threshold=3)` (narration-only turns reset the chain; identical non-empty observations increment; `threshold` reached fires once and resets the run so the same spin doesn't re-fire every turn). Real progress *changes* the observation — a write alters the data a later read sees, a different query returns different rows — so a normal create→update→delete cycle never counts as a spin.
- **Nudge**: on 3 consecutive identical observations the loop injects one `[TASK]` user message ("stop repeating this step; re-read the task and do the next step; if a step was rejected by the user, do NOT retry it") and continues. Bounded to one per run (`_spin_nudged`). The rejection-aware clause directly breaks the re-attempt-a-rejected-write loop (which previously popped a new confirmation each turn).
- **Verified**: 15/15 unit tests (`/tmp/test_spin.py` — signature shapes incl. error results, narration-reset, fire-at-threshold, threshold param, normal create-cycle trace never fires, rejected-write repeat fires). Live: the menu cycle e2e stays SUCCESS with **zero** "Agent spin guard fired" log lines (normal observations never false-fire). Regression: bounded/mission/counts/count_gap suites all pass.

### 2026-08-08 (later) — Bound oversized tool results in LLM context (pi context budgeting)

- **Problem**: a `db_list` can return up to 1000 docs (~75K tokens) — far past the 16K `ollama_num_ctx` window. Compaction only fires *after* the turn and *summarizes away* the exact data the model needs, and Ollama over-window truncates from the middle (can split a tool_result mid-JSON) — a real cause of small-model task failures on list-heavy menu work. Measured: `db_list menus limit=1000` returns **21,267 chars** today.
- **`_bound_tool_result(name, content, max_chars)` (`domain/ai/agent.py`)**: a pure helper that bounds a single tool result *only at the LLM-context rendering boundary* — persistence and the UI keep the full content. Keeps a head (70%) + tail (22%) and appends an explicit note (`… [result truncated for context: N chars, ~K lines omitted … Re-query with filter/fields/key to fetch the specific items you need.]`) so the model knows the rest exists and how to re-query it — correctness preserved, context bounded. Shorted results (< `AgentConfig.max_tool_result_chars`, default 6000) pass through byte-identical. Wired into `_stream_llm_response`'s `tool_result` → `[Tool result: <name>]` rendering.
- **Verified**: 20/20 unit tests (`/tmp/test_bounded.py` — short/exact-boundary passthrough, head/tail/note for list-shaped and single-blob inputs, omitted-line count accuracy, custom bound, pathological input still bounded). Live: the large-list read-only e2e (`/tmp/e2e_bounded.py`, qwen3-coder) — one `db_list` of 21KB, `stop=completed`, correct answer (66 docs, first two named), **zero mutating tools**; the normal cycle e2e remains SUCCESS. Regression: mission/counts/count_gap suites still 35/35.
