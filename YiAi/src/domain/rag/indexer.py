...
# operate on the same persist directory concurrently.
...
def _to_rel_file_path(doc: Any) -> str:
    ...
    works with ``engineer/learn/projects/yivad/`` style scopes.
    """
    base = os.path.realpath(os.path.abspath(settings.knowledge_base_dir))
...
async def refresh_index_async(added: list, removed: list, changed: list) -> Any:
    """Async wrapper — runs the sync refresh in a worker thread, with a global
    lock so incremental refresh and full rebuild never overlap."""
...
async def rebuild_index_async() -> Any:
    """Full rebuild with the global index lock — prevents overlap with
    incremental refresh triggered by the knowledge watcher."""
    async with _index_lock:
        import asyncio
