...
def _apply_rss_date_filters(
    ...
    if "publishedStart" not in query_params and "publishedEnd" not in query_params:
        return None
    start_ms = _parse_ms_ts(query_params.get("publishedStart"))
    end_ms = _parse_ms_ts(query_params.get("publishedEnd"))
...
async def create_document(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if 'duplicate key' in str(e).lower() or 'E11000' in str(e):
            if collection_name == 'rss':
                raise ValueError(f"Link field value '{data_copy.get('link', '')}' already exists, cannot create duplicate") from e
            raise ValueError("Data creation failed: unique constraint violation") from e
        raise
