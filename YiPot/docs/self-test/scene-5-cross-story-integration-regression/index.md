...
(arch or self-test) does not silently invalidate another story, and that
the dashboard wiring (`docs/index.html` + `docs/data.js` + `docs/arch/` +
`docs/self-test/`) still resolves end-to-end.
...
flowchart LR
    ...
    E[arch/scene-5/ index.md] -- "5-dim baseline" --> F[self-test/scene-4/ index.md]
    F -- "regression rules" --> G[git diff keyword scan]
    H[self-test/scene-3/ index.md] -- "doc-code parity" --> I[README + CLAUDE + arch + self-test]
    I --> J{drift?}
...
**Steps**: For every `href: 'arch/scene-*/index.md'` and `href: 'self-test/scene-*/index.md'` in `data.js`, the target file exists.
...
## AC-5: `self-test/scene-3` references match `arch/scene-5` baseline
**Steps**: The "5-dim baseline" row in self-test/scene-3 matches the "Trust boundaries (5 dimensions)" header in arch/scene-5.

## AC-6: `data.js` panelHub buttons match `arch/index.html` and `self-test/index.html` (if those exist)
**Steps**: If `arch/index.html` and `self-test/index.html` exist, their `data.js` files are also linked.
**Verify**: `panelHub.buttons` includes both `arch` and `self-test` panels.
...
| `data.js` (panelHub.buttons) | `arch/index.html` + `self-test/index.html` | `find` |
| `data.js` (sceneLinks in stories) | `arch/scene-*/index.md` + `self-test/scene-*/index.md` | `find` |
...
| `self-test/scene-3/index.md` | `arch/scene-5/index.md` (5-dim baseline) | `grep` |
| `self-test/scene-4/index.md` | `arch/scene-5/index.md` (baseline) | `grep` |
| `README.md` (Domain Language) | `src/services/*` + `src/window/*` + `src-tauri/src/server.rs` | `grep` |
| `CLAUDE.md` (Documentation Navigation) | `docs/arch/*/index.md` + `docs/self-test/*/index.md` | `test -e` |
...
| `self-test/scene-1-post-init-full-self-check` | "§0 Effect Sketch — Post-Init Full Self-Check" | ✅ |
| `self-test/scene-2-pre-commit-incremental-self-check` | "§0 Effect Sketch — Pre-Commit Incremental Self-Check" | ✅ |
| `self-test/scene-3-doc-code-consistency` | "§0 Effect Sketch — Doc-Code Consistency" | ✅ |
| `self-test/scene-4-security-surface-regression` | "§0 Effect Sketch — Security Surface Regression" | ✅ |
| `self-test/scene-5-cross-story-integration-regression` | "§0 Effect Sketch — Cross-Story Integration Regression" | ✅ |
| `self-test/scene-6-third-party-framework-service` | "§0 Effect Sketch — Third-Party Framework Service Health" | ✅ |
...
| AC-5 | ✅ | `self-test/scene-3` 5-dim baseline matches `arch/scene-5` |
| AC-6 | ✅ | `panelHub.buttons` covers both `arch` and `self-test` |
...
- **`docs/arch/index.html` and `docs/self-test/index.html` are listed as expected but not yet emitted**. The `panelHub` URL in `data.js` points to them; the dashboard will 404 on the panel button until those files are created. (Future work; not in scope of this init run.)
- **The 5-dimension baseline in `arch/scene-5` is textually** consistent with `self-test/scene-3`, but a contributor who edits one but not the other will silently drift. A `pnpm test:cross-story` script that `diff`s the two would catch this.
...
- Add a `pnpm test:cross-story` script that runs all 6 ACs above + a `diff` of the 5-dim baseline text between `arch/scene-5` and `self-test/scene-3`.
- Add a `docs/arch/index.html` + `docs/self-test/index.html` that summarize the 5/6 scenes with the same `data.js` shape, so the `panelHub` URL has a real target.
