...
export async function recognize(base64, language, options = {}) {
    ...
    let request_body = {
        header: {
            app_id: appid, // appid obtained from the iFlytek open platform
            status: 3, // Request status. Value: 3 (send all at once)
        },
        parameter: {
            sf8e6aca1: {
                category: 'ch_en_public_cloud', // ch_en_public_cloud: Chinese-English recognition
                result: {
                    encoding: 'utf8',
                    compress: 'raw',
                    format: 'json',
    ...
    let text = CryptoJS.enc.Base64.parse(res_payload['result']['text']); // Base64 decode
    let text_string = CryptoJS.enc.Utf8.stringify(text);
    let text_json = JSON.parse(text_string);
    let return_content = ''; // Final result
