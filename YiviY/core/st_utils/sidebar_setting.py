...
def page_setting():
    ...
    with st.expander(t("Youtube Settings"), expanded=False):
        config_input(t("Cookies Path"), "youtube.cookies_path",
                     help=t("Path to YouTube cookies file for authentication"))
        config_input(t("Proxy"), "youtube.proxy",
                     help=t("Proxy for accessing YouTube (e.g., http://127.0.0.1:7890)"))
        download_fmt = st.selectbox(
            t("Download Format"),
            options=["video+audio", "audio-only"],
            format_func=lambda x: "🎬 Video + Audio" if x == "video+audio" else "🎵 Audio Only",
            index=["video+audio", "audio-only"].index(load_key("youtube.download_format")
                                                       if load_key("youtube.download_format") in ["video+audio", "audio-only"]
                                                       else "video+audio"),
        )
        if download_fmt != load_key("youtube.download_format"):
            update_key("youtube.download_format", download_fmt)
            st.rerun()
        download_subs = st.toggle(
            t("Download Subtitles"),
            value=load_key("youtube.download_subtitles"),
            help=t("Download YouTube subtitles/CC during video download"),
        )
        if download_subs != load_key("youtube.download_subtitles"):
            update_key("youtube.download_subtitles", download_subs)
            st.rerun()
