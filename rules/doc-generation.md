...
flowchart TB
    ...
    subgraph C8["⑧ 效果证"]
        V1["03/04 技术评审 → 效果示意"]:::c
        V2["06/07 实施报告 → 实施截图"]:::c
        V3["06/07 实施报告 → 可操作验证步骤"]:::c
    end
...
| ⑧ 效果证 | 03/04 技术评审必含效果示意，06/07 实施报告必含截图+可操作验证步骤（后端 curl，前端操作路径） | 实施报告只有文字无截图无验证命令 |
...
| # | 规则 | 反例 |
|---|------|------|
| 15 | **评审有效果** — 03 后端技术评审必须包含架构效果示意 mermaid 图，展示实现后的预期系统行为与数据流；04 前端技术评审必须包含 UI 效果示意，展示实现后的预期界面形态 | 技术评审只有接口列表和组件树，无效果总览图 |
| 16 | **报告有截图** — 06 后端实施报告必须包含实际 API 响应截图或终端运行输出截图；07 前端实施报告必须包含实际 UI 截图。截图需附简要说明 | 实施报告无任何截图，仅文字描述"已完成" |
| 17 | **报告可操作** — 06 后端实施报告必须包含每个接口的 curl 验证命令（含完整 URL、请求体、预期响应）；07 前端实施报告必须包含从入口到目标页面的操作路径（含点击序列与预期显示） | "API 已通过测试" — 无 curl 命令可供复现；"页面正常显示" — 无操作路径可供验证 |

### 效果图类型指南

| 文档 | 效果图类型 | 最低数量 | 内容要求 |
|------|-----------|---------|---------|
| 03 后端技术评审 | mermaid flowchart / sequenceDiagram | ≥ 1 | 展示核心业务流经各服务/模块的完整路径与关键决策点 |
| 04 前端技术评审 | mermaid 组件交互图 / UI 线框图 | ≥ 1 | 展示核心用户操作对应的组件渲染路径与状态变化 |
| 06 后端实施报告 | 终端截图（含 curl 请求+响应） | ≥ 1 张/接口 | 完整展示请求命令与服务器返回结果 |
| 07 前端实施报告 | UI 截图 | ≥ 1 张/场景 | 展示实际渲染的页面，覆盖正常态 + 关键状态（空/错误/加载） |

### 可操作验证步骤规范

| 文档 | 验证格式 | 必含要素 |
|------|---------|---------|
| 06 后端实施报告 | fenced code block (`bash`)，每接口一块 | 完整 curl 命令（含 URL、method、headers、body）、预期响应摘要。不可用 `localhost`，使用可配置的基础 URL 占位符 `${BASE_URL}` |
| 07 前端实施报告 | 编号操作步骤列表 | 起始页面 → 每步操作（点击/输入）→ 预期显示。步骤可被独立复现，不依赖上下文记忆 |
...
| 效果证：03/04 有效果示意，06/07 有截图+可操作验证步骤 | 补效果图，补 curl/操作步骤 |
