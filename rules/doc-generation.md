...
`docs/故事任务面板/` 下的故事文档产出。`libs/` 外部参考文档以及 `rules/` `agents/` 规则与角色定义不受此约束。
...
flowchart LR
    ...
    subgraph 产出["创建文件"]
        O1["故事任务<br/>使用场景<br/>技术评审<br/>测试设计"]:::file
        O2["实施报告<br/>测试报告"]:::file
        O3["自改进复盘"]:::file
        O4["消息通知列表"]:::file
    end
    P1 --> O1
...
| 阶段 | 创建文件 | 条件 |
|------|---------|------|
| 文档生成 | 故事任务 + 使用场景 + 技术评审 + 测试设计 + 补充 | 故事任务/使用场景 必创建；技术评审 按项目类型 |
...
| 基线声明：故事任务+使用场景 均有 §0 基线声明且无禁止内容 | 补基线声明或移除禁止内容 |
