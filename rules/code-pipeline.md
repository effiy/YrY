...
flowchart TB
    ...
    GB -->|"✅ 通过"| SI["⑤ 自改进<br/>09-自改进复盘"]:::phase
    SI --> DONE["交付"]:::done
...
| ② Gate A | 05-测试用例评审.md 存在且就绪 | `skip-gate-a` | 单行 CSS/文案 |
| ③ 逐模块清零 | 每模块 P0 清零后进下一模块 | `chain-broken` | — |
| ④ Gate B | 5 步验证 + 三报告闭合，修复 ≤ 2 轮 | `gate-b-limit` | — |
| ⑤ 自改进 | 产出 09-自改进复盘 | `no-metrics`（降级不阻断） | 数据采集失败时降级 |
...
```mermaid
flowchart TD
    ENTER["准备编码"] --> Q1{"05-测试用例评审.md<br/>存在?"}
...
| # | 规则 | 说明 |
|---|------|------|
| 5 | `05-测试用例评审.md` 不存在，不得编码 | 阻断标识 `skip-gate-a` |
...
| 15 | 自改进必须产出 09-自改进复盘 | `no-metrics`（降级） |
...
| 技术 | 适用阶段 | 来源 |
|------|---------|------|
| 根因追溯 | P0 修复 · Gate B 验证 | superpowers 调试纪律 |
| 纵深防御 | P0 修复 · 安全约束 | superpowers 多层校验 |
| 条件等待 | 测试编写 · Gate A | superpowers 确定性等待 |
| 验证门禁 | Gate A · Gate B · 交付 | superpowers 五步法 |
| 反馈回路 | 诊断 · 调试 | superpowers 2秒确定回路 |
| 深度模块 | 架构设计 · 逐模块实现 | Ousterhout《A Philosophy of Software Design》 |
| 垂直切片 | Gate A · TDD | superpowers TDD 纪律 |
| 研究优先 | 影响分析 · 架构设计 | everything-claude-code 上下文质量优先 |
| 记忆注入 | P0 修复 · 自改进 | claude-mem 跨会话相似检索 |

### ⑧ 研究优先开发

**Iron Law: NO ACTION WITHOUT FACTS FIRST**

> 源自 [everything-claude-code](https://github.com/affaan-m/everything-claude-code) 的上下文质量优先原则。涉及不熟悉模块、外部依赖、或 API 变更时：先 Read/Grep/Glob 建立事实基线，再行动。猜 = 浪费上下文。
