...
17. 故事目录名必须包含项目前缀：`docs/故事任务面板/<project>-<name>/`（`<project>` 为项目标识，`<name>` 为 kebab-case 故事名）。缺失项目前缀的目录由 `node ~/.claude/plugins/marketplaces/yry/skills/rui/scripts/list.js` 和 `node ~/.claude/plugins/marketplaces/yry/skills/rui/scripts/recommend.js` 告警。