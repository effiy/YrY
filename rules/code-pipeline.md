...
| ② Gate A | 场景文档 §1 测试设计存在且就绪 | `skip-gate-a` | 单行 CSS/文案 |
...
```mermaid
flowchart TD
    ENTER["准备编码"] --> Q1{"场景文档 §1<br/>测试设计存在?"}
...
| # | 规则 | 说明 |
|---|------|------|
| 5 | `场景文档 §1` 不存在，不得编码 | 阻断标识 `skip-gate-a` |
...
| 15 | 自改进必须产出 场景-N-<slug>.md §4 | `no-metrics`（降级） |
...
| Gate A 通过（场景文档 §1 存在且就绪） | 退回 tester 补充测试设计 |
