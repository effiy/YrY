...
10. **分支隔离强制**：`/rui-claude <requirement>` 对 `.claude/` 目录的任何代码修改必须通过 rui code 管线（预检→测试先行→实现→验证→自改进）进行，预检阶段必须从 main 创建 `feat/<name>` 分支并 checkout，禁止在非故事分支上直接修改 `.claude/` 文件，禁止跳过管线直接编辑文件（`no-checkout`）
