...
import { scanFiles } from "./recommend-detect.mjs";

function detectType(/** @type {string} */ root) {
...
function extractVueSignatures(/** @type {string} */ content) {
  const sigs = [];
...
function extractReactSignatures(/** @type {string} */ content) {
  const sigs = [];
...
function extractSvelteSignatures(/** @type {string} */ content) {
  const sigs = [];
  const exportMatch = content.match(/<script[^>]*>\s*export\s+let\s+(\w+)/g);
  if (exportMatch) {
    const names = exportMatch.map((/** @type {string} */ m) => /** @type {RegExpMatchArray} */ (m.match(/export\s+let\s+(\w+)/))[1]);
    sigs.push("Props: " + names.join(", "));
  }
...
function extractRouteSignatures(/** @type {string} */ content) {
  const sigs = [];
  ...
  for (const m of decoratorMatches) {
    const method = /** @type {RegExpMatchArray} */ (m[0].match(/@(\w+)/))[1];
...
function extractSignatures(/** @type {string} */ filePath, /** @type {string} */ fileType) {
  let content;
...
function buildDependencyGraph(/** @type {Array<{ path: string, type: string }>} */ files) {
...
function toKebab(/** @type {string} */ s) {
  return s
...
function deriveName(/** @type {string} */ file, /** @type {string} */ project) {
...
function docStatus(/** @type {string} */ root, /** @type {string} */ project, /** @type {string} */ name) {
  ...
  if (dirExists) {
    try {
      existingFiles = readdirSync(expectedDir).filter((/** @type {string} */ f) => f.endsWith(".md"));
    } catch { existingFiles = []; }
  }
...
function gitMetrics(/** @type {string} */ root, /** @type {string} */ file) {
  try {
    ...
    const authorCount = parseInt(execSync(
      ...
      { cwd: root, encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] }
    ).trim().split("\n").filter(Boolean).filter((/** @type {string} */ v, /** @type {number} */ _i, /** @type {string[]} */ a) => a.indexOf(v) === _i).length, 10) || 0;
...
// --- line count ------------------------------------------------------------
function countLines(/** @type {string} */ content) {
  if (!content) return 0;
  return content.split("\n").length;
}

// --- security signals -------------------------------------------------------
function securitySignals(/** @type {string} */ content, /** @type {string} */ _type) {
...
async function collect(/** @type {string} */ root, /** @type {Array<{ path: string, type: string }>} */ files, /** @type {string} */ project, /** @type {string} */ _projectType) {
  ...
  for (const { path: file, type } of files) {
    ...
    const importedByList = (importedBy.get(file) || []).map((/** @type {string} */ f) => relative(root, f));
...
function groupIntoStories(/** @type {any[]} */ fileResults, /** @type {string} */ project, /** @type {string} */ projectType) {
  const stories = [];
  const used = new Set();
        ...
        if (dirname(importer) === dir) {
          ...
          if (match) {
            ...
            const importerImportsThis = match.metrics.importedBy.some(
              (/** @type {string} */ ib) => fileResults.find((/** @type {any} */ r) => r.file === ib)?.file === f.file
            ...
            const hasSharedConsumer = match.metrics.importedBy.some(
              (/** @type {string} */ ib) => f.metrics.importedBy.includes(ib)
            );
...
function buildStoryCandidate(/** @type {any[]} */ group, /** @type {string} */ project, /** @type {string} */ projectType) {
...
// --- format output ---------------------------------------------------------
function formatOutput(/** @type {any[]} */ results, /** @type {string} */ format) {
  if (format === "jsonl") {
...
// --- summary stats ---------------------------------------------------------
function printSummary(/** @type {any[]} */ stories) {
  const totalFiles = stories.reduce((/** @type {number} */ s, /** @type {any} */ st) => s + st.metrics.fileCount, 0);
...
async function main() {
  ...
  const projectType = /** @type {any} */ (args.type === "auto" ? detectType(root) : args.type);
