...
import { nowISO, isMain } from "../../lib/fs.mjs";
 ...
 */
function cronIntervalMinutes(cronExpr) {
 ...
 */
function isCronDue(cronExpr, lastRunISO, now = new Date()) {
 ...
 */
function runCheck(skill, checkScript, timeoutMs = 120_000) {
 ...
 */
function generateReport(skill, status, summary) {
  const args = [
 ...
 */
async function notifyP0Escalation(skill, status, summary, consecutiveFails) {
 ...
 */
function getDueSkills(state, now, forceSkill) {
  if (forceSkill) {
 ...
 */
function getOverdueSkills(state, now = new Date()) {
  return getCronEligibleSkills().filter(s => {
    const last = state[s.skill];
    ...
    const ageMin = (now - new Date(last.lastRun)) / 60000;
 ...
 *   Returns { ran, passed, failed, p0Escalated, p0Notified }.
 */
async function dispatch(opts = {}) {
  ...
  if (due.length === 0) {
    ...
    return { ran: 0, skipped: 0, p0Escalated: 0, p0Notified: 0 };
  }
  ...
  for (const skill of due) {
    ...
    const { exitCode, status, output } = runCheck(skill.skill, skill.checkScript);
  ...
  if (overdue.length > 0) {
    ...
    overdue.forEach(s => {
      const last = state[s.skill];
      const ageMin = ((now - new Date(last.lastRun)) / 60000).toFixed(0);
      const intervalMin = cronIntervalMinutes(s.intervalCron);
      console.log(`    ⚠️ ${s.skill}: age=${ageMin}min, interval=${intervalMin}min (overdue ×${(ageMin/intervalMin).toFixed(1)})`);
    });
  }
...
function printStatus() {
  ...
  for (const s of skills) {
    ...
    const ageMin = ((now - new Date(last.lastRun)) / 60000).toFixed(0);
    const intervalMin = cronIntervalMinutes(s.intervalCron);
    const isOverdue = ageMin > intervalMin * OVERDUE_MULTIPLIER;
