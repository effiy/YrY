...
import { findPluginHelpPath } from "./io.mjs";
import { NODE_ARGV_OFFSET } from "./constants.mjs";
import { bold as ttyBold, underline, dim, yellow, green } from "./tty.mjs";

const SKILL_NAME = "rui";

/** Parse process.argv for the recommend CLI.
 * @returns {{ root: string|null, type: string, format: string }}
 *   - `root`: explicit `--root=X` override (or null)
 *   - `type`: project type (auto-detected if `auto`)
...
/** Display CLI help, preferring installed plugin help.mjs over inline fallback.
 * Exits the process with code 0 after rendering. */
export function showHelp() {
  ...
  function hdr(/** @type {string} */ t) { return `\n${bold(underline(t))}\n`; }
  /**
   * @param {string} c
   * @param {string} d
   * @param {(s: string) => string} [clr]
   */
  ...
  function line(/** @type {string} */ t) { return `${INDENT}${t}`; }
