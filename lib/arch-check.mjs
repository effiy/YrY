...
/**
 * Run the 10-dimension architecture compliance check.
 *
 * @param {string} projectRoot - Absolute path to project root
 * @returns {{ pass: boolean, dimensions: Array<{ dim: string, label: string, pass: boolean, checks: Array<{ id: string, pass: boolean }> }>, summary: { totalChecks: number, passedChecks: number, failedChecks: number, failedDimensions: string[], overallPass: boolean, grade: string } }}
 */
...
/**
 * Return a one-line human-readable summary of the architecture compliance result.
 *
 * @param {string} projectRoot - Absolute path to project root
 * @returns {string} e.g. `[arch] A级 | 30/30 通过 | 全部通过`
 */
...
/**
 * Extract per-dimension scores for trend persistence.
 *
 * @param {string} projectRoot - Absolute path to project root
 * @returns {{ archComposite: number, archGrade: string, archScores: object, archFailedDims: string[] }}
 */
export function archScoresForTrend(projectRoot) {
  ...
  for (const dim of dimensions) {
    const passed = dim.checks.filter((/** @type {{ pass: boolean }} */ c) => c.pass).length;
...
/**
 * Append a trend entry to .memory/arch-trend.jsonl with current scores and git branch.
 *
 * @param {string} projectRoot - Absolute path to project root
 * @returns {object|null} The appended entry, or null if write failed
 */
...
if (_isMain) {
  ...
  if (args.includes("--append-trend")) {
    ...
    if (entry) {
      /** @type {any} */
      const e = entry;
      console.log(`[arch] 趋势已追加: ${e.archGrade}级 ${e.archComposite}分`);
      process.exit(0);
    } else {
