...
/** Compute population mean, variance, and stddev for a non-empty numeric array.
 * @param {number[]} arr
 * @returns {{ mean: number, variance: number, stddev: number }}
 */
function meanVar(arr) {
  const n = arr.length;
  const mean = arr.reduce((/** @type {number} */ a, /** @type {number} */ b) => a + b, 0) / n;
  const variance = arr.reduce((/** @type {number} */ s, /** @type {number} */ v) => s + (v - mean) ** 2, 0) / n;
 ...
 * @param {number} score
 * @returns {{ grade: string, label: string, color: string, ansi: string, min: number }}
 */
 ...
 * @returns {(number|null)[]} SMA values, same length as input (first window-1 values are null)
 */
...
export function scoreDistribution(scores) {
  ...
  const { mean, stddev } = meanVar(sorted);
  const median = n % 2 === 0
 ...
 * @param {Record<string, number>} weights - { dimKey: weight }
 * @returns {{
 *   topDrag: Array<{dim: string, score: number, weight: number, contribution: number, maxContribution: number, gap: number}>,
 *   topBoost: Array<{dim: string, score: number, weight: number, contribution: number, maxContribution: number, gap: number}>,
 *   dragTotal: number
 * }}
 */
export function contributionAnalysis(scores, weights) {
  const totalWeight = Object.values(weights).reduce((/** @type {number} */ a, /** @type {number} */ b) => a + b, 0);
 ...
 * @param {Record<string, number>} scores - { dimKey: score }
 * @param {Record<string, number>} weights - { dimKey: weight }
 * @param {Record<string, number[]>} [history] - { dimKey: number[] } historical scores per dimension
...
export function volatilityAdjustedComposite(scores, weights, history) {
  ...
  for (const [dim, weight] of Object.entries(weights)) {
    ...
    if (hist && hist.length >= VOLATILITY_MIN_HISTORY) {
      const { mean, stddev } = meanVar(hist);
...
export function confidenceAdjustedComposite(composite, reliability) {
  /** @type {'high'|'medium'|'low'} */ let level;
  let discount;
 ...
 *
 * @param {Record<string, any>} dimHistory - { dimLabel: [{date: string, score: number}] }
...
export function crossDimensionCorrelation(dimHistory) {
  ...
  for (let i = 0; i < dims.length; i++) {
    for (let j = i + 1; j < dims.length; j++) {
      ...
      const xMean = xs.reduce((/** @type {number} */ a, /** @type {number} */ b) => a + b, 0) / n;
      const yMean = ys.reduce((/** @type {number} */ a, /** @type {number} */ b) => a + b, 0) / n;
 ...
 * @param {Record<string, number>} scores - { dimKey: score }
 * @param {Record<string, number>} weights - { dimKey: weight }
 * @param {Record<string, number[]>} [history] - { dimKey: number[] } historical scores
...
export function improvementPotentialRanking(scores, weights, history) {
  ...
  for (const [dim, weight] of Object.entries(weights)) {
    ...
    let volatility = 0;
    /** @type {'stable'|'rising'|'falling'} */ let trend = "stable";
 ...
 * @param {Record<string, number>} scores - { dimKey: score }
 * @param {Record<string, number>} weights - { dimKey: weight }
 * @param {Record<string, number[]>} [history] - { dimKey: number[] }
...
export function scoreStabilization(history) {
  ...
  for (let i = 0; i <= history.length - windowSize; i++) {
    ...
    const { mean, stddev } = meanVar(window);
    const cv = mean !== 0 ? stddev / Math.abs(mean) : 0;
    rollingCVs.push(cv);
  }
  ...
  /** @type {'stable'|'converging'|'diverging'} */ let status;
  let description;
 ...
 *
 * @param {Record<string, number>} scores - { dimKey: score }
 * @param {Record<string, any>} dimensions - { dimKey: { category, weight } }
 * @returns {Record<string, any>} { [category]: { score, weight, dims } }
 */
 ...
 *
 * @param {Record<string, number>} scores - Current { dimKey: score }
 * @param {Record<string, any>} dimensions - HEALTH_SCORING_DIMENSIONS { dimKey: { weight, label, category } }
 * @param {Record<string, any>} [history] - Optional { label: [{date, score}] } from getDimensionHistory()
...
export function rankDimensionInfluence(scores, dimensions, history) {
  ...
  for (const [dim, cfg] of Object.entries(dimensions)) {
    ...
    const weightFactor = cfg.weight;

    let varianceFactor = 0;
    if (history) {
      const points = history[cfg.label] || [];
      if (points.length >= 3) {
        const scoreVals = points.map((/** @type {any} */ p) => p.score);
        varianceFactor = scoreDistribution(scoreVals).stddev;
      }
    }

    const gapFactor = 100 - (scores[dim] || 0);
    const correlationFactor = 0;
 ...
 * @param {Array<any>} [opts.topDrag] - From contributionAnalysis
 * @param {Array<any>} [opts.influence] - From rankDimensionInfluence
...
export function generateExecutiveSummary(opts) {
  ...
  const grade = opts.grade || "D";
  /** @type {Record<string, number>} */
  const scores = /** @type {any} */ (opts.scores || {});
  /** @type {any} */
  ...
  const gradeLabel = grade === "A" ? "优秀" : grade === "B" ? "良好" : grade === "C" ? "一般" : "需关注";
  const healthAdj = composite >= 90 ? "健康" : composite >= 75 ? "基本健康" : composite >= 60 ? "需要关注" : "存在风险";

  /** @type {Record<string, {pass: number, fail: number}>} */
  const catCounts = { core: { pass: 0, fail: 0 }, structural: { pass: 0, fail: 0 }, engineering: { pass: 0, fail: 0 }, quality: { pass: 0, fail: 0 } };
  for (const [dim, score] of Object.entries(scores)) {
    const cat = (dimensions[dim] && dimensions[dim].category) || "core";
  ...
  const sortedDims = Object.entries(scores)
    .filter(([k]) => dimensions[k] !== undefined)
    .sort((a, b) => a[1] - b[1]);
  const worst3 = sortedDims.slice(0, 3).map(([k]) => (dimensions[k] && dimensions[k].label) || k);
  const best3 = sortedDims.slice(-3).reverse().map(([k]) => (dimensions[k] && dimensions[k].label) || k);

  let summary = `项目健康综合评分为 ${composite} 分（${grade}级·${gradeLabel}），整体状态${healthAdj}。`;

  /** @type {Record<string, string>} */
  const catLabels = { core: '核心运营', structural: '结构健康', engineering: '工程成熟度', quality: '组件质量' };
  const catParts = [];
  for (const cat of ['core', 'structural', 'engineering', 'quality']) {
  ...
  if (prev) {
    const diff = composite - prev.composite;
  ...
  if (archResult) {
    const archFailed = archResult.archFailedDims?.length || 0;
  ...
  const highlights = [];
  if (worst3.length > 0) highlights.push(`需优先改进: ${worst3.slice(0, 3).join('、')}`);
  if (best3.length > 0) highlights.push(`表现最佳: ${best3.slice(0, 3).join('、')}`);
  ...
  const risks = [];
  for (let i = 0; i < sortedDims.length && i < 5; i++) {
    const [dim, score] = sortedDims[i];
    const label = (dimensions[dim] && dimensions[dim].label) || dim;
    if (score < 60) risks.push(`${label}得分 ${score}，需立即处理`);
    else if (score < 80) risks.push(`${label}得分 ${score}，建议改进`);
  }
 ...
 *
 * @param {any} current - Current period data { composite, scores: {dimKey: score}, date }
 * @param {any} previous - Previous period data { composite, scores: {dimKey: score}, date }
 * @returns {{
...
export function scoreReliability(history, zScore) {
  ...
  const n = history.length;
  const { mean, stddev } = meanVar(history);
  ...
  /** @type {'low'|'moderate'|'high'} */ let volatility;
 ...
 *
 * @param {Record<string, number>} scores - { dimKey: score (0-100) }
 * @param {Record<string, any>} dimensions - { dimKey: { label, weight, category } }
 * @param {object} [opts]
...
export function spiderChartData(scores, dimensions, opts) {
  ...
  const categories = Object.values(catMap).map((/** @type {any} */ c) => ({
    key: c.key,
    label: c.label,
 ...
 * @param {Record<string, number>} scores - Current { dimKey: score }
 * @param {Record<string, any>} dimensions - HEALTH_SCORING_DIMENSIONS { dimKey: { weight, label, category } }
 * @param {Record<string, any>} [history] - Per-dimension history { label: [{date, score}] }
 * @param {object} [opts]
...
export function dimensionBreakdown(scores, dimensions, history, opts) {
  ...
  for (let _a = 0, _b = Object.entries(dimensions); _a < _b.length; _a++) {
    ...
    if (history) {
      const points = history[cfg.label] || [];
      if (points.length >= 3) {
        const vals = points.map((/** @type {any} */ p) => p.score);
  ...
  breakdown.sort((/** @type {any} */ a, /** @type {any} */ b) => {
    /** @type {Record<string, number>} */
 ...
 *   summary: { summary: string, highlights: string[], risks: string[] },
...
export function generateScoreReport(opts) {
  ...
  const dimensions = opts.dimensions || {};
  /** @type {number[]} */
  const history = /** @type {any} */ (opts.history || []);
  /** @type {any} */
  ...
  const comparison = prevPeriod
    ? periodComparison(
        { composite, scores },
        { composite: prevPeriod.composite, scores: prevPeriod.scores || {} }
      )
    : null;
  ...
  const architecture = archResult
    ? {
        ...
        dims: (archResult.archDimResults || []).map((/** @type {any} */ d) => ({
          dim: d.dim || d.name,
  ...
  const summary = generateExecutiveSummary({
    ...
    scores,
    dimensions,
    trend: /** @type {any} */ (trend),
    prev: prevPeriod ? { composite: prevPeriod.composite, date: prevPeriod.date } : /** @type {any} */ (null),
    archResult,
    diagTriggered,
  ...
  if (dimHistory) {
    for (const [label, points] of Object.entries(dimHistory)) {
      dimScoreHistory[label] = points.map((/** @type {any} */ p) => p.score);
    }
  }
