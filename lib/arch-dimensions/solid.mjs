 ...
 * @returns {Record<string, any>|null} Parsed frontmatter object, or null if missing/invalid
 */
...
function hasConjunctions(/** @type {string} */ desc) {
...
export function checkYAGNI(projectRoot) {
  ...
  try {
    const files = readdirSync(libDir).filter((/** @type {string} */ f) =>
        ...
        if (callerFiles === 1) {
          ...
          if (singleCaller) {
            const otherExports = exports
              .map((/** @type {string} */ e) => e.replace(/export (?:const|function|let|var) /, ""))
              .filter((/** @type {string} */ n) => n !== name && !n.startsWith("_") && !/^[A-Z][A-Z_0-9]+$/.test(n) && !/^cmd[A-Z]/.test(n));
