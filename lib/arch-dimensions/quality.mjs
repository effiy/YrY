...
import { readFrontmatter } from "./solid.mjs";
...
/**
 * Check Interface Segregation Principle: each agent should have minimal tool set for its role.
 *
...
export function checkISP(projectRoot) {
  ...
  try {
    const agentFiles = readdirSync(agentsDir).filter((/** @type {string} */ f) => f.endsWith(".md") && f !== "AGENT.md");
    for (const file of agentFiles) {
      const agentName = file.replace(".md", "");
      const spec = (/** @type {any} */ (AGENT_TOOL_SPEC))[agentName];
      if (!spec) continue;
...
/**
 * Check that SKILL.md and agent .md files have required frontmatter (name/description/user_invocable/lifecycle).
 *
 * @param {string} projectRoot - Absolute path to project root
 * @returns {{ dim: string, label: string, pass: boolean, checks: Array<any> }}
 */
...
/**
 * Check document freshness: referenced files exist, no stale paths in CLAUDE.md/README.md.
 *
 * @param {string} projectRoot - Absolute path to project root
 * @returns {{ dim: string, label: string, pass: boolean, checks: Array<any> }}
 */
export function checkDocFreshness(projectRoot) {
  ...
  if (existsSync(skillsDir) && existsSync(agentsDir)) {
    try {
      const agentFiles = readdirSync(agentsDir).filter((/** @type {string} */ f) => f.endsWith(".md") && f !== "AGENT.md");
      const agentNames = agentFiles.map((/** @type {string} */ f) => f.replace(".md", ""));
      ...
      for (const d of skillDirs) {
        ...
        if (agentMatch) {
          agentMatch[1].split(",").forEach((/** @type {string} */ a) => {
        ...
        if (optMatch) {
          optMatch[1].split(",").forEach((/** @type {string} */ a) => {
            const name = a.trim().replace(/["']/g, "");
            if (name) declaredAgents.push(name);
          });
        }
        ...
        for (const ag of declaredAgents) {
          if (!agentNames.includes(ag)) {
            violations.push({
              skill: d.name,
              agent: ag,
              issue: `引用的 Agent "${ag}" 不存在于 skills/ 目录`,
            });
          }
        }
      }
