#!/usr/bin/env node
/**
 * branch-check — deterministic branch isolation enforcement.
 * CLI entry point: node lib/branch-check.mjs --story=<name> [--mode=write|read|init]
 * Modes: write (must be feat/<name>) | read (check only) | init (project bootstrap)
 */
...
// --- constants ----------------------------------------------------------------
import { NODE_ARGV_OFFSET, STORY_PANEL_DIR } from "./constants.mjs";
import { bold, dim, red, green, yellow } from "./tty.mjs";
import { findProjectRoot, writeJson } from "./fs.mjs";
...
function showHelp() {
  ...
  console.log("");
  console.log("用法: node lib/branch-check.mjs --story=<name> [--mode=<mode>]");
  console.log("");
  console.log("模式:");
...
function parseArgs() {
  ...
  /** @type {{ story: string, mode: string, [k: string]: string }} */
...
function currentBranch(/** @type {string} */ cwd) {
  return git("branch --show-current", cwd);
}

function branchExists(/** @type {string} */ name, /** @type {string} */ cwd) {
  const result = git(`rev-parse --verify ${name}`, cwd);
  return result !== null;
}

function branchFromMain(/** @type {string} */ name, /** @type {string} */ cwd) {
...
function existingFeatBranches(/** @type {string} */ cwd) {
  const result = git("branch --list 'feat/*'", cwd);
  if (!result) return [];
  return result.split("\n").map((/** @type {string} */ l) => l.replace(/^[* ] /, "").trim()).filter(Boolean);
}

// --- rui-state update ---------------------------------------------------------
function updateRuiState(/** @type {string} */ storyPath, /** @type {string} */ branch) {
...
function checkWriteMode(/** @type {string} */ projectRoot, /** @type {string} */ storyName) {
  ...
  if (branch !== featBranch) {
    ...
    if (branch === "main") {
      ...
      console.log(dim(`  或创建: node lib/branch-check.mjs --story=${storyName} --mode=write`));
      process.exit(1);
    }
  ...
  if (existsSync(rootStatePath)) {
    try {
      ...
      writeJson(rootStatePath, rootState);
...
function checkReadMode(/** @type {string} */ projectRoot, /** @type {string} */ storyName) {
  const featBranch = `${FEAT_PREFIX}${storyName}`;
  const curr = currentBranch(projectRoot);
...
function checkInitMode(/** @type {string} */ projectRoot, /** @type {string} */ storyName) {
  const curr = currentBranch(projectRoot);
  const featBranches = existingFeatBranches(projectRoot);
