/**
 * diagnostics.mjs — D0-D8 diagnostic engine (pure logic, no FS/I/O)
 *
...
import {
  ...
  DIAGNOSTIC_EARLY_WARNING_LEAD,
  DIAGNOSTIC_ESCALATION,
} from "../constants.mjs";

/**
 * Run D0-D8 diagnostic checks on execution memory data.
 *
 * @param {any} data - Aggregated execution memory data (allExec, blocked, p0, etc.)
 * @param {any[]} [docIssues=[]] - Document quality issues from doc-scan
 * @param {any} [archIssues=null] - Architecture check result, or null to skip
 * @returns {any[]} Diagnostic entries with id/triggered/evidence fields
 */
  ...
  if (execCount >= DIAGNOSTIC_MIN_CONFIDENCE.D0) {
    ...
    if (conflicts > 0) {
      diagnostics.push({
        ...
        suggestion: "核查执行记录中的阻断标记和阶段字段，确保与 CLAUDE.md / skills/ 基线（原 agents/ 已整合入 skills/）一致",
      });
    }
  }
...
function runD4Diagnostic(/** @type {any} */ data, /** @type {any[]} */ diagnostics) {
  if (data.statusHistory && data.statusHistory.length >= DIAGNOSTIC_MIN_CONFIDENCE.D4) {
    ...
    const deliveryFailures = data.deliveryTrack.filter((/** @type {any} */ r) => r.status === "failure").length;
...
function runD5Diagnostic(/** @type {any} */ data, /** @type {any[]} */ diagnostics) {
  if (data.toolAudit.length >= DIAGNOSTIC_MIN_CONFIDENCE.D5) {
    const toolErrors = data.toolAudit.filter((/** @type {any} */ r) => r.result === "failure").length;
...
function runD6Diagnostic(/** @type {any} */ data, /** @type {any[]} */ docIssues, /** @type {any[]} */ diagnostics) {
  const execCount = data.allExec.length;
...
function runD7Diagnostic(/** @type {any} */ data, /** @type {any[]} */ diagnostics) {
  ...
  if (closureRate < PROPOSAL_CLOSURE_MIN_RATE) {
    diagnostics.push({
      id: "D7", label: DIAGNOSTIC_LABELS.D7, triggered: true,
      confidence: data.proposals.length - closed,
      evidence: `提案闭合率 ${(closureRate * 100).toFixed(1)}% < ${(PROPOSAL_CLOSURE_MIN_RATE * 100).toFixed(0)}% (${closed}/${data.proposals.length})`,
...
function runD8Diagnostic(/** @type {any} */ data, /** @type {any} */ archIssues, /** @type {any[]} */ diagnostics) {
      ...
      suggestion: failedDims.includes("范式合规")
        ...
        : failedDims.includes("SRP 合规")
          ...
          : failedDims.includes("配置完整性")
            ...
            : `架构退化: ${failedDims.join(", ")}。审查 skills/rui/rules/architecture-principles.md 基线`,
    });
  }
}

// ── Trend-aware diagnostic helpers ──────────────────────────────────

/**
 * Detect linear trend in a metric history and predict when it will cross a threshold.
 */
/**
 * Compute trend direction and projection for a metric series.
 *
 * @param {number[]} metricHistory - Historical metric values (newest last)
 * @param {number} threshold - Alert threshold for projection
 * @returns {{ direction: string, slope: number, projected: number|null, willCrossIn: number|null }}
 */
export function computeDiagnosticTrend(metricHistory, threshold) {
  if (!metricHistory || metricHistory.length < 3) {
    return { direction: "stable", slope: 0, projected: null, willCrossIn: null };
  }

  const n = metricHistory.length;
  const xMean = (n - 1) / 2;
  const yMean = metricHistory.reduce((a, b) => a + b, 0) / n;

  let ssXY = 0, ssXX = 0;
  for (let i = 0; i < n; i++) {
    ssXY += (i - xMean) * (metricHistory[i] - yMean);
    ssXX += (i - xMean) ** 2;
  }

  const slope = ssXX !== 0 ? ssXY / ssXX : 0;
  const direction = Math.abs(slope) < 0.001 ? "stable" : slope > 0 ? "rising" : "falling";

  if (direction === "stable") {
    return { direction, slope: 0, projected: null, willCrossIn: null };
  }

  const lastValue = metricHistory[n - 1];
  const gap = threshold - lastValue;

  if ((slope > 0 && gap > 0) || (slope < 0 && gap < 0)) {
    const willCrossIn = Math.abs(gap / slope);
    return {
      direction,
      slope: Math.round(slope * 1000) / 1000,
      projected: Math.round((lastValue + slope * DIAGNOSTIC_EARLY_WARNING_LEAD) * 100) / 100,
      willCrossIn: Math.round(willCrossIn * 10) / 10,
    };
  }
...
/**
 * Compute diagnostic severity escalation based on consecutive triggers.
 */
/**
 * Compute severity score for a diagnostic based on consecutive occurrences in history.
 *
 * @param {string} diagId - Diagnostic identifier (e.g. "D3")
 * @param {any[]} history - Past diagnostic entries
 * @returns {{ severity: string, consecutiveCount: number }}
 */
export function computeDiagnosticSeverity(diagId, history) {
  let consecutiveCount = 0;

  for (let i = history.length - 1; i >= 0; i--) {
    const diags = history[i].triggeredDiags || [];
    if (diags.includes(diagId)) {
      consecutiveCount++;
    } else {
      break;
    }
  ...
  let severity = "info";
  if (consecutiveCount >= DIAGNOSTIC_ESCALATION.critical) {
    severity = "critical";
  } else if (consecutiveCount >= DIAGNOSTIC_ESCALATION.warning) {
    severity = "warning";
  }

  return { severity, consecutiveCount };
}

/**
 * Compute diagnostic co-occurrence matrix.
 */
/**
 * Compute co-occurrence correlation between diagnostic pairs in history.
 *
 * @param {any[]} history - Past diagnostic entries
 * @returns {any} Correlation matrix keyed by `D1+D2`
 */
export function computeDiagnosticCorrelation(history) {
  const allDiags = ["D0", "D1", "D2", "D3", "D4", "D5", "D6", "D7", "D8"];
  ...
  for (const diag of allDiags) {
    counts[diag] = 0;
    for (const d2 of allDiags) {
      if (diag !== d2) cooccurrences[`${diag}|${d2}`] = 0;
    }
  }

  for (const entry of history) {
    const triggered = entry.triggeredDiags || [];
    for (const d of triggered) {
      if (counts[d] !== undefined) counts[d]++;
    }
    for (let i = 0; i < triggered.length; i++) {
      for (let j = i + 1; j < triggered.length; j++) {
        const key1 = `${triggered[i]}|${triggered[j]}`;
        const key2 = `${triggered[j]}|${triggered[i]}`;
        if (cooccurrences[key1] !== undefined) cooccurrences[key1]++;
        if (cooccurrences[key2] !== undefined) cooccurrences[key2]++;
      }
    }
  }
  ...
  for (const diag of allDiags) {
    for (const d2 of allDiags) {
      if (diag >= d2) continue;
      const cooc = cooccurrences[`${diag}|${d2}`] || 0;
      if (cooc === 0) continue;
      pairs.push({
        diag1: diag,
        diag2: d2,
        cooccurrence: cooc,
        total,
        rate: Math.round((cooc / total) * 100),
      });
    }
  }

  pairs.sort((a, b) => b.cooccurrence - a.cooccurrence);

  const clusters = [];
  const visited = new Set();
  for (const pair of pairs) {
    if (pair.rate < 30) continue;
    let found = false;
    for (const cluster of clusters) {
      if (cluster.includes(pair.diag1) || cluster.includes(pair.diag2)) {
        if (!cluster.includes(pair.diag1)) cluster.push(pair.diag1);
        if (!cluster.includes(pair.diag2)) cluster.push(pair.diag2);
        visited.add(pair.diag1);
        visited.add(pair.diag2);
        found = true;
        break;
      }
    }
    if (!found) {
      clusters.push([pair.diag1, pair.diag2]);
      visited.add(pair.diag1);
      visited.add(pair.diag2);
    }
  }
