...
function deriveStoryDirName(/** @type {any} */ proposal, /** @type {string} */ projectRoot) {
...
function createRuiState(/** @type {string} */ storyPath, /** @type {any} */ proposal) {
...
function buildBaselineDocContent(/** @type {any} */ proposal, /** @type {string} */ _projectName) {
  const diagId = proposal.id.match(/D\d+/i)?.[0] || "DIAG";
  const diagLabel = (/** @type {any} */ (DIAGNOSTIC_LABELS))[diagId] || "自改进诊断";
  const diagBaseline = (/** @type {any} */ (DIAGNOSTIC_BASELINES))[diagId] || "—";
...
function generateBaselineDoc(/** @type {any} */ proposal, /** @type {string} */ projectName, /** @type {string} */ storyDirName) {
...
function materializeProposal(/** @type {any} */ proposal, /** @type {string} */ projectRoot, /** @type {string} */ projectName, /** @type {boolean} */ dryRun) {
  ...
  updateJsonlById(join(proposalsDir, PROPOSALS_FILE), proposal.id, (/** @type {any} */ r) => {
...
/**
 * CLI entry for materializing proposals into story directories under docs/故事任务面板/.
 *
 * @param {any} opts - CLI options (proposal id filter, dry-run, etc.)
 * @returns {void}
 */
export function cmdMaterialize(opts) {
  ...
  const minPriorityOrder = (/** @type {any} */ (PRIORITY_ORDER))[minPriority] !== undefined ? (/** @type {any} */ (PRIORITY_ORDER))[minPriority] : (/** @type {any} */ (PRIORITY_ORDER)).P2;
  ...
  const candidates = allProposals.filter((p) => {
    ...
    const pOrder = (/** @type {any} */ (PRIORITY_ORDER))[p.priority];
