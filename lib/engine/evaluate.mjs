...
/**
 * Evaluate a proposal by comparing pre/post metrics (E1-E4 evaluation).
 *
 * @param {any} proposal - Proposal entry with id/action/target
 * @param {any} preMetrics - Metrics snapshot before implementation
 * @param {any} postMetrics - Metrics snapshot after implementation
 * @returns {object} Evaluation result with improved/degraded flags and severity
 */
...
/**
 * Aggregate metrics from execution memory entries.
 *
 * @param {Array<any>} memories - Execution memory records
 * @returns {{ count: number, block_rate: number, p0_density: number, t3_proportion: number, agent_participation: Record<string, number> }}
 */
export function computeMetrics(memories) {
  let blockedCount = 0;
  let totalP0 = 0, totalIssues = 0;
  let t3Count = 0;
  const agentCounts = /** @type {Record<string, number>} */ ({});
...
/**
 * CLI entry for evaluating proposals: scan proposals.jsonl, pair with pre/post metrics, emit evaluation report.
 *
 * @param {any} opts - CLI options (since, limit, etc.)
 * @returns {void}
 */
export function cmdEvaluate(opts) {
  ...
  for (const sd of storyDirs) {
    const proposalsPath = join(sd.path, PROPOSALS_FILE);
  ...
  const proposalsPath = join(targetStoryPath, PROPOSALS_FILE);
