...
// ── API reachability health scoring (layered) ──────────────────────
// Three-layer scoring so environmental unreachability does not tank the score
// while configuration / historical delivery signals still surface real issues.

export const API_PROBE_TIMEOUT_MS = 3_000;
export const API_HEALTH_L1_CONFIG_MAX = 40;
export const API_HEALTH_L2_PROBE_MAX = 30;
export const API_HEALTH_L3_HISTORY_MAX = 30;
export const API_HEALTH_PROBE_TIMEOUT_SCORE = 15;
export const API_HEALTH_L1_BOTH = 40;
export const API_HEALTH_L1_ONE = 25;
export const API_HEALTH_L1_NONE = 0;
export const API_HEALTH_L2_OK = 30;
export const API_HEALTH_L2_TIMEOUT = 15;
export const API_HEALTH_L2_ERROR = 0;
export const API_HEALTH_L3_FULL = 30;
export const API_HEALTH_L3_HIGH = 24;
export const API_HEALTH_L3_MID = 12;
export const API_HEALTH_L3_LOW = 6;
export const API_HEALTH_INDIRECT_EVIDENCE_MIN_SUCCESSES = 1;
...
export const STORY_PANEL_DIR = "cdn";
...
// ── Commit granularity guard (D3 prevention) ──────────────────────
// Aligns with bot-health-diagnostics.mjs largeCommits proxy (>N files changed)
export const LARGE_COMMIT_FILE_THRESHOLD = 15;
export const COMMIT_MESSAGE_MIN_LENGTH = 10;

// ── Dependency analysis entry-point patterns ──────────────────────
// Files matching these patterns are CLI tools or framework-loaded configs,
// not modules imported by other code. They are legitimate entry points,
// not orphans.
export const ENTRY_POINT_PATTERNS = [
  /^.*\/scripts\/[^/]+\.mjs$/,  // CLI tools in any scripts/ directory
  /^[^/]+\.config\.mjs$/,        // Framework configs at repo root (e.g. vitest.config.mjs)
];
...
/** Penalty factor for volatile dimensions (0-1, higher = more penalty) */
export const VOLATILITY_PENALTY_FACTOR = 0.3;
/** Minimum history entries required for volatility adjustment */
export const VOLATILITY_MIN_HISTORY = 3;

/** Confidence discount applied to composite score based on reliability */
export const CONFIDENCE_DISCOUNT = {
  high: 0,       // reliability >= 0.8: no discount
  medium: 0.03,  // reliability 0.5-0.8: 3% discount
  low: 0.08,     // reliability < 0.5: 8% discount
};
export const CONFIDENCE_RELIABILITY_HIGH = 0.8;
export const CONFIDENCE_RELIABILITY_MEDIUM = 0.5;

// ── Diagnostic trend awareness ───────────────────────────────────────

/** Window size for diagnostic trend detection */
export const DIAGNOSTIC_TREND_WINDOW = 5;
/** Warn if threshold will be crossed within this many periods */
export const DIAGNOSTIC_EARLY_WARNING_LEAD = 3;

/** Diagnostic escalation: consecutive triggers to escalate severity */
export const DIAGNOSTIC_ESCALATION = {
  warning: 2,    // escalate to warning after 2 consecutive triggers
  critical: 3,   // escalate to critical after 3 consecutive triggers
};

// ── Cross-dimension correlation ──────────────────────────────────────

/** Minimum overlapping data points required for correlation analysis */
export const CORRELATION_MIN_OVERLAP = 5;
/** Pearson r threshold for strong correlation */
export const CORRELATION_STRONG = 0.7;
/** Pearson r threshold for moderate correlation */
export const CORRELATION_MODERATE = 0.4;

// ── Score stabilization ──────────────────────────────────────────────

/** Window size for stabilization analysis */
export const STABILIZATION_WINDOW = 7;
/** CV threshold below which a score is considered stable */
export const STABILIZATION_CV_THRESHOLD = 0.05;

// ── Improvement potential ────────────────────────────────────────────

/** Default number of quick wins to return */
export const IMPROVEMENT_QUICK_WIN_COUNT = 3;
/** Volatility weight in effort estimation (0-1) */
export const IMPROVEMENT_EFFORT_FACTOR = 0.3;

// ── Diagnostic → dimension mapping ───────────────────────────────────

/** Maps each diagnostic (D0-D8) to the health dimensions it affects */
export const DIAGNOSTIC_DIMENSION_MAP = {
  D0: ["token", "config", "git"],
  D1: ["em_cicd", "reports"],
  D2: ["em_testing", "em_linting", "comp_qual"],
  D3: ["file_size", "dep_analysis"],
  D4: ["git", "em_git"],
  D5: ["dep_analysis", "em_deps"],
  D6: ["em_docs"],
  D7: ["config", "robots"],
  D8: ["file_size", "dep_analysis", "em_docs"],
};
 ...
 *   Core (8 dims, Σ=68) — operational health, most critical for system function
 ...
 *   Total weight pool = 148, normalized per dim as weight/Σweights
 *
...
export const HEALTH_SCORING_DIMENSIONS = {
  ...
  api:          { label: "API 可达性",    weight: 8,  icon: "🌐", category: "core" },
  reports:      { label: "自循环报告",    weight: 8,  icon: "📊", category: "core" },
  format:       { label: "消息格式合规",  weight: 8,  icon: "📋", category: "core" },
  diagnostics:  { label: "D0-D8 诊断",   weight: 9,  icon: "🔬", category: "core" },
 ...
 * @param {{ decliningStreak?: number }} [opts] - { decliningStreak }
 * @returns {{ level: string, label: string, icon: string, color: string, ansi: string }}
 */
export function getSLOStatus(composite, opts = {}) {
  ...
  if ((opts.decliningStreak ?? 0) >= HEALTH_SLO.maxDecliningStreak && level === "ok") {
    level = "warning";
  }

  /** @type {keyof typeof levels} */
  const levelKey = /** @type {keyof typeof levels} */ (level);
  return { level, ...levels[levelKey] };
}
