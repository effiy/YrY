...
function computeGrade(/** @type {number} */ failedDimCount) {
...
function computeScore(/** @type {number} */ passed, /** @type {number} */ total) {
  if (total === 0) return 100;
  return Math.round((passed / total) * 100);
}

function computeSummary(/** @type {any[]} */ dimensions) {
  const totalChecks = dimensions.reduce((/** @type {number} */ s, /** @type {any} */ d) => s + d.checks.length, 0);
  const passedChecks = dimensions.reduce(
    (/** @type {number} */ s, /** @type {any} */ d) => s + d.checks.filter((/** @type {any} */ c) => c.pass).length, 0
  );
  const failedDims = dimensions.filter((/** @type {any} */ d) => !d.pass);

  return {
    totalChecks,
    passedChecks,
    failedChecks: totalChecks - passedChecks,
    failedDimensions: failedDims.map((/** @type {any} */ d) => d.label),
