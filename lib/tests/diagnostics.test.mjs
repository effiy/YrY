...
function makeData(allExec, extra = {}) {
  return {
    allExec,
    statusHistory: [],
    deliveryTrack: [],
...
describe('engine/diagnostics.mjs', () => {
  describe('runDiagnostics()', () => {
    ...
    it('D0: 阻断但无原因的记录触发基线偏离', async () => {
      ...
      const d0 = result.find(d => d.id === 'D0');
    ...
    it('D3: T3 占比超过阈值时触发', async () => {
      ...
      const result = runDiagnostics(makeData(allExec));
      const d3 = result.find(d => d.id === 'D3');
      if (d3) {
    ...
    it('正常数据不触发任何诊断', async () => {
      ...
      const result = runDiagnostics(makeData(allExec));
      const triggered = result.filter(d => d.triggered);
      assert.ok(triggered.length === 0, `expected 0 triggered, got ${triggered.length}: ${triggered.map(d => d.id).join(',')}`);
    });
    ...
    it('D4: 交付失败达到阈值时触发', async () => {
      ...
      const result = runDiagnostics(data);
      const d4 = result.find(d => d.id === 'D4');
      if (d4) {
    ...
    it('D5: 工具调用失败率超过阈值时触发', async () => {
      ...
      const result = runDiagnostics(data);
      const d5 = result.find(d => d.id === 'D5');
      if (d5) {
    ...
    it('D6: 文档问题时触发', async () => {
      ...
      const d6 = result.find(d => d.id === 'D6');
    ...
    it('D7: 提案闭合率低于阈值时触发', async () => {
      ...
      const result = runDiagnostics(data);
      const d7 = result.find(d => d.id === 'D7');
      if (d7) {
    ...
    it('D8: 架构合规问题不触发 (无 archIssues)', async () => {
      ...
      const result = runDiagnostics(data);
      const d8 = result.find(d => d.id === 'D8');
