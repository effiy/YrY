...
import { findProjectRoot, isMain, writeJson } from "./fs.mjs";
import {
  detectTrend,
  forecastScore,
  scoreReliability,
...
export function computeCdnScoreReport(projectRoot) {
  ...
  // ── Composite history ──────────────────────────────────────────
  const history = scoreTrend.map((/** @type {{ score?: number }} */ e) => e.score).filter((/** @type {unknown} */ s) => typeof s === "number");
  ...
  const dimScores = {};
  /** @type {Record<string, { label: string, weight: number, category: string }>} */
  const dimCfgs = {};
  ...
  for (const d of dimSummary) {
    ...
    const dd = d;
    const key = dd.label;
    dimScores[key] = dd.avgScore || 0;
    dimCfgs[key] = {
      label: dd.label,
      weight: dd.entries || 1,
      category: ["CSS 组件覆盖", "双主题完备性", "JS API 一致性"].includes(dd.label)
        ? "quality" : ["加载链稳定", "页面迁移率", "字体性能"].includes(dd.label)
        ? "structural" : "engineering",
    };
    // Per-dimension history from scoreTrend (simplified: use trend value)
    dimHistories[key] = scoreTrend.map((/** @type {{ date?: string }} */ e) => ({
      date: e.date,
  ...
  const diagTriggered = diagSummary.filter((/** @type {{ count?: number }} */ d) => (d.count ?? 0) > 0).length;
  ...
  const scoreReport = {
    ...
    diagnostics: {
      ...
      topDiags: diagSummary.filter((/** @type {{ count?: number }} */ d) => (d.count ?? 0) > 0).sort((/** @type {{ rate: number }} */ a, /** @type {{ rate: number }} */ b) => b.rate - a.rate).slice(0, 3),
    },
  };
...
if (_isMain) {
  ...
  /** @type {any} */
  const enrichedAny = enriched;
  const sr = enrichedAny.scoreReport;
