...
export function generateSummary(projectRoot) {
  ...
  if (composites.length >= 3) {
    ...
    // Period comparison (latest week vs previous week, if available)
    if (weekly.length >= 2) {
      ...
      if (currWeek && prevWeek && currWeek.dimAvgs && prevWeek.dimAvgs) {
        ...
        scoreReport.comparison = {
          ...
          topChanges: comparison.dimDeltas.slice(0, 5).map(d => ({
            dim: d.dim,
    ...
    // Dimension breakdown with recommendations
    const dimHistory = {};
    for (const d of dimSummary) {
  ...
  if (daily.length >= 2) {
    const velocityScores = daily.map(d => d.avgScore);
    const velTrend = detectTrend(velocityScores);
    const velDist = scoreDistribution(velocityScores);

    // Calculate improvement rate: % of days with positive change
    let positiveDays = 0;
    let totalChanges = 0;
    ...
    // Dimension-level improvement velocity
    const dimVelocity = {};
    for (const d of dimSummary) {
      if (d.entries >= 3) {
    ...
    // Bottleneck analysis: dimensions that drag down composite the most
  ...
  if (latest.scores) {
    ...
    // API risk: API unreachable
    const apiScore = latest.scores.api || 100;
    if (apiScore < 60) riskFactors.push({ factor: "API 不可达", weight: 0.25, impact: Math.round((60 - apiScore) * 0.4) });
  ...
  if (composites.length >= 3 && dimSummary.length >= 1) {
    ...
    const dimWeights = {};
    const dimHist = {};
    const totalWeight = Object.values(HEALTH_SCORING_DIMENSIONS).reduce((s, d) => s + d.weight, 0);

    for (const d of dimSummary) {
      dimScores[d.dim] = d.avgScore;
      dimWeights[d.dim] = (HEALTH_SCORING_DIMENSIONS[d.dim]?.weight || 8) / totalWeight;
  ...
  if (dimSummary.length >= 1) {
    ...
    for (const d of dimSummary) {
      ...
      // Map to related diagnostics (reverse lookup: dimension → diagnostics)
      const allDiags = Object.entries(DIAGNOSTIC_DIMENSION_MAP);
      for (const [diagId, affectedDims] of allDiags) {
        if (affectedDims.includes(d.dim)) {
          const diagInfo = diagSummary.find((ds) => ds.id === diagId);
      ...
      // Find correlated dimensions from crossDimension
      if (summary.crossDimension && summary.crossDimension.pairs) {
        const correlated = summary.crossDimension.pairs
          .filter((p) => (p.dim1 === d.label || p.dim2 === d.label) && Math.abs(p.r) >= 0.4)
          .map((p) => ({
