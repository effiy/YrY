 ...
 * @returns {any[]} Parsed records (empty if file missing or invalid)
 */
 ...
 * @returns {any|null} Parsed JSON, or null if file missing or invalid
 */
...
export function updateJsonlById(filePath, id, updater) {
  ...
  try {
    ...
    const updated = lines.map((/** @type {string} */ line) => {
 ...
 * @param {number} [indent=2] - Indentation level for pretty-printing
 */
export function writeJson(path, obj, indent = 2) {
...
export function findStoryDirs(projectRoot) {
  ...
  try {
    return readdirSync(panelDir, { withFileTypes: true })
      .filter((/** @type {{ isDirectory: () => boolean, name: string }} */ d) => d.isDirectory() && !d.name.startsWith("."))
      .map((/** @type {{ isDirectory: () => boolean, name: string }} */ d) => ({ name: d.name, path: join(panelDir, d.name) }));
  } catch {
    return [];
  }
}
