 ...
 * @param {object & { headers?: object, method?: string, body?: string }} [options] - fetch options (method, body, etc.)
 ...
 * @returns {Promise<any[]>} Session records (may be empty)
 */
export async function querySessionsFull(apiUrl, apiToken) {
  ...
  /** @type {{ data?: { list?: any[] }, list?: any[] }} */
  const d = data;
  return d.data?.list || d.list || [];
}
 ...
 * @param {string} apiToken - Auth token
 * @returns {Promise<object|string>} API response payload
 */
...
export function findPluginHelpPath(skillName) {
  ...
  try {
    const versions = readdirSync(pluginRoot).filter((/** @type {string} */ d) => /^\d+\.\d+\.\d+$/.test(d)).sort();
