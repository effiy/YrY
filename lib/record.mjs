...
import { findProjectRoot, findStoryDirs, readJsonl, readJson, nowISO, isMain, writeJson } from "./fs.mjs";
...
function parseArgs() {
  ...
  const cmd = args[0];
  /** @type {{ command: string, story: string, all: boolean, window: number, blocked: boolean, [k: string]: string|boolean|number }} */
  const opts = { command: cmd, story: "", all: false, window: 12, blocked: false };
...
function ensureMemoryDir(/** @type {string} */ storyPath) {
...
function appendJsonl(/** @type {string} */ path, /** @type {object} */ obj) {
 ...
 * @param {{ story: string, status?: string, step?: string, session?: string, message?: string, 'duration-ms'?: string }} opts
 */
 ...
 * @param {{ story: string, tool?: string, result?: string, 'duration-ms'?: string, session?: string, error?: string }} opts
 */
 ...
 * @param {{ story: string, stage?: string, blocked?: boolean|string, reason?: string } & Record<string, unknown>} opts
 */
...
function cmdBootstrap(opts) {
  ...
  for (const story of stories) {
    ...
    const improveDir = join(story.path, PROPOSALS_DIR);
    if (!existsSync(improveDir)) mkdirSync(improveDir, { recursive: true });
 ...
 * @param {{ story?: string, window?: string | number } & Record<string, unknown>} opts
 */
function cmdCompress(opts) {
  ...
  for (const r of recent) {
    /** @type {any} */
    const rr = r;
    if (rr.was_blocked) blockedCount++;
    const qi = rr.quality_issues || {};
    totalP0 += (qi.P0 || []).length;
    totalIssues += (qi.P0 || []).length + (qi.P1 || []).length + (qi.P2 || []).length;
    for (const a of (rr.agents_called || [])) {
      agentCounts[a] = (agentCounts[a] || 0) + 1;
    }
    const transitions = rr.phase_transitions || [];
  ...
  for (const [key, durs] of Object.entries(stageDurations)) {
    if (durs.length > 0) {
      stageAvgs[key] = Math.round(durs.reduce((/** @type {number} */ a, /** @type {number} */ b) => a + b, 0) / durs.length);
    }
  }
  ...
  writeJson(targetPath, compressed);
