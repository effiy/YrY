...
flowchart TB
    subgraph 内核["内核 Kernel — 不可妥协"]
        ...
        K4["核心规则<br/>管线·设计原则·安全基线"]:::core
    end

    subgraph 扩展["扩展 Extensions — 丰富可替换"]
        direction TB
        E1["skills/ 技能<br/>19 个自包含单元<br/>含 Agent 角色 + 规则"]:::ext
        E2["docs/ 面板与报告<br/>健康·趋势·仪表板"]:::ext
    end
...
| **扩展** | skills/（自包含：含 Agent 角色 + 规则）· docs/面板 | 走 rui 管线即可 | 新增/删除扩展不修改内核任一文件 |
...
| `agents.required` | `string[]` | 否 | 每个值必须是 skills/ 目录下存在的 agent 角色名 |
...
每个 Agent 角色定义位于其所属 skill 目录内（如 `skills/rui/pm.md`），通过 **YAML frontmatter** 声明其配置：

```yaml
# skills/<skill>/<agent-role>.md 的 frontmatter 规范
---
...
outputs:                     # 必填 — 产出物类型
  - <output-type>
contract: <skill-path>       # 必填 — 交接契约所在 skill
---
```
...
| `contract` | `string` | ✅ | 指向 skill 目录下的交接契约文件 |

### Rule 配置 API

每个 Rule 通过 `skills/<skill>/rules/<name>.md` 的 **YAML frontmatter** 声明其适用范围：

```yaml
# skills/<skill>/rules/<name>.md 的 frontmatter 规范
---
...
> 新增能力不得修改内核。所有扩展通过 skills/ 目录自包含注册。
...
flowchart TB
    subgraph 入口["扩展入口"]
        ...
        AG["新增 Agent<br/>skills/&lt;skill&gt;/&lt;role&gt;.md"]:::ext
        RL["新增 Rule<br/>skills/&lt;skill&gt;/rules/&lt;name&gt;.md"]:::ext
    end
...
| 步骤 | 动作 | 验证 |
|------|------|------|
| 1 | 在目标 skill 目录创建 `skills/<skill>/<role>.md` — 含完整 frontmatter | `tools` 最小集，`contract` 指向存在的文件 |
| 2 | 更新 skill 内 AGENT.md 角色拓扑图 + 表格 | 新 agent 出现在拓扑图中 |
| 3 | 更新 skill 内 AGENT.md 管线参与表 | 新 agent 的参与阶段已标注 |
...
| 步骤 | 动作 | 验证 |
|------|------|------|
| 1 | 在目标 skill 目录创建 `skills/<skill>/rules/<name>.md` — 含 `paths` frontmatter | paths 覆盖目标文件范围 |
...
| **DIP** | `grep -r "import.*from.*skills/" skills/` — skill 间不应直接 import（通过 lib/ 共享） | > 0 |
...
| D5 依赖退化 | skill 间出现直接 import | 重构为通过 rui 编排器路由 | skills/ |
...
| # | 检查项 | 通过条件 | 违反时 |
|---|--------|---------|--------|
| A1 | 内核不变 | 新增/修改只涉及 skills/ docs/，未改 lib/ 或编排器 | 启动 ADR |
