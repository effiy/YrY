...
> YrY 项目 Mermaid 图统一配色系统。源自 beautiful-mermaid 的两色基础设计——所有视觉元素从 bg + fg 两个颜色通过 `color-mix()` 派生。本文件是配色唯一真相源（single source of truth），所有 `skills/`、`docs/` 下的 mermaid 图必须遵循此色板。
