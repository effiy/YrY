...
| 覆盖维度 | 最少用例数 | 说明 | 示例 |
|---------|:---:|------|------|
| **正常路径** | ≥ 2 | 标准输入的标准输出 | 有效用户名+密码 → 登录成功 |
| **异常路径** | ≥ 2 | 无效输入的错误处理 | 错误密码 → 登录失败 + 错误提示 |
| **边界条件** | ≥ 2 | 极值/空值/零值 | 空用户名、超长密码、并发登录 |
| **安全路径** | ≥ 1 | 注入/越权/绕过 | SQL 注入、未授权访问 |
| **回归验证** | ≥ 1 | 已有功能不受影响 | 修改登录后，注册功能仍正常 |

### 覆盖完整性判定

```
每 FP# 覆盖完整性:
  ├─ 正常 + 异常 + 边界 + 安全 + 回归 全覆盖 → ✅ 完整
  ├─ 正常 + 异常 覆盖但边界/安全缺失 → ⚠️ 不足，退回补用例
  ├─ 仅正常路径覆盖 → ❌ 严重不足，Gate A 阻断
  └─ 无测试用例 → 🚫 skip-gate-a 阻断
```
...
TC-N-01: 正常登录 — 有效凭据返回 token
TC-E-01: 异常登录 — 错误密码返回 401
TC-B-01: 边界登录 — 空用户名返回校验错误
TC-S-01: 安全登录 — SQL 注入输入被校验拒绝
```
