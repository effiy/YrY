...
补充文档按需触发，决策树见 [rules/doc-generation-lifecycle.md](../rui-html/rules/doc-generation-lifecycle.md#supplementary)，公式见 [formulas/supplement.md](./formulas/supplement.md)。
...
> coder 在提交代码前自审查的快速参考。逐模块 P0 清零的核心工具。

### 实现正确性

| # | 检查项 | 验证方式 |
|---|--------|---------|
| 1 | 所有 AC 已实现 | 逐条 AC 对照，每 AC 有对应代码 |
| 2 | 边界条件已处理 | 空值/零值/最大值/并发 |
| 3 | 错误路径已处理 | 网络失败/超时/权限不足/资源不存在 |
| 4 | 影响链已闭合 | 二级传递 Grep 已做，引用点已更新 |

### 代码质量

| # | 检查项 | 验证命令 |
|---|--------|---------|
| 5 | 无 class/extends | `grep -rn "class\|extends"` 返回空 |
| 6 | 无 export default | `grep -rn "export default"` 返回空 |
| 7 | 无空 catch | `grep -rn "catch\s*{\s*}"` 返回空 |
| 8 | 无魔法数字 | 仅 0/1/-1 豁免 |
| 9 | 无跨 Skill 导入 | `grep -rn "from.*skills/"` 返回空 |

### 安全合规

| # | 检查项 | 验证方式 |
|---|--------|---------|
| 10 | 用户输入已校验 | 每个输入点有校验函数 |
| 11 | 密钥不落盘 | 无硬编码 token/key |
| 12 | 第三方 CDN 有 SRI | integrity 属性存在 |
| 13 | SQL 使用参数化查询 | 无字符串拼接 SQL |

### 测试覆盖

| # | 检查项 | 验证方式 |
|---|--------|---------|
| 14 | 正常路径有测试 | 每函数至少 1 条正常用例 |
| 15 | 异常路径有测试 | 每函数至少 1 条异常用例 |
| 16 | 边界条件有测试 | 空值/零值/极限值 |
| 17 | 测试通过 | `npm test` 0 failures |
