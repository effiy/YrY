...
|------|:---:|:---:|:---:|:---:|--------|------|
| 故事任务.md | ✓ | ✓ | ✓ | ✓ | pm | 文档生成 |
| 后端技术评审.md | | — | ✓ | ✓ | coder + security | 文档生成（架构设计后） |
| 前端技术评审.md | | ✓ | — | ✓ | coder | 文档生成（架构设计后） |
| 测试用例评审.md | ✓ | ✓ | ✓ | ✓ | tester | 文档生成（架构设计后） |
| 后端实施报告.md | | — | ✓ | ✓ | coder | 验证 |
| 前端实施报告.md | | ✓ | — | ✓ | coder | 验证 |
| 测试用例报告.md | ✓ | ✓ | ✓ | ✓ | tester | 验证 |
| 自改进复盘.md | ✓ | ✓ | ✓ | ✓ | pm + reporter | 自改进 |
| 消息通知列表.md | 自动 | ✓ | ✓ | ✓ | wework-notify hook | 交付 |
...
> **关键约束**：故事任务文档是唯一真相源，所有引用最终追溯于此；技术评审在文档生成阶段创建，实施与测试报告在验证阶段创建——不可提前；附属目录由脚本管理，人工不编辑。
...
```markdown
详见 [UserLogin 接口契约](./故事任务.md#接口契约)。
```
