...
- [故事目录骨架](#故事目录骨架) · 八份主线（01–08）+ 补充（10–19）+ 附属
- [补充文档决策](#补充文档决策) · 何时写补充文档（编号 10–19）
- [参考文档骨架](#参考文档骨架) · 索引 + 主体章节（01–04）的统一结构
- [文档级导航与跨引用](#文档级导航与跨引用)
- [阅读路径](#阅读路径)
- [文件创建生命周期](#文件创建生命周期)
- [完整度判定](#完整度判定)
...
```mermaid
flowchart LR
    Req[需求] --> Story["故事级执行<br/>八份主线 01–08"]:::story
    Story -->|引用| Ref["项目级参考<br/>索引 + 主体 01–04"]:::ref
    Ref -.体现到.-> Story
...
```
docs/
├── 故事任务面板/<Project>/<name>/   ← 执行：八份主线（01–08）+ 通知（00-消息通知列表）+ 补充（10–19）
├── 组件文档/<Project>/<component>/  ← 参考：索引 + 主体章节（01-组件概述 / 02-状态与依赖 / 03-样式与交互 / 04-操作场景）
├── 接口文档/<Project>/<resource>/   ← 参考：索引 + 主体章节（01-接口概述 / 02-数据模型 / 03-中间件与安全 / 04-操作场景）
├── 页面文档/<Project>/<page>/       ← 参考：索引 + 主体章节（01-页面概述 / 02-组件编排 / 03-交互流程 / 04-操作场景）
└── 领域模型/<Project>/<domain>/     ← 参考：索引 + 主体章节（01-领域概述 / 02-实体模型 / 03-领域服务 / 04-操作场景）
```
...
> **关键约束**：故事任务（01-故事任务.md）是唯一真相源，所有引用最终追溯于此；技术评审（02/03/04）在文档生成阶段创建，实施与测试报告（05/06/07）在验证阶段创建——不可提前；附属目录由脚本管理，人工不编辑。
...
| 其他专题 | ad-hoc | `1x-{专题}.md`（编号 17–19） | pm 决策 | F.supp 自定义 |

## 参考文档骨架

四类参考文档统一结构：索引（00-索引.md，导航入口）+ 主体章节（编号 01–04，按递进展开）。每份文件的「一句话定位」按递进：

| 文件 | 定位 | 各文档类对应章节标题 |
|------|------|------------------|
| 00-索引.md | 阅读入口（导航） | 索引 |
| 01-\*.md | API 参考手册（查阅型） | 组件概述 / 接口概述 / 页面概述 / 领域概述 |
| 02-\*.md | 架构蓝图 / 数据字典（理解型） | 状态与依赖 / 数据模型 / 组件编排 / 实体模型 |
| 03-\*.md | 视觉规范 / 安全白皮书（审查型） | 样式与交互 / 中间件与安全 / 交互流程 / 领域服务 |
| 04-\*.md | 用户手册 / 集成手册 / 操作手册（验证型） | 操作场景 |
...
主体章节（01–04）首尾包含标准导航块；索引（00-索引.md）是入口，不含导航：
...
故事文档引用参考文档时，优先指向索引（00-索引.md），按需深入具体章节：
...
每个索引（00-索引.md）提供按角色和场景的推荐路径。常见路径示例（节点编号简称对应「索引 / 主体 §1~§4」）：

| 类型 | 角色 | 路径 | 时间 |
|------|------|------|-----|
| 组件 | 调用方 | 索引 → 01 §2 接口契约 → 04 正常场景 | 5 min |
| 组件 | 维护者 | 索引 → 01 → 02 → 03 | 15 min |
| 组件 | 测试者 | 索引 → 04 全场景 → 01 §2 必填可选 | 8 min |
| 接口 | 前端 | 索引 → 01 §2 端点清单 → 04 调用场景 | 5 min |
| 接口 | SRE | 索引 → 03 §6 错误码 → 04 §5 性能约束 | 5 min |
| 页面 | 前端 | 索引 → 01 §1 → 02 §1 组件树 → 02 §3 通信 | 10 min |
| 领域 | 后端 | 索引 → 01 §2 限界上下文 → 03 §2 领域事件 | 10 min |
...
参考文档完整度（同样由 `list.js` 判定）。判定基于文件**职责**而非裸编号：
