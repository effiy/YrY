...
> **口诀：执行管做事、参考管现状、阶段定时序。** 故事级回答「做什么 / 怎么做 / 做了什么」，项目级回答「当前是什么」。文件按管线阶段创建，不可提前。

哲学源自 [CLAUDE.md](../../CLAUDE.md)。本文件只回答「哪个文件 / 谁产 / 何时产 / 谁读」；章节字段在 [formulas.md](./formulas.md)，强制约束在 [rules/doc-generation.md](../../rules/doc-generation.md)。
