...
| # | 输入类型 | 搜索模式 | 示例 |
|---|---------|---------|------|
| 1 | HTTP 请求参数 | `req.body`, `req.query`, `req.params` | 表单提交、URL 参数 |
| 2 | 文件上传 | `multer`, `formidable`, `input type=file` | 头像上传、文档导入 |
| 3 | 用户输入渲染 | `innerHTML`, `dangerouslySetInnerHTML`, `v-html` | 富文本、评论 |
| 4 | URL/路径参数 | `:id`, `:slug`, `useParams()` | 资源 ID、页面路由 |
| 5 | 第三方 API 响应 | `fetch`, `axios`, `http.request` | 外部数据源 |
| 6 | WebSocket 消息 | `ws.on`, `socket.on` | 实时消息 |
| 7 | 环境变量/配置 | `process.env`, `config.get` | 运行时配置 |
...
| 认证绕过 | 高 | 低 | **High** | P0 |
| 权限提升 | 高 | 低 | **High** | P0 |
| XSS 注入 | 中 | 中 | **Medium** | P1 |
| 信息泄露（错误消息） | 低 | 高 | **Medium** | P1 |
| DoS 资源耗尽 | 中 | 低 | **Low** | P2 |
...
| 优先级 | 措施类型 | 示例 |
|:---:|------|------|
| P0 | 输入校验 + 参数化 | SQL 注入防护、XSS 过滤 |
| P0 | 认证 + 授权 | JWT 验证、RBAC 权限 |
| P0 | 密钥管理 | 环境变量、密钥轮换 |
| P1 | 安全头 | CSP、HSTS、X-Frame-Options |
| P1 | 速率限制 | API 限流、登录尝试限制 |
| P2 | 审计日志 | 操作记录、异常告警 |
