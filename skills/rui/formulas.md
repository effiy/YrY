...
flowchart TB
    ...
    subgraph 主线["故事主线（编号文档 + 必选）"]
        subgraph 生成["文档生成阶段"]
            ...
            F03["F.story.backend-review<br/>03-{project}-后端技术评审"]
            F04["F.story.frontend-review<br/>04-{project}-前端技术评审"]
            F05["F.story.test-review<br/>05-测试用例评审"]
        end
        subgraph 验证["验证阶段"]
            F06["F.story.backend-report<br/>06-{project}-后端实施报告"]
            F07["F.story.frontend-report<br/>07-{project}-前端实施报告"]
...
flowchart LR
    subgraph 基线["基线（问题+用户空间）"]
        01["01-故事任务<br/>WHAT & WHY"]
        02["02-用户使用场景<br/>WHO & HOW"]
    end
    01 --> 02
    02 --> 03["03-{project}-后端技术评审"]
    02 --> 04["04-{project}-前端技术评审"]
    04 --> 05["05-测试用例评审"]
    03 --> 05
    05 --> 06["06-{project}-后端实施报告"]
    05 --> 07["07-{project}-前端实施报告"]
    06 --> 08["08-测试用例报告"]
    07 --> 08
    08 --> 09["09-自改进复盘"]
    09 --> 00["00-消息通知列表"]
...
| 场景 | 前驱 `←` | 后继 `→` |
|------|----------|---------|
| 基线 01-故事任务 | 省略 | 02-用户使用场景 |
| 基线 02-用户使用场景 | 01-故事任务 | 03(后端) 或 04(前端)，按项目类型 |
| 链中 | 上一编号文件 | 下一编号文件 |
| 链尾（00） | 上一编号文件 | 省略 |
...
> 01-故事任务 与 02-用户使用场景 是整个故事目录的**双基线文档**——问题空间与用户空间的定义。所有下游文档（03-09 及补充文档）均为解决方案/验证/改进空间，必须显式溯源至基线。
...
|------|------|---------|---------|---------|
| **问题空间** | 01-故事任务 | WHAT（做什么）WHY（为什么做） | 禁止代码路径、API 路由、组件名、数据库表名、技术栈名 | 基线，不溯源 |
| **用户空间** | 02-用户使用场景 | WHO（谁使用）HOW（如何体验） | 禁止技术术语、组件名、API 端点、文件路径、数据库概念、框架名 | 溯源至 01 |
...
| §3 场景覆盖矩阵 | pm | `场景 \| FP# \| AC# \| 实现文档(03/04) \| 测试文档(05) \| 覆盖状态 \| 备注`，与 01-故事任务 §2 FP# 及 §5 AC# 对齐 | 必填 |
...
> 以下两节原属 01-故事任务 §3 和 §4，移至此处以保持 01 为纯问题空间基线。
...
| §5 性能与限制 | `维度 \| 约束 \| 应对` |
| §6 评审清单 | 权限最小化 / 通信对齐 / 存储兼容 / API 鉴权 / 无硬编码密钥 / 无误用长连接 / 输入校验完整 / 基线溯源完备 |
...
| 子章节 | 负责人 | 表头/字段 | 约束 |
|--------|--------|----------|------|
| §0.0 基线溯源 | coder | `本设计章节 \| 实现 01-故事任务 \| 服务 02-用户使用场景 \| 覆盖状态`。每个技术章节必须声明其满足的 01 需求(FP#/SC#/Story#)与服务 02 场景 | 必填，≤ 1 个未覆盖项且标注原因 |
| §0.1 设计决策 | coder + security | `决策领域 \| 选定方案 \| 选择理由 \| 详见 \| 实现 01 FP#`。每个决策必须标注其满足的 01 功能需求 | 必填 |
| §0.2 任务规划 | coder | `ID \| 描述 \| 工作量(S/M/L) \| 依赖 \| 交付物 \| Agent \| 门禁 \| 交接下游 \| 实现 01 FP#` + 任务依赖 mermaid。每个任务必须标注其实现的功能点 | 必填 |

```mermaid
flowchart LR
    S0["§0 设计决策+任务规划"] --> S1["§1 组件架构"]
    S1 --> S2["§2 状态管理"]
    S2 --> S3["§3 交互设计"]
    S3 --> S4["§4 样式方案"]
    S4 --> S5["§5 DOM 与事件"]
...
| §7 评审清单 | 组件命名空间 / 资源注册 / 状态约定 / 样式隔离 / 事件清理 / 加载顺序 / 模块语法 / 样式文件注册 / 基线溯源完备 |
...
| §5 性能观察 | `维度 \| 观察 \| 与评审预期` |
| §6 评审清单 | 文件与任务对应 / 接口与评审一致 / 偏差有因有据 / P0 清零 / 存储已验证 / 性能可观察 / 基线溯源闭合 |
...
| 章节 | 表头/内容 |
|------|----------|
| §0 基线溯源与校准 | `基线文件 \| 关键条款 \| 本次执行适用性 \| 偏差` 覆盖：01-故事任务(问题空间) / 02-用户使用场景(用户空间) / CLAUDE.md / 5 rules / 3 agents。追加表：`01 成功标准 SC# \| 目标值 \| 最终达成 \| 未达原因 \| 后续动作` | 必填 |
...
| 效果示意 | **01-故事任务必须含 `### 效果示意` 节**：mermaid flowchart 展示当前痛点→目标状态→关键里程碑。节点 ≥ 5，classDef 区分状态。位于需求概述之后、主要价值之前。其余文档按需添加 | ✅ |
...
| 校验 | F.meta/F.nav 占位符必须替换；表列齐备；mermaid 可渲染；主要价值 ≥ 4 条；效果示意完整；回溯链闭合；基线声明(01/02)；基线溯源(03-09)；禁止内容扫描(01/02) | ✅ |
...
| 2 | `### 效果示意` 存在（01-故事任务必含） | grep + mermaid 渲染 |
| 3 | 来源引用存在且为相对链接 | grep 链接格式 |
| 4 | 变更记录表存在且 ≥ 1 行 | grep 表头 |
| 5 | F.meta 无 `{...}` 占位符 | grep 残留 |
| 6 | 所有交叉引用可点击（相对路径有效） | 逐个 verify |
| 7 | 01-故事任务 §0 基线声明存在 | grep "问题空间基线" |
| 8 | 02-用户使用场景 §0 基线声明存在 | grep "用户空间基线" |
