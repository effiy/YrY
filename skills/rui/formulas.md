...
| 可追溯 | {project}-测试设计必须覆盖本文档 §2 的每个场景及其异常分支 |
...
| 效果示意 | **{project}-故事任务必须含 `### 效果示意` 节**：mermaid flowchart 展示当前痛点→目标状态→关键里程碑。节点 ≥ 5，classDef 区分状态。位于需求概述之后、主要价值之前。03/04 技术评审必须含效果示意 mermaid 图 | ✅ |
...
| 7 | {project}-故事任务 §0 基线声明存在 | grep "问题空间基线" |
| 8 | {project}-使用场景 §0 基线声明存在 | grep "用户空间基线" |
| 9 | 故事任务不含禁止内容（文件路径/API路由/组件名/技术栈名） | 扫描 `/api/`、`/src/`、`<.*>` 组件标签、`Redis\|PostgreSQL\|Express` 等技术栈名 |
| 10 | 使用场景不含禁止内容（技术术语/组件名/API端点） | 扫描技术名词（emit、store、API等）、组件标签、URL 路径 |
| 11 | 技术评审/测试设计/安全审计/实施报告/测试报告/自改进复盘的 §0 基线溯源存在，表头完整，≥ 3 行映射 | grep "基线溯源"；检查行数 ≥ 表头+3 |
| 12 | 技术评审含效果示意 mermaid 图（§1 系统架构内） | grep "效果示意" + mermaid 渲染 |
| 13 | 安全审计含威胁建模（§2）且 STRIDE 六类全覆盖 | grep "STRIDE"；检查 §2 表格 |
