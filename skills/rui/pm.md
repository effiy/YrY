...
> 数据采集由 `node lib/recommend.mjs` 完成，评分由 PM 按 [ranking.md](./ranking.md) 的 5 层框架执行。

| 项目类型 | 扫描命令 | 排序依据 | 命名格式 |
|---------|---------|---------|---------|
| 前端 | `node lib/recommend.mjs --root . --type frontend` | [5层评分](./ranking.md) → P0→P3 | `<project>-<component>-doc` |
...
| 4 | 目录命名见 [doc-generation.md](../rui-html/rules/doc-generation.md) | 自创目录结构 |
...
> 计划执行规则详见 [plan-execution.md](../rui-plan/rules/plan-execution.md)。planner agent 规约见 [planner.md](../rui-plan/planner.md)。

## 规划深度准则

> pm 定义「做什么」的深度标准，不替代 planner 做「怎么做」。详细规划执行见 [planner.md](../rui-plan/planner.md) 和 [rules/plan-execution.md](../rui-plan/rules/plan-execution.md)。
...
| 维度 | 权重 | 评估标准 | 评分 |
|------|:---:|---------|:---:|
| **业务价值** | 30% | 对用户/业务的核心价值 | 1-5 |
| **技术风险** | 25% | 技术复杂度、依赖不确定性 | 1-5 (风险越高分越高) |
| **依赖关系** | 20% | 被其他故事依赖的程度 | 1-5 (被依赖越多分越高) |
| **变更范围** | 15% | 涉及的模块/文件数量 | 1-5 (范围越大分越高) |
| **紧急程度** | 10% | 时间敏感性 | 1-5 |

### 优先级计算公式

```
优先级分 = 业务价值 × 0.3 + 技术风险 × 0.25 + 依赖关系 × 0.2 + 变更范围 × 0.15 + 紧急程度 × 0.1

P0: 优先级分 ≥ 4.0 或 含安全/合规需求
P1: 优先级分 ≥ 3.0
P2: 优先级分 ≥ 2.0
P3: 优先级分 < 2.0
```
...
| 反模式 | 表现 | 纠正 |
|--------|------|------|
| **过细拆分** | 每个函数一个故事 | 按用户可见功能聚合 |
| **过粗拆分** | 一个故事含 10+ FP# | 按业务领域拆分 |
| **技术拆分** | 按技术层拆分（API层/数据层/UI层） | 按用户价值垂直拆分 |
| **依赖隐藏** | 故事间有隐式依赖但未标注 | 显式标注依赖关系 |
| **AC 模糊** | AC 含"应该/可能/大概" | AC 必须是可客观验证的断言 |
