...
import {
  REPORT_DIR, CDN_DEPTH, DIM_LABELS, DIM_WEIGHTS,
...
export function generateHealthReport(/** @type {any} */ hr) {
  ...
  /** @type {Record<string, string>} */ const lowDimRecs = {
  ...
  const trendScores = healthTrend.map((/** @type {any} */ e) => e.composite).filter((/** @type {any} */ s) => typeof s === "number");
  /** @type {Record<string, any[]>} */ const dimHistoryForReport = {};
  ...
  /** @type {Record<string, number>} */ const prevScores = {};
  ...
  /** @type {Record<string, string>} */ const DIM_ICONS_LOCAL = {
  ...
  function buildDimCard(/** @type {string} */ dim, /** @type {string} */ label) {
  ...
  if (hasEmDims) {
    emCardsHtml = `
    <div class="h-section">
      <h2>🏗️ 工程化成熟度 <span class="h-section-sub-inline">rui-init §7</span></h2>
  ...
  if (hr.diagnostics && !hr.diagnostics.skip) {
    ...
    const currentTriggeredIds = new Set((hr.diagnostics.triggered || []).map((/** @type {any} */ d) => d.id));
    const diagRows = allDiags.map((/** @type {any} */ d) => {
    ...
    const passedCount = allDiags.filter((/** @type {any} */ d) => !d.triggered).length;
  ...
  const detailItems = (hr.details || []).map((/** @type {any} */ d) => {
...
<div class="h-hero">
  <div class="h-score-ring grade-${hr.grade}">
    <div class="h-score-num">${hr.composite}</div>
    <div class="h-score-grade">${hr.grade} 级</div>
  ...
  ${gradeSparkline ? `<div class="h-sparkline-wrap">${gradeSparkline}</div>` : ""}
</div>
...
<div class="h-panel on" id="overview">
  ${buildExecutiveSummaryHTML(generateExecutiveSummary({
    ...
    dimensions: HEALTH_SCORING_DIMENSIONS,
    prev: prev ? { composite: prev.score, date: prev.date } : undefined,
    archResult: hr.archResult ? { archFailedDims: hr.archResult.archFailedDims } : undefined,
    diagTriggered: hr.diagnostics?.triggered?.length || 0,
...
document.querySelectorAll('.h-tab').forEach(function(t) {
  ...
  const lines = html.replace(/<!--.*?-->/gs, '').split('\n').map(l => l.trim()).filter(l => l);
  /** @type {string[]} */ const merged = [];
  for (const line of lines) {
    const prevLine = merged[merged.length - 1];
    if (prevLine && line.length < 80 && prevLine.length < 120 && (prevLine + line).length < 200) {
      merged[merged.length - 1] = prevLine + line;
    } else {
      merged.push(line);
    }
  }
 ...
 * Now imported from report-sections (was previously a local helper).
 */

/**
 * Health-report-summary — generateHealthIndex creates the index HTML
 * page that links to all generated date-stamped reports.
 */
...
<div class="yry-stats" id="stats">
  ...
  <div class="yry-stat"><div class="yry-val pass">${latestReport && latestReport.score !== null && latestReport.score >= 80 ? latestReport.score : '—'}</div><div class="yry-lbl">最新评分</div></div>
...
<div class="ft">
  健康报告索引<br>
  <span class="h-meta-tag">由 rui-bot health-report 自动生成 · 9 核心 + 7 工程维度 · D0-D8 诊断</span>
</div>
