...
| # | 规则 | 反例 |
|---|------|------|
| 6 | 诊断以基线文件为判定基准（CLAUDE.md / `skills/`） | 凭经验判断"复杂度太高" |
...
| # | 信号 | 假设 | 置信度 | 基线依据 |
|---|------|------|--------|---------|
| **D0** | 执行与基线冲突 | 哲学偏离 | ≥1 条记忆 | CLAUDE.md · skills/rui/AGENT.md |
| **D1** | 阻断率 > 20% | 预处理不充分 | ≥5 条记忆 | code-pipeline.md |
| **D2** | P0 密度 > 均值 2× | 设计遗漏 | ≥3 条记忆 | doc-generation.md |
| **D3** | T3 占比 > 30% | 需求边界模糊 | ≥3 条记忆 | skills/rui/pm.md（故事拆分） |
| **D4** | Gate B > 2 轮 | 测试先行不足 | Gate B 计数 | code-pipeline.md |
| **D5** | 阶段耗时 > 均值 3× | Agent 协作瓶颈 | ≥3 条记忆 | skills/rui/AGENT.md |
