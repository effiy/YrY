...
> 每条诊断必须引用基线文件作为依据。详见 [skills/rui-yry/rules/self-improve.md](../skills/rui-yry/rules/self-improve.md)。
...
| 故事诊断 | per-story 子流程 | 单故事全量数据 | D0–D7 诊断表（场景文档 §4 自改进 §2） |
...
> 持续自改进闭环。Agent 可按间隔周期性执行 D0-D7 诊断 → 提案 → 评估 → 经验技能化。
...
| 终止条件 | 连续 2 次无新提案生成 |
| 迭代动作 | 全量数据采集 → D0-D7 诊断 → 生成提案 → 候选评估 → 升级检测 |
| 收敛判定 | 无新增诊断告警 + 无待评估提案 |
