...
> 采数据（采），按 D0-D8 出诊断（断），每诊断写一条提案（出）。无证据不出，无评估不闭合。
>
> D5 依赖退化诊断时可结合趋势数据评估外部参考新鲜度（趋势数据由 rui-trends skill 提供）。

[四段闭环](#四段闭环) · [触发](#触发) · [观察：数据源](#观察数据源) · [诊断：D0-D8](#诊断d0-d8) · [改进：提案矩阵 + 经验技能化](#改进提案矩阵--经验技能化) · [规则](#规则) · [操作](#操作) · [生效标志](#生效标志)
...
rui 自改进阶段（代码管线完成后）。每个故事走完 Gate B 自动进入本阶段，产出场景文档 §4 自改进并向 `proposals.jsonl` 追加诊断结果。
...
> D5 依赖退化诊断时，应通过 `/rui-trends` 查询当前技术趋势，验证外部参考的时效性。结果写入场景文档 §4 自改进 §2.1 技术趋势验证。
...
|------|---------|---------|---------|---------|
| `process` | 阻断率上升 / 阶段耗时 > 基线 2x | 调整 {阶段} 流程 | 连续 3 故事触发 | `skills/rui-code/rules/code-pipeline.md` 或 `skills/rui/AGENT.md` |
| `quality` | P0 密度上升 / Gate B > 2 轮 | 强化 {阶段} 审查 | 连续 3 故事触发 | `skills/rui/tester.md` 或 `skills/rui/coder.md` |
| `refactor` | 文件 > 500 行 / 循环依赖 > 3 | 拆分 {模块} | 连续 3 故事触发 | `skills/rui-code/rules/code-pipeline.md` §深度模块 |
| `security` | 安全边界模糊 / 威胁未缓解 | 加固 {边界} | 当前故事即修 | `skills/rui/security.md` P0 约束 |
| `skill` | 重复操作模式 / Agent 反复犯同类错误 | 创建 {skill/rule} | 连续 2 故事触发 | `skills/` 新条目 |
...
| 操作 | 触发方式 | 输入 | 输出 |
|------|---------|------|------|
| 架构反思 | self-improve 阶段 snapshot 子流程 | 代码快照 + Git diff | 复杂度热点报告（写入场景文档 §4 自改进 §2 诊断） |
| 工流趋势 | retro 子流程（默认窗口 8 周） | `execution-memory.jsonl` × N | 趋势图 + 异常点（场景文档 §4 自改进 §1.3） |
| 故事诊断 | per-story 子流程 | 单故事全量数据 | D0-D8 诊断表（场景文档 §4 自改进 §2） |
| 效果评估 | evaluate 子流程 | `proposals.jsonl` | 闭合率 + E1-E4（场景文档 §4 自改进 §3.3） |
| 回顾报告 | rui 自改进阶段 | 故事面板目录 | 场景文档 §4 自改进 |

> 所有子流程均为本规约约束的逻辑步骤，由 rui 管线自改进阶段直接执行，不依赖外部脚本。

## Red Flags — 暂停并回到 Iron Law

- "这个指标看起来改善了，不用算 E1-E4 了"
- "数据不够也能出提案，反正观察类不阻断"
- "压缩窗口调大一点数据更好看"
- "这次诊断结果和前几次一样，直接复用上次的提案"
- "没有 regression 就是改善"
- "记忆窗口太窄，但就用这些数据吧"
- "提案闭合率低是因为故事太少，不是提案质量差"
...
| 合理化借口 | 实际情况 |
|-----------|---------|
| "数据不足也能出观察" | 观察记录 ≠ 评估结论，不可闭合提案 |
| "指标差不多就是改善了" | E1-E4 量化比较，不可凭感觉 |
| "这个诊断不重要，跳过" | D0-D8 全覆盖，不可选择性跳过 |
| "上次也这样，说明是正常波动" | 每次都是独立评估，历史不背书 |
| "提案太多闭合不了，降低标准" | 闭合率低 = 提案质量或粒度问题 |
...
> 持续自改进闭环。Agent 可按间隔周期性执行 D0-D8 诊断 → 提案 → 评估 → 经验技能化。

| 属性 | 值 |
|------|-----|
| 推荐间隔 | `0 9 * * 1`（每周一早 9 点，完整周期） |
| 触发条件 | 有 ≥ 3 个故事积累了新的执行记忆 |
| 终止条件 | 连续 2 次无新提案生成 |
| 迭代动作 | 全量数据采集 → D0-D8 诊断 → 生成提案 → 候选评估 → 升级检测 |
| 收敛判定 | 无新增诊断告警 + 无待评估提案 |
