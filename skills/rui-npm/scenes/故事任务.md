...
> | v5.4.0 | 2026-06-06 | deepseek-v4-pro | 🌿 feat/rui-npm | 📎 [CLAUDE.md](../../../CLAUDE.md) |
> **故事交付物**: [🔗 知识图谱](知识图谱.html) · 各场景 7 件交付物见场景 index.md

[概述](#概述) · [§1 Story](#s-1-story) · [§2 Requirements](#s-2-requirements) · [§3 成功标准](#s-3-成功标准) · [§4 范围边界](#s-4-范围边界) · [§5 AC](#s-5-ac) · [§6 风险与假设](#s-6-风险与假设) · [§7 跨文档索引](#s-7-跨文档索引) · [§R 关联故事](#s-r-关联故事)

<a id="概述"></a>
## 概述

用户频繁需要与 npm registry 交互——搜索包、安装依赖、查看信息、升级版本、卸载清理。同时，一次性脚本希望能快速发布为 npm 包后通过 npx 直接使用，而非走正式的 publish 流程。已发布的包还需要账号级管理——查看自己发布了哪些包、废弃旧版本、删除误发布。

当前这些操作依赖手动 `npm` 命令，需要记忆繁琐的参数组合，缺乏统一入口。本故事的目标是为 YrY 新增 `rui-npm` 技能，将包搜索安装、本地发布、账号管理整合为 14 个子命令的统一入口，让用户在对话中自然完成所有 npm 操作。
...
flowchart TD
    ...
    NOW --> ENTRY["/rui-npm 统一入口\n14 子命令 · 5 场景覆盖"]:::entry

    ENTRY --> S1["Story 1 搜索与发现\nsearch · cdn\n关键词搜索 → 表格输出\nCDN 三源引用地址"]:::story
    ENTRY --> S2["Story 2 安装与版本\ninstall · update · list\npackage.json ↔ node_modules\n依赖树/平面列表"]:::story
    ENTRY --> S3["Story 3 发布与执行\npublish · npx\n本地文件 → npm 包\n不安装直接运行"]:::story
    ENTRY --> S4["Story 4 信息与卸载\ninfo · audit · uninstall\n元数据查询 · 漏洞审计\n依赖清理"]:::story
    ENTRY --> S5["Story 5 账号管理\nmy-packages · deprecate · unpublish\n认证 → 所有权验证\n个人资产统一管理"]:::story

    S1 --> REG["npm registry"]:::ext
    S2 --> REG
    S3 --> REG
    S4 --> REG
    S5 --> REG
...
作为 开发者，我想要 在对话中用自然语言搜索 npm registry 中的包并获取其 CDN 引用地址，以便 快速发现需要的包获取关键信息（描述/版本/下载量/更新时间），并获知如何在浏览器中通过 CDN 直接引用。

优先级 **P0**。范围边界：只读 npm registry，不修改本地文件。不涉及私有 registry 配置和 CDN 缓存策略。依赖：npm CLI 可用，网络可达 registry.npmjs.org。
...
作为 开发者，我想要 安装 npm 包并管理其版本（安装/更新/切换版本），以便 项目依赖保持最新且可控。

优先级 **P0**。范围边界：修改当前目录下的 package.json 和 node_modules。不涉及全局包管理和 monorepo 工作空间。依赖：npm CLI 可用，当前目录存在 package.json。
...
作为 开发者，我想要 将本地文件或目录快速发布为 npm 包，然后直接用包名引用或通过 npx 执行，以便 一次性脚本和小工具无需走正式的 npm publish 流程即可复用。

优先级 **P0**。范围边界：本地文件系统 + npm registry。不涉及 scope 包自动创建和私有 registry 发布。依赖：npm CLI 可用，已登录 npm（`npm whoami`）。
...
作为 开发者，我想要 查看已安装或 registry 中包的详细信息以及卸载不再需要的包，以便 审计依赖健康状况，保持项目整洁。

优先级 **P0**。范围边界：npm registry 查询 + 本地 node_modules。依赖安全漏洞自动扫描可后续扩展 `audit` 子命令。依赖：npm CLI 可用。
...
作为 npm 包维护者，我想要 查看我发布的所有 npm 包、废弃不再维护的版本、以及删除误发布的包或版本，以便 统一管理自己在 npm registry 上的个人资产，保持包列表健康整洁。

优先级 **P0**。范围边界：npm registry 账号级操作（需认证）。不涉及 npm 组织管理、包所有权转移和访问权限管理。依赖：npm CLI 可用，已登录 npm（`npm whoami`），网络可达 registry.npmjs.org。
...
**P0 — 包搜索与发现 (Story 1)**
- **FP1 包搜索** — 按关键词搜索 npm registry。输入：关键词字符串。输出：结构化搜索结果表（名称/描述/版本/下载量/更新日期）。降级：无网络时提示。
- **FP10 帮助输出** — 展示完整命令用法和场景示例。输入：--help/-h。输出：命令全景 + 参数说明 + 使用场景。

**P0 — 包安装与版本管理 (Story 2)**
- **FP2 包安装** — 安装指定包到当前项目。输入：包名 + 可选版本。输出：安装确认 + 版本信息。阻断：package.json 不存在时提示。
- **FP3 包更新** — 更新指定包到兼容最新版。输入：包名。输出：旧版本→新版本变更摘要。阻断：包未安装时提示。
- **FP6 包卸载** — 从当前项目移除指定包。输入：包名。输出：卸载确认。阻断：包未安装时提示。

**P0 — 本地发布与 npx (Story 3)**
- **FP7 本地发布** — 将本地文件或目录发布为 npm 包。输入：文件路径或目录路径。输出：包名+版本+发布确认。阻断：npm 未登录时提示。
- **FP8 npx 执行** — 通过 npx 直接运行 npm 包。输入：包名。输出：执行结果（stdout/stderr）。降级：包不存在或执行失败时展示错误。

**P0 — 包信息审计 (Story 4)**
- **FP5 包信息** — 查看指定包的完整元数据。输入：包名。输出：版本/依赖/许可证/维护者/GitHub 链接。降级：包不存在时提示。

**P0 — 账号级包管理 (Story 5)**
- **FP12 我的包列表** — 列出当前认证用户拥有的所有 npm 包。输入：无（需认证）。输出：包列表表格。阻断：未认证时提示 login。
- **FP13 包废弃** — 标记指定包或版本为 deprecated。输入：包名 + 可选版本 + 废弃消息。输出：废弃确认。阻断：未认证或非包所有者。
- **FP14 包删除** — 从 registry 删除指定包或版本。输入：包名 + 可选版本。输出：删除确认（含安全警告前置）。阻断：未认证/非包所有者/超 72h 无 --force。

**P1**
- **FP4 包列表** — 列出当前项目已安装的包。输入：可选 depth 参数。输出：依赖树或平面列表。阻断：package.json 不存在时提示。
- **FP9 依赖审计** — 检查已安装依赖的安全漏洞。输入：无。输出：漏洞摘要（按严重级别分组）。降级：无网络时提示。
- **FP11 CDN 引用** — 查看 npm 包在主流 CDN 的引用地址。输入：包名 + 可选版本。输出：unpkg/jsDelivr/esm.sh 三条 URL。降级：包不存在或无网络时提示。
...
**P0**
- **SC1** 用户可用一条命令搜索 npm registry。度量：`/rui-npm search <kw>` 执行时间。目标：< 5s 返回结构化结果。→ FP1
- **SC2** 用户可在对话中完成包的完整生命周期管理。度量：连续执行 install → list → info → update → uninstall。目标：每步成功。→ FP2–FP6
- **SC3** 用户可一键发布本地文件并在同会话中 npx 使用。度量：publish → npx 执行成功。目标：全流程通过。→ FP7,FP8
- **SC4** help 输出覆盖全部子命令和场景示例。度量：`node skills/rui-npm/help.mjs` 输出。目标：完整帮助。→ FP10
- **SC5** 所有错误场景有明确提示和恢复建议。度量：模拟 8 种错误场景。目标：100% 覆盖。→ FP1–FP14
- **SC7** 用户可列出自己发布的所有 npm 包。度量：`/rui-npm my-packages` 执行时间。目标：< 5s 返回结构化列表。→ FP12
- **SC8** 用户可废弃自己发布的包版本。度量：`/rui-npm deprecate <pkg> "<msg>"` 执行。目标：版本标记为 deprecated。→ FP13
- **SC9** 用户可删除误发布的包或版本。度量：`/rui-npm unpublish <pkg>@<version>` 含安全警告。目标：版本从 registry 删除。→ FP14

**P1**
- **SC6** 用户可用一条命令获取包的 CDN 引用地址。度量：`/rui-npm cdn react` 执行时间。目标：< 3s 返回三路 CDN URL。→ FP11
...
**范围内**
- npm 包搜索 + CDN 引用 (FP1,FP11) — 关键词搜索结果表格化；CDN 三源引用地址
- 包安装/更新/卸载 (FP2,FP3,FP6) — 操作当前项目依赖
- 包信息查询 (FP5) — registry 元数据查询
- 已安装列表 (FP4) — 依赖树展示
- 本地文件/目录发布 (FP7) — 自动处理 package.json，一键 publish
- npx 远程执行 (FP8) — 不安装直接运行
- 安全审计 (FP9) — npm audit 封装
- 帮助与文档 (FP10) — SKILL.md + help.mjs
- 账号级包管理 (FP12,FP13,FP14) — my-packages · deprecate · unpublish，认证 + 所有权验证

**范围外**
- 私有 registry 配置 — 属项目级 npm 配置。替代：手动 `.npmrc` 配置
- 全局包管理 — 初始版本聚焦项目级。替代：后续版本扩展 `--global`
- monorepo 工作空间 — 复杂度高，非核心场景。替代：手动 npm workspace 命令
- scope 包注册 — 需 npm 组织配置。替代：手动 npm 官网操作
- 包所有权转移 — 属组织级管理。替代：手动 `npm owner add/rm`
...
**高影响风险**
- **registry 不可达** (L·H → FP1,FP5) — npm registry 不可达导致 search/info 失败。缓解：捕获网络错误，输出友好提示和手动访问 URL。
- **误执行 unpublish 不可逆** (M·H → FP14) — 用户误删包无法恢复。缓解：删除前展示安全警告 + 待删除详情，要求显式确认，提示 72h 内可恢复。

**中影响风险**
- **npm 未登录** (M·M → FP7) — publish 前未检测登录状态导致失败。缓解：publish 前检查 `npm whoami`，未登录时引导 `npm login`。
- **包名冲突** (M·M → FP7) — 本地文件名与已有 npm 包冲突。缓解：publish 前检查 registry 是否存在同名包，提示改名。
- **registry API 返回不完整** (M·M → FP12) — maintainer 过滤结果不准确。缓解：优先使用 `npm access ls-packages` 补充校验，API 不可用时降级提示。

**低影响风险**
- **CDN URL 格式变更** (L·L → FP11) — unpkg/jsDelivr/esm.sh URL 格式变化。缓解：URL 为字符串模板，格式变更时只需更新模板。
- **npm CLI 版本过旧** (L·L → FP1–FP14) — 版本 < 7.0.0 导致参数不兼容。缓解：入口检查版本，不可用时提示升级。
- **包所有权转移缓存过期** (L·L → FP13,FP14) — 本地缓存未更新。缓解：执行前实时 `npm view` 验证当前所有者。

**假设**
- 用户能在对话中自然表达 npm 操作意图 → 命令名设计语义化，贴近自然语言 (FP1–FP14)
- npm CLI 在用户环境中可用且版本兼容 → 入口检查 `npm --version`，不可用时明确提示安装 (FP1–FP14)
...
| 测试套件 | [rui-npm.test.mjs](../tests/rui-npm.test.mjs) | 基线完整性测试 |
...
```mermaid
flowchart LR
    NPM["本故事\nrui-npm\nnpm 包管理技能"]:::here --> ARCH["yry-arch\n架构基线\n技能纳入模块拓扑"]:::story
    NPM --> SELF["yry-self-test\n自主测试方案\n纳入自检覆盖范围"]:::story
...
>   - 文档公式：[F.story.task](../../../skills/rui/formulas.md#fstorytask--故事任务-meta--storyn) — 故事任务公式
