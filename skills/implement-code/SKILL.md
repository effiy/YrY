...
**写代码前必须完成真实场景最小可用测试（Gate A）：** 在开始任何代码实现前，必须基于 `02_需求任务.md` 的主场景，在真实页面/真实入口完成一次最小可用（MVP）流程验证并留存证据（命令、日志、截图或录屏路径）；未通过或无证据时，禁止进入代码实施阶段。**细则与证据标准**：[`rules/implement-code-testing.md`](./rules/implement-code-testing.md) §2。
