...
${hdr("快速入门")}
${item("/import-docs workspace=true", "项目根全量扫描 + 上传（最常用）", cyan)}
${item("/import-docs", "状态检测：token / 远端可达性 / 文件差异 → 推荐任务", cyan)}
...
${item("/import-docs workspace=true", "扫描项目根全部 .md + .claude/ → 上传", cyan)}
${scene("从远端拉取故事文档")}
${item("/import-docs dir=docs/故事任务面板/user-login/ mode=pull", "远端 → 本地覆盖", cyan)}
${scene("从远端拉取 .claude/ 配置")}
${item("/import-docs dir=.claude/ mode=pull", "远端 → 本地覆盖 .claude/ 全量", cyan)}
${scene("同步指定目录 + 多文件类型")}
${item("/import-docs dir=/path/to/docs exts=md,json,yaml", "仅该目录下的 md/json/yaml", cyan)}
${scene("全量 + 排除临时目录")}
${item("/import-docs workspace=true exclude=tmp,vendor", "跳过 tmp/ 和 vendor/", cyan)}
${scene("预览不上传")}
${item("/import-docs workspace=true mode=list", "列出待上传文件清单", cyan)}
${scene("带远端前缀上传")}
${item("/import-docs workspace=true prefix=docs,api-v2", "远端路径前追加 docs/api-v2/", cyan)}
${scene("按文件名关键词精确拉取")}
${item("/import-docs dir=docs/故事任务面板/user-login/ mode=pull names=技术评审,安全审计", "仅拉取名中含「技术评审」或「安全审计」的文件", cyan)}
${scene("覆盖远端 API 地址")}
${item("/import-docs workspace=true apiUrl=https://staging-api.example.com", "切换 API 目标（如预发验证）", cyan)}
${scene("先预览再同步（安全工作流）")}
${item("/import-docs workspace=true mode=list", "Step 1：预览待上传文件清单", cyan)}
${item("/import-docs workspace=true", "Step 2：确认无误后实际同步", cyan)}
${scene("仅同步 .claude/ 配置到远端")}
${item("/import-docs dir=.claude/", "扫描 .claude/ 全量 → 上传到远端", cyan)}
${scene("拉取单个故事的全部文档")}
${item("/import-docs dir=docs/故事任务面板/user-login/ mode=pull", "从远端拉取整个故事目录覆盖本地", cyan)}
`;
