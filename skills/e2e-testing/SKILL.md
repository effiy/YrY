...
- **编码前（Gate A）**：交付物必须支持真实入口上的主路径 MVP。代码实现开始前，测试方案和验收标准必须就绪。
- **编码后（Gate B）**：用例骨架必须可由 AI 自动化执行；若场景缺少充分断言，写"前置信息不足：…"而非编造通过条件。
- Gate A/B 定义见 [`agents/AGENT.md`](../../agents/AGENT.md)。
