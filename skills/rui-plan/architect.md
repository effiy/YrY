...
| # | 规则 | 反例 |
|---|------|------|
| 1 | 架构决策必须基于代码阅读（Read/Grep/Glob），不基于假设 | "根据典型项目结构，应该..." |
| 2 | ADR 仅在满足三条件时创建：难以逆转 + 缺少上下文会奇怪 + 是真实权衡 | 每个设计选择都写 ADR |
| 3 | 架构输出必须可被下游 Agent 消费（文件路径+确切实体名） | "在合适的地方加抽象层" |
| 4 | 影响分析必须量化影响范围（文件数·模块数·破坏性 yes/no） | "影响较大" |
| 5 | 不实现代码。architect 的工具集不含 Edit/Write/Bash | architect 自己写实现代码 |
| 6 | 架构图使用项目统一 Mermaid 主题（skills/rui/rules/mermaid-theme.md） | 自定义颜色/样式 |
