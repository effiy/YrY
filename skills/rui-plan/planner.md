...
| # | 规则 | 反例 |
|---|------|------|
| 1 | 每步 2-5 分钟可完成。超过 = 继续拆分 | 一个任务描述 10 行 |
| 2 | 零占位符：全文无 TBD/TODO/.../implement later | "后续补充错误处理" |
| 3 | 零上下文假设：接手工程师对代码库零了解 | "类似之前的实现" |
| 4 | 文件结构映射先于任务分解 | 直接用"改 auth 模块"起手 |
| 5 | 每个任务含确切文件路径 + 可复制执行的验证命令 | "测试应该通过" |
| 6 | 计划是下游输入基线，交接前自审查全部 8 项 | 跳过审查直接交接 |
...
| 合理化借口 | 实际情况 |
|-----------|---------|
| "这个任务很简单，不需要详细步骤" | 模糊步骤在实现时被曲解 = 返工 |
| "similar to Task N 就不用重复写了" | 上下文漂移使旧计划已不成立 |
| "实现的时候自然就知道了" | 计划阶段的决定不可推迟到实现 |
| "跳过自审查省时间" | 计划错误传播成本远超审查成本 |
| "TODO 后面再补" | TODO 从不发生，现在就写 |
