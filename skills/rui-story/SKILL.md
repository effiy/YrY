...
flowchart TD
    ...
    CHK01 -->|"否"| NS["任务"]:::s0
    CHK01 -->|"是"| CHKDOC{"文档基线齐全?<br/>含 场景-N-.md + 知识图谱"}
    CHKDOC -->|"否"| DIP["设计"]:::s1
    CHKDOC -->|"是"| CHKIMP{"场景 §2 实施报告<br/>存在?"}
    CHKIMP -->|"否"| IMP["实施"]:::s2
    CHKIMP -->|"是"| CHKVER{"场景 §3 测试报告<br/>存在?"}
    CHKVER -->|"否"| TST["测试"]:::s3
...
| `设计` | 故事任务存在于远端，场景文档/知识图谱不完整 | 文档生成进行中 |
| `实施` | 远端基线文档齐全，场景 §2 实施报告不存在 | 等待编码 |
| `测试` | 场景 §2 存在于远端，场景 §3 测试报告不存在 | 实现验证中 |
| `报告` | 场景 §3 存在于远端，场景 §4 自改进不存在 | 可交付 |
| `改进` | 场景 §4 自改进存在于远端 | 持续改进中 |
