...
7. **幂等更新，级联刷新**：同一命令可反复调用。若目标已存在，执行**增量更新**而非强制重建：读取既有文档作为上游事实 → 差异对比 → 级联重写受影响的下游文件 → 版本号次版本 `+1` → 追加更新记录。`init` 命令按 `rules/项目基础文件.md` 的 re-init 策略处理。

## 命令扩展：项目初始化（init）

`init` 命令用于生成或更新项目基础文件和 `docs/项目初始化/` 全文档编号集。它是所有功能文档生成的前置基础设施，**可反复执行**（re-init）。
...
| `docs/network.md` | 网络请求库与调用约定 |
| `docs/state-management.md` | 数据与状态管理约定 |
| `docs/FAQ.md` | 常见问题、故障排查与自愈系统 |
| `docs/auth.md` | 认证/鉴权方案与自检规则 |
| `docs/security.md` | 安全策略与自检规则 |

#### B. 全文档编号集（`docs/项目初始化/`，7 个）

`01_需求文档.md` ~ `07_项目报告.md`，其中 `06_实施总结.md` 由 init 自行写入（非 implement-code）。

### 首次执行工作流

1. **扫描仓库结构**：读取 `package.json`、锁文件、构建配置、源码目录结构、git 历史等
2. **生成基础文件**：按 `rules/项目基础文件.md` 逐文件生成完整内容，禁止只写骨架
3. **生成全文档编号集**：对功能名 = "项目初始化" 执行标准 5 步工作流（步骤 1-5）
4. **执行步骤 6**：先 `import-docs`，再 `wework-bot`

### 再次执行（re-init）更新策略

依据 `rules/项目基础文件.md` §「可重复运行（re-init）更新策略」：

1. **优先更新事实**：技术栈、目录结构、命令、入口、关键路径等可由代码/配置推断的内容必须刷新为最新结果
2. **保留人工补充**：团队约定/经验沉淀类内容若已存在且未与事实冲突，默认保留
3. **冲突标注**：检测到文档陈述与代码事实冲突时，标注 `> 待确认（原因：…；建议以代码为准）` 并给出冲突证据路径
4. **哨兵块安全重写**：对使用 `<!-- AUTO-GENERATED:BEGIN -->...<!-- AUTO-GENERATED:END -->` 包裹的段落，允许二次运行重写块内内容、尽量保留块外人工内容
5. **可观测更新**：在 `docs/项目初始化/06_实施总结.md` 的"文件清单/验证结果"里记录本轮对基础文件的更新摘要
6. **版本追踪**：更新文档头部「最后更新时间」；`docs/项目初始化/` 下文件版本号次版本 `+1`
...
|------|--------|----------------|
| 功能名 | 固定为 `项目初始化` | 用户指定 |
| 06_实施总结 | **由 init 自行写入** | 由 implement-code 写入，本技能不创建 |
...
- **更新机制**：同一自然周内反复调用，覆盖更新同一文件。版本号次版本 `+1`（`v1.0` → `v1.1`），KPI 数据重新执行 `collect-weekly-kpi.js --with-logs` 拉取，复盘与后期规划按最新文档状态重建。
...
- **项目初始化/再初始化**：仓库缺乏基础项目文件或基础文件已过时，使用 **`init`**
- **功能文档新建**：为某一功能在 `docs/<功能名>/` 下生成完整文档集
- **功能文档更新**：同一功能需求变化、验收标准调整或用户故事扩展时，再次调用 `/generate-document <功能名>-<描述>`，执行增量更新
- **周报生成/更新**：生成本周周报或覆盖更新同一自然周的周报
- **从周报拆解**：已有周报，需将「后期规划与改进」落成多套标准需求文档集时，使用 **`from-weekly`**
...
- **探测既有文档**：检查 `docs/{功能名}/` 是否存在且包含有效 Markdown 文件
  - **不存在（新建模式）**：按标准 5 步工作流继续
  - **已存在（更新模式）**：
    1. 加载现有 `01_需求文档.md` 作为**既有需求基线**
    2. 加载现有 `02_需求任务.md`、`03_设计文档.md` 作为**下游事实**
...
**新建模式**：严格按 `rules/<类型>.md` 章节顺序完整产出每份文档。

**更新模式**：基于步骤 1 的「更新影响表」执行**增量生成**：
- **01_需求文档.md**：
  - 重写发生变更的章节（用户故事、验收标准、功能点）
  - 未变更章节保留原文，仅刷新头部元数据（版本号、最后更新时间）
  - 在头部或尾部追加「更新记录」小节：本次更新日期、触发原因、变更摘要
- **02_需求任务.md**：
  - 若 01 的用户故事或验收标准变更，同步更新对应表格与流程图
  - 若新增场景，追加到功能分解图/用户流程图/功能流程图（保持 Mermaid 语法正确）
  - 影响分析章节（第 6 章）必须重新调用 `impact-analyst` 刷新
- **03_设计文档.md**：
  - 若 01/02 的 scope 变更，重新调用 `architect` 和 `planner`，重写架构设计、模块划分、接口规范
  - 若仅验收标准微调，可仅更新验证策略章节
  - **仅规范驱动**，禁止读取模板
- **04_使用文档.md**：
  - 按 03 的最新状态重建
- **05_动态检查清单.md**：
  - 从最新 02/03 重新抽取场景与实现细节
  - 为新增/变更场景补充检查项，删除已移除场景的检查项
  - **仅规范驱动**
- **07_项目报告.md**：
  - 追加本次更新记录到「变更概览」或独立「更新历史」小节
  - 刷新交付摘要与验证结论
...
- 写入 01-05, 07（06 不创建）
- 文档头部版本号为 `v1.0`
- **init 命令例外**：保存 10 个项目基础文件 + `docs/项目初始化/` 下 01-07（含 06_实施总结）

**更新模式**：
- 目录已存在，**不删除**原有文件
- 对受影响文件执行**覆盖写入**；未受影响文件保持不动
- 版本号规则：
  - 主版本不变，次版本 `+1`（例：`v1.0` → `v1.1`，`v2.3` → `v2.4`）
  - 若发生破坏性变更（用户故事 scope 大幅缩减或重构），允许主版本 `+1`（例：`v1.4` → `v2.0`），但需在 `07_项目报告.md` 中说明理由
- 更新头部元数据：「最后更新时间」写当前日期，「维护者」写实际模型/工具
- **追加更新记录**：在 `01_需求文档.md` 头部元数据区或尾部独立小节追加：
  ```markdown
  ## 更新记录

  | 日期 | 版本 | 变更摘要 | 触发来源 |
  |------|------|---------|---------|
  | YYYY-MM-DD | v1.1 | 新增用户故事「…」、调整验收标准… | 用户二次调用 `/generate-document 功能名-描述` |
  ```
- `07_项目报告.md` 必须追加本次更新到「变更概览」或「更新历史」小节
...
- **必须调用 `wework-bot` 发送完成通知**：
  ...
  - 功能文档更新模式的通知差异：
    - `🎯 结论` 应体现「更新」而非「新建」，如「功能文档集已更新（v1.0 → v1.1）」
    - `📦 产物` 需注明「更新 N 个文件 / 保持 N 个文件不变」
    - `🔍 P0 自检` 基于本次实际修改的文件重新统计
