...
description: 在 `docs/<功能名>/` 下创建或更新全文档编号集（01-05, 07）。典型调用 `/generate-document 功能名-描述`；亦可 `/generate-document from-weekly <周报路径>` 从周报后期规划拆解为多套全文档。用户要求生成/更新功能文档集或从周报落地需求时使用。
user_invocable: true
---
...
2. **梳理为需求**：为每条（或合并后的每条）生成 **`{功能名}`** 与 **一句话用户故事**（用于目录名与 `01_需求文档` 立意）：
   - **命名**：`docs/<功能名>/` 须符合 `shared/path-conventions.md`（简洁、与仓库现有目录不冲突）；系统类改进可用前缀如 `系统改进-xxx`，避免与业务功能混淆。
