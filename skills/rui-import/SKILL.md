...
[工作流全景](#工作流全景) · [项目根定位](#项目根定位) · [扫描规则](#扫描规则) · [一级标签约束](#一级标签约束) · [rui 强制触发](#rui-强制触发) · [调用形态](#调用形态) · [API 契约](#api-契约) · [错误模型](#错误模型) · [hook 触发器](#hook-触发器) · [空输入](#空输入) · [生效标志](#生效标志)
...
> 远端路径第一级标签**仅允许**以下两者之一，禁止其他任何值：

| 第一级标签 | 来源 | 说明 |
|-----------|------|------|
| 项目根目录名 | 非 `docs/故事任务面板/` 的所有文件 | `resolveRemotePath` 自动前置 |
| `故事任务面板` | `docs/故事任务面板/` 下的文件 | 去除 `docs/` 后提升为第一级 |

```mermaid
flowchart TD
    FILE["本地文件"] --> Q{"位于 docs/故事任务面板/?"}
    Q -->|"是"| STORY["第一级 = 故事任务面板"]:::rule
    Q -->|"否"| PROJ["第一级 = 项目根目录名"]:::rule
    STORY & PROJ --> VALID["✅ 合法"]:::ok
    OTHER["其他任何值"] --> INVALID["❌ 禁止"]:::block
```

`prefix` 参数追加在第一级标签之前，不作为第一级标签校验对象。第一级标签始终指项目自身路径的第一段。
...
flowchart LR
    ...
    S2 --> S3["标签合法<br/>第一级 = 项目根名 或 故事任务面板"]:::sig
    S3 --> S4["路径映射<br/>远端路径 = 本地相对路径，一一对应"]:::sig
    S4 --> S5["上传完成<br/>逐文件 POST 无遗漏"]:::sig
```
...
| 标签合法：第一级 = 项目根名 或 故事任务面板 | 检查 resolveRemotePath 实现，修复后重跑 |
