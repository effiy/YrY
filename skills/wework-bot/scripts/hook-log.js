...
/**
 * hook-log — 交付管线步骤 1: 追加通知日志（不发送 HTTP）
 *
 * 从 stdin 或参数获取故事信息，调用 send-message --no-send 写入日志。
 * 无活跃故事时静默退出。
 */
...
function main() {
  ...
  if (!active) {
    console.log('hook-log: no active story, skipped');
    process.exit(0);
  }

  const message = buildMessage(active);
  const tmpFile = path.join(PROJECT_ROOT, '.hook-log-content.tmp');
  const sendScript = path.join(__dirname, 'send-message.js');

  try {
    fs.writeFileSync(tmpFile, message, 'utf8');
    execFileSync('node', [sendScript, '--no-send', '--agent', 'rui', '--name', active.name, '--content-file', tmpFile], {
      cwd: PROJECT_ROOT,
      stdio: 'inherit',
      timeout: 15000,
    });
  } catch (err) {
    console.error(`hook-log: ${err.message}`);
  } finally {
