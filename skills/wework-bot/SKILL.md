...
```mermaid
flowchart TB
    RUI["rui 管线完成 / 阻断 / 失败"]:::src --> LOG["① 追加日志<br/>等价 send --no-send → {project}-00-消息通知列表"]:::step
...
| 步骤 | 操作 | 说明 |
|------|------|------|
| ① 追加日志 | `send --no-send` 等价行为 | 写入 `{project}-00-消息通知列表.md`，不发 HTTP |
...
name = "<story>"   # kebab-case 的故事名
  → 日志路径 = docs/故事任务面板/<story>/{project}-00-消息通知列表.md
```
...
🌐 影响: docs/故事任务面板/user-login/{project}-01-故事任务.md
📎 证据: git log --oneline -1
⏱️ 会话: 自适应规划→策展 全流程 3.2min | 3 agents 参与

———

变更文件: docs/故事任务面板/user-login/{project}-01-故事任务.md (新增, 285行)
```
...
flowchart LR
    ...
    APPEND --> FILE["docs/故事任务面板/<br/>&lt;story&gt;/<br/>{project}-00-消息通知列表.md"]:::file
...
flowchart LR
    ...
    BUILD --> APPEND["执行 send（noSend=true）<br/>写入 {project}-00-消息通知列表"]:::out
...
flowchart TD
    ...
    CHECK --> T3["{project}-00-消息通知列表.md 是否存在?"]:::check
...
无参数时不发送消息，仅检测 `API_X_TOKEN` / 内置配置 / 故事面板 `{project}-00-消息通知列表.md` 并输出推荐任务。
...
flowchart LR
    ...
    S3 --> S4["日志完整<br/>{project}-00-消息通知列表 已追加"]:::sig
