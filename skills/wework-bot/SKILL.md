...
| `--robot <name>` | 直接指定机器人 |
| `--name, -n <story>` | 故事名，发送前追加到 `docs/故事任务面板/<project>-<name>/00-消息通知列表.md` |
| `--content, -c <text>` | 消息正文 |
...
🌐 影响: docs/故事任务面板/<project>-user-login/01-故事任务.md
📎 证据: git log --oneline -1
⏱️ 会话: 自适应规划→策展 全流程 3.2min | 3 agents 参与

———

变更文件: docs/故事任务面板/<project>-user-login/01-故事任务.md (新增, 285行)
```

## 消息通知列表

指定 `--name` 时，发送前追加到 `docs/故事任务面板/<project>-<name>/00-消息通知列表.md`。每条消息以 `【yyyy-mm-dd hh:mm:ss】` 分割线分隔。目录不存在时自动创建，文件为追加模式。
