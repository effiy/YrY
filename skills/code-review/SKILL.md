---
name: code-review
description: 审查代码示例和架构实现是否符合项目规范（入口初始化、状态管理模式、组件注册/导出、编码规范等）。当项目报告或动态检查清单中包含代码实现验证场景时使用。
---
...
- **入口初始化模式**：视图入口是否遵循项目既有初始化方式
- **状态管理模式**：Store/状态层是否遵循项目既有组织方式
- **共享组件注册/导出**：共享组件是否按项目约定集中管理并可被正确引用
