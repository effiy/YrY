...
本 skill 负责审查方法、维度和输出格式；若需要并行专家角色或必答问题，改用 `../../agents/code-reviewer.md`。统一边界见 `../../shared/document-contracts.md` Skill-Agent 边界章节。
...
- **项目架构**：是否符合项目架构约定（视图入口、状态管理、组件注册等）
- **编码规范**：是否符合项目编码规范（命名、注释、格式等）
...
- 读取 `../../docs/architecture.md` 作为项目架构与编码规范判断依据。
