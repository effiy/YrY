...
- **Skills**: 用户输入 `/<name>` 或任务匹配时直接调用。项目内置 4 个 skill：`rui`（编排器）、`rui-claude`（配置管理）、`wework-bot`（企微通知）、`import-docs`（文档同步）。系统内置 `init`、`review`、`security-review`、`simplify`、`loop` 等辅助 skill，见 `<skills>` 块完整列表。
...
├── rules/                 # 规则库（6 条共享约束）
│   ├── code-pipeline.md   # 代码管线：分支隔离、逐模块审查等 16 条
