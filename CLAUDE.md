...
- [开发工作流](#开发工作流)
- [项目约束](#项目约束)
- [RPC 协议详细规范](#rpc-协议详细规范)
- [认证与授权](#认证与授权)
- [测试策略](#测试策略)
- [部署架构](#部署架构)
- [MongoDB 集合概览](#mongodb-集合概览)
- [指引](#指引)
...
>
> 每个项目沿着各自的轴线前进：YiVad → **组件化**，YiAi → **模块化**，YiPet → **组件化 + API 分层**。YiKnowledge 是同时服务于人类和 AI 的共享知识库（YiAi RAG 的数据源）。
...
| PascalCase 组件 | YiVad（Vue），YiPet（Vue）|
...
1. **阅读两个项目的 CLAUDE.md 文件**——每个文件都有其特定的模块边界、约束和近期变更。同时查看 `YiKnowledge/projects/{project}/specs/` 和 `YiKnowledge/projects/{project}/workflows/` 以了解架构模式和任务工作流。
...
- YiKnowledge 由 YiAi 的知识监视器（apscheduler 轮询，间隔由 `config.yaml` 的 `knowledge.watcher_poll_seconds` 配置，默认 60 秒）扫描。
...
// HTTP 层错误（非 RPC 信封）
{ "detail": "<error message>" }  // HTTP 422/500 等
```
...
- **Token 格式**：JWT（`bcrypt` + `PyJWT`），由 YiAi `domain/auth/` 签发
- **前端处理**：YiVad 的 `RequestHttp` 拦截器自动附加 Token；401 响应触发清除 stores → 重定向登录页
- **权限模型**：YiVad 通过后端菜单 API 加载权限树，`v-auth` 指令实现按钮级权限控制
- **会话管理**：YiPet 通过 `chrome.storage.local` 持久化会话；YiVad 通过 Pinia + `pinia-plugin-persistedstate` 持久化

### 测试策略

| 项目 | 框架 | 覆盖范围 | 关键命令 |
|------|------|----------|----------|
| YiAi | pytest 8 + pytest-asyncio + httpx + pytest-cov | 76 个测试，shared 模块 92%+ 覆盖率 | `python -m pytest tests/ -v` |
| YiVad | Vitest + @vue/test-utils + jsdom | 组件 + composables + stores | `pnpm test` |
| YiPet | Vitest 2 + jsdom 29 | 97 个测试，配置 + 服务层 | `npm test` |
| YiKnowledge | 无自动化测试 | 依赖就绪检查清单手动验证 | — |

**测试原则**：
- 单元测试覆盖共享工具、错误码、响应格式（纯函数，无外部依赖）
- 集成测试覆盖 API 端点（httpx AsyncClient + 真实/模拟 MongoDB）
- 确定性测试（stub LLM）用于 Agent 循环逻辑验证
- 在线端到端测试（真实 Ollama）用于 Agent 行为验证
- 前端测试通过 `vue-tsc --noEmit` / `tsc --noEmit` 类型检查补充
...
开发环境（本机）                         生产环境
─────────────                         ─────────
YiVad :8848  ──┐                      YiVad 静态站点
                ├── YiAi :10086 ──── MongoDB（本地/Atlas）
YiPet（扩展）──┘      │                 Ollama（自托管）
                      │                 OSS（对象存储）
                      ├── Ollama :11434
                      ├── MongoDB :27017
...
**开发启动顺序**：
1. 确保 MongoDB 运行（`mongod` 或 Atlas 连接）
2. 确保 Ollama 运行（`ollama serve`）
3. 启动 YiAi：`cd YiAi && python main.py`
4. 启动 YiVad：`cd YiVad && pnpm dev`
5. 构建 YiPet：`cd YiPet && npm run build`，然后在 Chrome 中加载 `dist/`

### MongoDB 集合概览

| 集合 | 用途 | 关键字段 |
|------|------|----------|
| `menus` | 动态菜单配置 | `path`, `name`, `parent`, `meta`, `component` |
| `sessions` | 聊天会话 | `key`, `messages`, `title`, `tags`, `pageContent` |
| `bugs` | 缺陷追踪 | `key`, `title`, `project`, `module`, `severity`, `status` |
| `static_files` | 文件备份 | `path`, `content`, `is_base64`, `tags` |
| `knowledge_files` | 知识库元数据 | `path`, `title`, `tags`, `category`, `frontmatter` |
| `users` | 用户账户 | `username`, `password_hash`, `roles`, `permissions` |

**数据访问规则**：前端不直接访问 MongoDB，所有数据操作通过 YiAi RPC 信封 → `data_service` → `data/repository.py` → Motor 异步驱动。
...
| [YiKnowledge/projects/](YiKnowledge/projects/) | 项目知识中心——架构、模式、工作流、需求、缺陷 |
