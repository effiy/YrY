...
| 信念 | 推论 |
|------|------|
| 模型有能力判断 | 信任模型的决策，不设多余规则 |
| 上下文有限且退化 | 每一条信息都要为结果负责 |
| 现实是唯一裁判 | 产出必须可验证，猜测不作数 |
...
| 口诀 | 准则 | 含义 |
|------|------|------|
| 思在前 | 思先于码 | 想清楚再动手，不确定就问 |
| 码从简 | 最少代码 | 只写必要的，过度设计就删 |
| 改必准 | 精确修改 | 每行改动可追溯到请求 |
| 测先行 | 目标驱动 | 先写失败测试再通过 |
| 毕则告 | 完成通知 | 做完或卡住必须同步 |
| 图为首 | 表达优先 | 口诀→图→结构化文本→表 |
| 自定验 | 生效标志 | 各 agent 自行定义验收标准 |
...
| 策略 | 手段 | 优先级 |
|------|------|--------|
| 可见 | 类型声明、合约断言、自动验证 | 首选 |
| 规则 | CLAUDE.md、检查清单 | 兜底 |
