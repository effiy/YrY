...
- **Decision（决策）**：知识库 markdown（`YiKnowledge/curator/okr/2026-Q3/loop/`）作为**事实源**，承载全部流程记录的完整内容；YiVad 新增「流程记录」页（`processRecord.vue`）通过 `knowledgeService.scanKnowledge` **只读**扫描 `loop/` 目录做整合展示，深链回 KB 文件（复用 `KnowledgePreviewDialog.vue`）。不做 YiVad → KB 的反向写入。
- **Consequences（后果）**：
