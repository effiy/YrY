...
- *No-tool task-completion nudge* — the two guards above only fire when the model *names* a tool. If it rambles vaguely (names none) on a write task and never executes a mutating tool, the run would silently end "completed" with the task undone. When the loop is about to end normally with **zero successful writes** and the last user message is a *write request* (`_is_write_request`, zh/en write-verb heuristic — 创建/删除/更新/create/delete… — that is **negation-aware**: "只读，不要创建/更新/删除任何菜单" does not read as a write task, so read-only queries are never nudged), it injects one `[TASK]` nudge asking the model to call the right write tool or state clearly that none is needed. Bounded to one per run (`_task_nudged`), logged as "Agent task-completion nudge fired". Runs where the user **rejected** a write confirmation are excluded (`_write_rejected`) so the nudge can never re-arm a write the user declined.
- *Completeness checkpoint* — a run can execute write(s) yet still be partially done: observed create+update completing, then the model stopping without the delete it planned (cycle ~2/5 before this). Neither the no-write nudge (`_write_executed` non-empty) nor the narrate-guard (the model *forgets* the tool rather than narrating it) fires. So when the run is about to end, the loop checks the **task text for write tools it named (`db_create`/`db_update`/`db_delete`) that were never executed** and, if any, injects one `[TASK]` nudge naming the missing tool(s) ("the task explicitly named db_delete but it was never invoked — call it now"). Naming the exact missing tool beats asking the model to self-assess (a generic "any remaining steps?" is declined when the model confidently believes it is done). Gating on named-but-unexecuted tools means a fully-complete multi-step run pays no extra turn, a single-create task (names only `db_create`, which did run) is never checkpointed, and rejections are excluded. Measured 2026-08-08: menu cycle create→update→delete **8/8 SUCCESS**, with the checkpoint recovering 2/2 partial runs (cd, cg) by completing the dropped delete.
...
> Verified (2026-08-08, e2e harnesses): read-only queries answer from `db_list` without mutating (`mutated=False`, and with the negation-aware nudge they are never nagged — the old false-positive cost ~40s of wasted escalation); rejecting a `db_create` confirmation leaves the run `stop=completed` with no doc created and neither nudge re-arms the rejected write; a single session chains create→update→delete on one key (8/8 with the completeness checkpoint). Known edge: if the model re-attempts a just-rejected write, it pops a *new* confirmation the user can reject again — data is always gated by the confirmation prompt, never forced. The escalation takeover message now reminds a switched-in model not to re-attempt a rejected write.
