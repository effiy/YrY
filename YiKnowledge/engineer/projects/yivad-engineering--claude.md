---
lifecycle: active
title: YiVad Engineering — CLAUDE.md
status: stable
type: summary
...
- YiVad is a Vue 3.5 + TypeScript 6 SPA with Rsbuild 1, Pinia 4, and Element Plus 2.14, using hash-mode dynamic routing, the `v-auth` directive for button-level permissions, and a custom `RequestHttp` Axios wrapper for all YiAi RPC calls
- ProTable is the canonical table pattern; the `v-auth` directive is the correct granularity for permission control; `filter` (not `query`) and `target_file` (not `path`) are the two contract names that have caused real cross-project bugs
- The SSE `onDone` guard (`!aborted && !error`) is required for any handler that auto-forwards to external channels — without it, aborted chat content is sent to WeCom robots
- The project has zero test coverage and no Vitest integration; the `authMenuList.json` static fallback must be maintained as a first-class asset or users see broken menus; `RSBUILD_ENV_` prefix replaces `VITE_` after the Rsbuild migration
- Recent changes include knowledge preview dialog local LLM chat, KnowledgeMetaStrip UX improvements, RAG/knowledge API modules, sidebar parity, and the Vite-to-Rsbuild migration
...
- **SSE `onDone` must check `!aborted && !error`** before auto-forwarding to external channels (WeCom) — see `src/stores/modules/aiChat.ts` 2026-07-28 fix. (Earlier-session references to `aicr/chat.ts` were STALE — no aicr store exists; the guard lives in the aiChat store.)

## Recent Changes

### 2026-08-06 — Knowledge preview dialog local LLM chat

- **`src/views/aiChat/components/KnowledgeChatPanel.vue`**: Added model selector to the chat panel toolbar. Per-file model selection persisted in localStorage (`kchat:model:<filePath>`). Available models fetched from the aiChat Pinia store (shared with the main aiChat page). The `streamChat` call now uses `selectedModel.value` instead of hardcoded `"qwen3.5"`. See [knowledge-preview-local-chat.md](../knowledge-preview-local-chat.md).
...
- **Reality (audited 2026-08-04)**: `src/views/aicr/` and `src/stores/modules/aicr/` do not exist on master. 0 aicr-specific commits. The claim was never landed — likely an earlier-session hallucination. The aiChat port (line above) is real and shipped; aicr functionality has been subsumed into aiChat components (`ConversationSidebar.vue`, `ConversationSessionSidebar.vue`, `KnowledgeChatPanel.vue`, `LlamaIndexPanel.vue`, etc.) rather than a separate aicr page tree.

## Action recommendations

1. **Add automated contract tests for the RPC envelope between YiVad and YiAi within the next sprint.** The `filter`/`query` and `target_file`/`path` bugs are the same pattern in different domains, and both were silent failures. A Vitest suite that sends real RPC calls to a running YiAi instance and asserts on the response shape (not just status code) would catch the next parameter name mismatch before it reaches production. Start with the 5 most frequently called RPC methods: `query_documents`, `create_document`, `update_document`, `delete_document`, and `streamChat`.

2. **Audit the `authMenuList.json` static fallback against the current dynamic routes within the next week.** When the backend menu API is unavailable, this file is the entire navigation structure. A stale fallback means users see a broken or outdated menu. Run a script that compares the routes in `authMenuList.json` against the actual `src/views/` directory structure and flags any mismatch. This should become a CI check, not a one-time audit.

3. **Create a "New Page Checklist" template in the ProTable patterns rule file.** The current documentation tells developers to use ProTable and `v-auth`, but the sequence of steps (create view, add route, register in menu API, update fallback JSON, add i18n keys, add permission) is scattered across multiple documents. A single checklist that a developer can copy into their PR description ensures no step is missed and gives the reviewer a clear verification path.

4. **Add a pre-commit hook that rejects any `import axios from 'axios'` statement outside of `src/api/index.ts`.** The rule "stores must not import axios" is documented in multiple places, but documentation does not enforce itself. A simple grep-based hook that scans changed files for direct axios imports and rejects the commit would prevent the most common architectural violation at the earliest possible point.

5. **Write an SSE stream contract test that verifies the `!aborted && !error` guard is present in every `onDone` handler that forwards data externally.** The 2026-07-28 fix in `aiChat.ts` was a single guard on a single handler. A test that programmatically enumerates all `onDone` callbacks, injects an aborted state, and verifies that no external forwarding occurs would catch the next handler that forgets this guard. This test should run in CI and block merge on failure.

## Anti-patterns

- **Using raw `el-table` instead of ProTable for new table pages.** ProTable is the canonical table pattern. It encapsulates search, pagination, sorting, and column configuration into a declarative API. Using raw `el-table` means reimplementing all of these features from scratch, creating inconsistency across pages and duplicating pagination/search logic. Every new table page must use ProTable.

- **Using `v-if` with permission state instead of `v-auth` directive.** The `v-auth` directive decouples permission logic from the view layer. Using `v-if` with `authStore.hasPermission('user:edit')` inline creates tight coupling and makes it impossible to change the permission model without touching every view. Button-level permissions must use `v-auth`.

- **Calling `axios` directly instead of going through `RequestHttp`.** The `RequestHttp` class wraps Axios with interceptors, cancellation, error mapping, and the RPC envelope. Importing `axios` directly bypasses all of these layers, breaking authentication (X-Token header), error handling (checkStatus), and the unified response envelope. All API calls must go through `src/api/modules/*` functions.
...
- **Skipping the `!aborted && !error` guard before auto-forwarding SSE results to WeCom.** The 2026-07-28 fix in `aiChat.ts` prevents partial or aborted chat content from being auto-forwarded. Every SSE `onDone` handler that has external side effects must include this guard. Without it, users who abort mid-stream will have incomplete content sent to external channels.

## Related

- [YiVad engineering README](./readme.md) — project overview, architecture, data flow, domain language, and quick start
- [YiVad engineering changelog](./changelog.md) — changelog index with entry pointers and STALE markers
...
| Document | Purpose |
|----------|---------|
| [README.md](../../../../executive/industry/README.md) | Project overview, quick start, domain language |
| `.env` | Environment variables |
| `rsbuild.config.ts` | Build configuration |
| `tsconfig.json` | TypeScript configuration |
| `src/config/index.ts` | App-level constants |
| `.claude/rules/api-request-layer.md` | HTTP request layer rules |
| `.claude/rules/protable-patterns.md` | ProTable conventions |
| `.claude/rules/vue-component-patterns.md` | Vue 3 SFC patterns |
