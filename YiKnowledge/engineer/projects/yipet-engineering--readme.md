...
2. **Create a `RagService` and `KnowledgeService` in `src/api/services/` to wire YiPet into the YiAi RAG and knowledge endpoints within the next 2 sprints.** The YiAi backend already exposes `/rag` and `/knowledge` endpoints, and YiPet's `ApiClient` already supports SSE streaming. Adding these two service classes would enable the chat window to ground its answers in the YiKnowledge markdown tree, matching the capability that YiVad's aiChat already has. This is the highest-impact feature gap between YiPet and YiVad.
...
- [YiVad engineering README](../../yivad/engineering/readme.md) — cross-project companion, shared RPC envelope contract
