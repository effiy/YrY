...
定义技能的 eval。位于技能目录中的 `evals/evals.json`。
...
- `iterations[].is_current_best`：此版本是否为当前最佳版本
...
**如何捕获：** 当子代理任务完成时，任务通知包含 `total_tokens` 和 `duration_ms`。立即保存这些数据——它们不会在其他地方持久化，事后也无法恢复。
...
**重要：** 查看器精确读取这些字段名。使用 `config` 而非 `configuration`，或将 `pass_rate` 放在运行的顶层而非嵌套在 `result` 下，将导致查看器显示空/零值。在手动生成 benchmark.json 时始终参考此 schema。

---

## comparison.json

盲法对比器的输出。位于 `<grading-dir>/comparison-N.json`。
...
事后分析师的输出。位于 `<grading-dir>/analysis.json`。
