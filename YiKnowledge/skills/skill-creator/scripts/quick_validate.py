...
def validate_skill(skill_path):
    ...
    if name:
        ...
        if not re.match(r'^[a-z0-9-]+$', name):
            return False, f"名称 '{name}' 应为 kebab-case（仅允许小写字母、数字和连字符）"
    ...
    if description:
        ...
        if len(description) > 1024:
            return False, f"描述过长（{len(description)} 个字符）。最多 1024 个字符。"
    ...
    if compatibility:
        ...
        if len(compatibility) > 500:
            return False, f"Compatibility 过长（{len(compatibility)} 个字符）。最多 500 个字符。"
