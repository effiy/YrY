#!/usr/bin/env python3
"""运行 eval + 改进循环，直到全部通过或达到最大迭代次数。

将 run_eval.py 和 improve_description.py 组合在循环中，跟踪历史记录
并返回找到的最佳描述。支持训练/测试拆分以防止过拟合。
"""
...
from scripts.utils import find_project_root, parse_skill_md


def split_eval_set(eval_set: list[dict], holdout: float, seed: int = 42) -> tuple[list[dict], list[dict]]:
    """将 eval 集拆分为训练集和测试集，按 should_trigger 分层。"""
    random.seed(seed)
...
def run_loop(
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if train_summary["failed"] == 0:
            exit_reason = f"全部通过 (迭代 {iteration})"
            if verbose:
...
def main():
    parser = argparse.ArgumentParser(description="运行 eval + 改进循环")
    parser.add_argument("--eval-set", required=True, help="eval 集 JSON 文件路径")
    ...
    parser.add_argument("--holdout", type=float, default=0.4, help="保留用于测试的 eval 集比例（0 表示禁用）")
    parser.add_argument("--model", required=True, help="用于改进的模型")
    parser.add_argument("--verbose", action="store_true", help="将进度输出到 stderr")
    parser.add_argument("--report", default="auto", help="在此路径生成 HTML 报告（默认: 'auto' 表示临时文件，'none' 表示禁用）")
    parser.add_argument("--results-dir", default=None, help="将所有输出（results.json、report.html、log.txt）保存到此目录中带时间戳的子目录")
