/**
 * yry-import api -- remote API CRUD operations
 * Extracted from sync.mjs for single-responsibility
 */

import { fetchJson, querySessionsFull } from "./io.mjs";
...
export async function querySessions(/** @type {string} */ apiUrl) {
  ...
  for (const item of list) {
    ...
    if (!item._id && !item.id && item.key) item._id = item.key;
    paths.set(filePath, item);
  }
  return paths;
}
