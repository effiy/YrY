...
> 实现方案见 [开发方案](../../devs/2026-09/77-prd-task-用户数据导出.md)，验证方案见 [测试方案](../../tests/2026-09/77-prd-test-用户数据导出.md)。
...
| 编号 | 功能 | 说明 |
|------|------|------|
| FR-1 | 当前数据导出系统现状 | 参见 §当前数据导出系统现状 |
| FR-2 | 数据导出数据流 | 参见 §数据导出数据流 |
| FR-3 | 根因矩阵 | 参见 §根因矩阵 |
| FR-4 | 数据导出异步任务流程 | 参见 §数据导出异步任务流程 |
| FR-5 | 数据导出指标 | 参见 §数据导出指标 |
| FR-6 | 数据导出服务层 | 参见 §数据导出服务层 |
| FR-7 | 数据导出页面 | 参见 §数据导出页面 |
