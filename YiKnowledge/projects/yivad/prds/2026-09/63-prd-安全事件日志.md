...
> 实现方案见 [开发方案](../../devs/2026-09/63-prd-task-安全事件日志.md)，验证方案见 [测试方案](../../tests/2026-09/63-prd-test-安全事件日志.md)。
...
| 编号 | 功能 | 说明 |
|------|------|------|
| FR-1 | 当前安全事件记录现状 | 参见 §当前安全事件记录现状 |
| FR-2 | 安全事件生命周期 | 参见 §安全事件生命周期 |
| FR-3 | 根因矩阵 | 参见 §根因矩阵 |
| FR-4 | 数据模型 | 参见 §数据模型 |
| FR-5 | YiAi 后端 — SecurityEventService | 参见 §YiAi 后端 — SecurityEv |
| FR-6 | 暴力破解检测 | 参见 §暴力破解检测 |
| FR-7 | YiVad 前端 — 安全仪表盘 | 参见 §YiVad 前端 — 安全仪表盘 |
