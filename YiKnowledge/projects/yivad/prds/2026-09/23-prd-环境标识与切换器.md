...
> 实现方案见 [开发方案](../../devs/2026-09/23-prd-task-环境标识与切换器.md)，验证方案见 [测试方案](../../tests/2026-09/23-prd-test-环境标识与切换器.md)。
> 依赖：无（纯前端功能，可独立开发）
...
|------|------|------|
| FR-1 | 环境配置 | 参见 §环境配置 |
| FR-2 | 环境 Store | 参见 §环境 Store |
| FR-3 | 环境 Composable | 参见 §环境 Composable |
| FR-4 | 环境标识徽章组件 | 参见 §环境标识徽章组件 |
| FR-5 | 环境切换器组件 | 参见 §环境切换器组件 |
| FR-6 | 生产环境危险操作警告对话框 | 参见 §生产环境危险操作警告对话框 |
| FR-7 | 页面标题工具 | 参见 §页面标题工具 |
| FR-8 | 环境变量文件 | 参见 §环境变量文件 |
| FR-9 | 全局布局集成 | 参见 §全局布局集成 |
