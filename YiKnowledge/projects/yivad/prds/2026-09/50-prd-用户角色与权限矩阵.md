...
> 实现方案见 [开发方案](../../devs/2026-09/50-prd-task-用户角色与权限矩阵.md)，验证方案见 [测试方案](../../tests/2026-09/50-prd-test-用户角色与权限矩阵.md)。
...
| 编号 | 功能 | 说明 |
|------|------|------|
| FR-1 | 当前权限管理能力 | 参见 §当前权限管理能力 |
| FR-2 | 管理员权限分析工作流（现状 vs 目标） | 参见 §管理员权限分析工作流（现状 vs 目标） |
| FR-3 | 根因矩阵 | 参见 §根因矩阵 |
| FR-4 | 有效权限计算流程 | 参见 §有效权限计算流程 |
| FR-5 | 权限矩阵类型定义 | 参见 §权限矩阵类型定义 |
| FR-6 | 权限矩阵组件 | 参见 §权限矩阵组件 |
| FR-7 | 后端权限服务 | 参见 §后端权限服务 |
