...
> 实现方案见 [开发方案](../../devs/2026-09/76-prd-task-用户API密钥管理.md)，验证方案见 [测试方案](../../tests/2026-09/76-prd-test-用户API密钥管理.md)。
...
| 编号 | 功能 | 说明 |
|------|------|------|
| FR-1 | 当前 API 令牌管理现状 | 参见 §当前 API 令牌管理现状 |
| FR-2 | API 密钥管理数据流 | 参见 §API 密钥管理数据流 |
| FR-3 | 根因矩阵 | 参见 §根因矩阵 |
| FR-4 | API 密钥使用流程 | 参见 §API 密钥使用流程 |
| FR-5 | API 密钥管理指标 | 参见 §API 密钥管理指标 |
| FR-6 | API 密钥服务层 | 参见 §API 密钥服务层 |
| FR-7 | API 密钥管理页面 | 参见 §API 密钥管理页面 |
| FR-8 | 文件变更清单 | 参见 §文件变更清单 |
