...
> 实现方案见 [开发方案](../../devs/2026-09/19-prd-task-多标签页工作区.md)，验证方案见 [测试方案](../../tests/2026-09/19-prd-test-多标签页工作区.md)。
> 依赖：无（纯前端组件，独立实现）
...
| 编号 | 功能 | 说明 |
|------|------|------|
| FR-1 | 标签页类型定义 | 参见 §标签页类型定义 |
| FR-2 | 标签页状态管理 Store | 参见 §标签页状态管理 Store |
| FR-3 | 标签页持久化 | 参见 §标签页持久化 |
| FR-4 | 标签页内存管理 | 参见 §标签页内存管理 |
| FR-5 | 标签页拖拽排序 | 参见 §标签页拖拽排序 |
| FR-6 | 分屏视图组件 | 参见 §分屏视图组件 |
| FR-7 | 标签页右键菜单 | 参见 §标签页右键菜单 |
