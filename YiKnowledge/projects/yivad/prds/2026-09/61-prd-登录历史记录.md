...
> 实现方案见 [开发方案](../../devs/2026-09/61-prd-task-登录历史记录.md)，验证方案见 [测试方案](../../tests/2026-09/61-prd-test-登录历史记录.md)。
...
| 编号 | 功能 | 说明 |
|------|------|------|
| FR-1 | 当前登录流程 | 参见 §当前登录流程 |
| FR-2 | 登录记录需求对比 | 参见 §登录记录需求对比 |
| FR-3 | 根因矩阵 | 参见 §根因矩阵 |
| FR-4 | 登录记录流程（目标） | 参见 §登录记录流程（目标） |
| FR-5 | 数据模型 | 参见 §数据模型 |
| FR-6 | YiAi 后端 — LoginHistoryService | 参见 §YiAi 后端 — LoginHisto |
| FR-7 | 异常检测器 | 参见 §异常检测器 |
| FR-8 | YiVad 前端 — 登录历史页面 | 参见 §YiVad 前端 — 登录历史页面 |
| FR-9 | 管理端登录统计仪表盘 | 参见 §管理端登录统计仪表盘 |
