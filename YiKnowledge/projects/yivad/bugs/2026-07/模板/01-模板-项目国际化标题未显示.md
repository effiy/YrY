...
key: project-i18n-title-not-displayed-20260903
tags:
- i18n
- vue-template
- v-bind
- project-list
category: projects/yivad/bugs/template
created: "2026-09-03"
updated: 2026-09-10
source: internal
type: bug
...
**规则**：在 Vue 模板中，所有需要 JavaScript 表达式求值的 prop 必须使用 `v-bind`（`:` 前缀）。静态 prop 传递的是字面字符串。

**检查清单**：
- `$t()` 调用 → 必须 `:prop="$t(...)"`
- 数字/布尔值 → 必须 `:prop="42"` / `:prop="true"`
- 数组/对象 → 必须 `:prop="[...]"` / `:prop="{...}"`
- 纯字符串字面量 → 可用 `prop="hello"`（无需 `:`）

**ESLint 规则**：建议启用 `vue/no-multi-spaces` 和 `vue/v-bind-style` 规则，配合模板中的表达式检查，可在 CI 阶段拦截此类问题。