...
tags: [yiai, workflow, openspec, standards, subagent]
...
roles: [engineer]
benefit: "Claude Code 在 OpenSpec 项目中的子代理协作、代码质量门禁和上下文管理规范"
---

# OpenSpec 工作流规范

> **读完你将能够**：Claude Code 在 OpenSpec 项目中的子代理协作、代码质量门禁和上下文管理规范
...
| 阶段 | 执行模式 | 拆分策略 |
|------|----------|----------|
| 提案（Proposal） | 并行搜索 | 同时启动多个 Explore 子代理搜索代码模式、API、数据模型、已有模块 |
| 设计（Design） | 主代理独立 | 决策性强，不适合拆分子代理 |
| 实现（Apply） | 混合模式 | 识别 tasks.md 中无依赖关系的任务组并行实现，有依赖的串行执行 |
| 验证（Verify） | 并行检查 | 同时启动子代理检查 API 契约一致性、RPC 信封格式、数据模型完整性 |
...
- **lint + typecheck 通过是底线**：`ruff check` + `mypy` 全部通过才算完成
...
3. 对照 design.md 检查关键实现（路径、命名、模块结构）
4. 运行 ruff + mypy 验证
5. 然后继续实施
...
未使用的 import/变量/函数先删除，单独提交，再做重构。

## 四、大规模任务处理

**200 行以上修改或显著架构变更必须走 OpenSpec**：代码改动超过 200 行，或涉及新增服务、API 契约变更、数据模型重构时，禁止直接修改，必须通过 OpenSpec 工作流（`/opsx:propose`）。
...
- 搜索项目时默认包含所有源码类型，跳过 `node_modules/`、`__pycache__`、`.venv/` 等依赖目录
...
- 修改源码文件只能使用内置编辑工具（Read/Edit/Write），禁止用 sed/awk/python -c 等管道命令改文件
- 格式化工具（ruff format）除外
...
**3. 设计审查**：实现前进行设计评审，确保方案合理。`/plan-design-review` 评分各设计维度，确保架构达标。
...
**4.3 自动化门禁**：ruff + mypy 自动执行，任一失败则停止。

**5. E2E 测试**：`/opsx:e2e <change-name>` 生成 pytest 测试 → 验证 API 端点 → `/qa` 真实请求验证。

**6. 发布**：由用户手动触发。内部项目可能直接部署，无需走 PR 机制。
...
- 项目源码文件扩展名检测：`.py` → Python（ruff + mypy）
- 工具不存在时告知用户，不做静默跳过
...
| 规范 | 文件 | 适用阶段 |
|------|------|----------|
| API 规范 | [../开发规范/02-规范-API规范.md](../开发规范/02-规范-API规范.md) | Design + Apply |
| 认证规范 | [../开发规范/03-规范-认证规范.md](../开发规范/03-规范-认证规范.md) | Design + Apply |
| 数据库规范 | [../开发规范/04-规范-数据库规范.md](../开发规范/04-规范-数据库规范.md) | Design + Apply |
| 模块结构规范 | [../架构设计/04-架构-模块结构规范.md](../架构设计/04-架构-模块结构规范.md) | Design + Apply |
| Domain Service 模式 | [../设计模式/01-模式-领域服务模式.md](../设计模式/01-模式-领域服务模式.md) | Design + Apply |

### 强制约束检查

- RPC 路由使用 `APIRouter + include_router` 模式
- Service 层通过依赖注入获取 Repository
- MongoDB 操作通过 Motor 异步驱动
- API 响应统一使用 `{code, message, data}` 格式
- JWT 认证中间件覆盖所有需认证的端点
- 参数名遵守跨项目 RPC 契约（`filter` 而非 `query`，`target_file` 而非 `path`，`cname` 而非 `collection_name`）