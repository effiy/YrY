...
related_modules: [05-prd-task-UI与皮肤中心]
related_tests: [05-prd-test-UI与皮肤中心]
---
...
## 十四、2026-09-15 Color Theme 专业版优化

### 14.1 问题

访问 `http://localhost:8848/#/knowledge/executiver` 时，YiPet 的 Color Theme 将暗色 CSS 变量注入到宿主页面的 `document.documentElement`（`:root`），导致 YiVad 知识页面原生亮色样式被覆盖，页面文字和背景变为不可读的暗色配色。

**根因**：`applyThemeColors(document.documentElement, colorIndex)` 在全页面根元素上注入主题变量，设计初衷是"重新着色整个页面"，但这对于已有自身样式系统的页面（如 YiVad）是破坏性的。

### 14.2 解决方案

**容器级作用域隔离**：将主题 CSS 变量从 `document.documentElement` 迁移到 YiPet 自己的 DOM 容器上。

| 入口点 | 修改 |
|--------|------|
| `overlay.ts` | `applyThemeColors(petContainer, ...)` — 仅作用到 `#yipet-overlay` |
| `relay.ts` | `applyThemeColors(overlay, ...)` + `applyThemeColors(chatRoot, ...)` + 分派 `yipet:colorChanged` 事件 |
| `chat/stores/chat.ts` | `setColorIndex` 中调用 `applyThemeColors(#yipet-chat-root, idx)` |
| `popup/App.vue` | 保持不变（弹窗有独立 document，不受影响） |

**CSS 继承保证**：将变量设置在 `#yipet-chat-root` 和 `#yipet-overlay` 上后，这些容器内的所有子元素通过 CSS 自定义属性继承自动获得主题色值，覆盖 `:root` 上 `variables.css` 定义的默认值。

### 14.3 色板重新设计

弃用 5 个 "Quantum X" 暗色主题，替换为 6 个专业色板：

| 序号 | 主题 | 主色 | 风格 | WCAG AA |
|------|------|------|------|---------|
| -1 | None (Light) | Indigo 基色 | 白底黑字，不干扰宿主页面 | ✓ |
| 0 | Slate Pro | `#64748b` 蓝灰 | 极简专业，冷灰色调 | ✓ |
| 1 | Indigo | `#6366f1` 靛蓝 | 现代专注，深蓝紫色调 | ✓ |
| 2 | Ocean | `#0d9488` 青蓝 | 冷静清晰，深海色调 | ✓ |
| 3 | Forest | `#16a34a` 翠绿 | 自然沉稳，森林色调 | ✓ |
| 4 | Sunset | `#d97706` 琥珀 | 温暖活力，落日色调 | ✓ |
| 5 | Rose | `#be185d` 玫瑰 | 柔和创意，玫瑰色调 | ✓ |

### 14.4 涉及文件

```
YiPet/src/
├── shared/theme/colors.ts              # 6 个专业色板 + 容器级注入
├── content/config/theme-config.ts       # 内容脚本副本（同步）
├── content/rendering/overlay.ts         # 作用域修正
├── content/ipc/relay.ts                 # 作用域修正 + 事件通知
├── chat/stores/chat.ts                  # 导入 applyThemeColors + setColorIndex 修正
├── popup/data.ts                        # COLOR_LABELS 更新
└── popup/components/ColorPicker.vue     # 显示主题名称标签 + 自定义颜色
```

### 14.5 验证

- [ ] `http://localhost:8848/#/knowledge/executiver` 页面保持原生亮色样式
- [ ] 弹窗皮肤中心可正常切换 6 种主题
- [ ] 聊天窗口主题与弹窗选择同步

---

## 十五、2026-09-15 页面主题模式 + 自定义颜色

### 15.1 页面主题模式

**功能**：新增"Apply theme to page"开关，启用后将当前浏览页面色调与所选颜色主题匹配。

**实现原理**：
1. `color-scheme: dark` — 设置到 `<html>`，触发支持网站的原生暗色模式
2. 微弱 CSS `filter` — 每个主题对应一个低强度滤镜（`sepia` + `hue-rotate` + `saturate`），给不支持暗色模式的页面一个微妙的色调
3. 最小化 `<body>` 调整 — 仅设置 `background-color` 和 `color`，不触碰组件级样式
4. 可即时开关 — 关闭后恢复页面原始外观
...
| 主题 | CSS Filter |
|------|-----------|
| Slate Pro | `sepia(0.06) hue-rotate(200deg) saturate(0.6)` |
| Indigo | `sepia(0.05) hue-rotate(230deg) saturate(0.55)` |
| Ocean | `sepia(0.06) hue-rotate(175deg) saturate(0.5)` |
| Forest | `sepia(0.10) hue-rotate(85deg) saturate(0.45)` |
| Sunset | `sepia(0.12) hue-rotate(15deg) saturate(0.65)` |
| Rose | `sepia(0.07) hue-rotate(320deg) saturate(0.5)` |

### 15.2 自定义颜色

**功能**：ColorPicker 新增自定义颜色入口，用户可输入任意 hex 颜色，系统自动生成完整主题色板。

**实现原理**：
- `color-generator.ts` — 从单个 hex 主色通过 HSL 操纵自动推导完整 `ThemePalette`
- 背景色系：主色色相 + 极低亮度（6%-15%）+ 低饱和度
- 文字色系：主色色相 + 高亮度（80%-92%）
- Accent 色：主色色相 +30° 旋转，中高饱和度
- 全部满足 WCAG AA 对比度（≥4.5:1）

### 15.3 涉及文件

```
YiPet/src/
├── content/rendering/page-theme.ts        # 新增：页面级主题应用/移除
├── shared/theme/color-generator.ts        # 新增：hex → ThemePalette 生成器
├── shared/theme/index.ts                  # 修改：导出 generatePalette
├── content/ipc/relay.ts                   # 修改：+setPageTheme 消息处理
├── shared/ipc/messages.ts                 # 修改：+setPageTheme action 类型
├── shared/i18n/index.ts                   # 修改：+新 MessageKey
├── popup/types.ts                         # 修改：+pageTheme, +customColor
├── popup/stores/popup.ts                  # 修改：+setPageTheme, +setCustomColor
├── popup/App.vue                          # 修改：+页面主题开关 + 自定义颜色支持
├── popup/components/ColorPicker.vue       # 修改：+自定义颜色色块 + 颜色选择器
└── public/_locales/{en,zh_CN}/messages.json  # 修改：+6 个 i18n key
```
- [ ] 宠物覆盖层皮肤环与弹窗选择同步
- [ ] `npm run typecheck` 通过
- [ ] `npm run build` 通过
