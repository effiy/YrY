  ...
  var DATA = {
    ...
    agents: {
      ...
      content: '<p style="color:var(--text-secondary);margin-bottom:12px">Agent 是管线中的角色定义，各有职责边界和读写权限。由 <code>agents/AGENT.md</code> 拓扑总纲统一协调。</p>'
        + '<h3 style="color:var(--color-accent);font-size:.78rem;margin:12px 0 6px">角色拓扑总纲（AGENT.md）</h3>'
        + '<p style="font-size:.66rem;color:var(--text-muted);margin-bottom:8px">pm 作为决策中枢，沿 <code>pm → planner → coder → tester → reporter</code> 主链委派，security / architect / code-reviewer 横切注入约束，self-improve 异步闭环。详细契约与多 Agent 协作模式见 <a href="../agents/AGENT.md" target="_blank" style="color:var(--cyan)">agents/AGENT.md</a>。</p>'
        + '<table style="width:100%;font-size:.68rem;color:var(--text-muted);border-collapse:collapse">'
    ...
    rules: {
      title: '&#128220; 治理规则 — 18 条约束',
      content: '<p style="color:var(--text-secondary);margin-bottom:12px">规则定义不可妥协的底线和最佳实践，按 5 类组织：管线执行 · 文档 · 安全配置 · 自改进 · 设计质量。所有 Agent 行为受其约束，合规性由健康检查与 rui-yry 自循环监控。</p>'
        + '<h3 style="color:var(--color-accent);font-size:.78rem;margin:12px 0 6px">管线与执行 (5)</h3>'
        + '<table style="width:100%;font-size:.68rem;color:var(--text-muted);border-collapse:collapse">'
        + '<tr><td style="padding:3px 8px"><a href="../rules/code-pipeline.md" target="_blank" style="color:var(--cyan)">code-pipeline</a></td><td style="padding:3px 8px">分支隔离 · Gate A/B 关卡 · 逐模块 P0 清零 · 研究优先开发</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/code-pipeline-techniques.md" target="_blank" style="color:var(--cyan)">code-pipeline-techniques</a></td><td style="padding:3px 8px">10 项支撑技术模式 · 贯穿管线各阶段 · 每项对应一条 Iron Law</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/delivery-gate.md" target="_blank" style="color:var(--cyan)">delivery-gate</a></td><td style="padding:3px 8px">交付收口三步 Hook：追加日志 → 文档同步 → 企微通知</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/architecture-diagram.md" target="_blank" style="color:var(--cyan)">architecture-diagram</a></td><td style="padding:3px 8px">Mermaid 语法 · 颜色语义体系 · 布局标准</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/plan-execution.md" target="_blank" style="color:var(--cyan)">plan-execution</a></td><td style="padding:3px 8px">计划创建 → 逐步骤执行 → 每步验证 → P0 强制清零 → 完成信号</td></tr>'
        + '</table>'
        + '<h3 style="color:var(--color-accent);font-size:.78rem;margin:12px 0 6px">文档规则 (4)</h3>'
        + '<table style="width:100%;font-size:.68rem;color:var(--text-muted);border-collapse:collapse">'
        + '<tr><td style="padding:3px 8px"><a href="../rules/doc-generation.md" target="_blank" style="color:var(--cyan)">doc-generation</a></td><td style="padding:3px 8px">表达优先：图 → 结构化文本 → 表格 · 不可降级</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/doc-generation-lifecycle.md" target="_blank" style="color:var(--cyan)">doc-generation-lifecycle</a></td><td style="padding:3px 8px">补充文档触发条件 · 文档策展策略 · 例外处理 · 生效标志</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/knowledge-graph.md" target="_blank" style="color:var(--cyan)">knowledge-graph</a></td><td style="padding:3px 8px">三层节点 story→scene→source · 边关系约束 · JSON Schema</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/mermaid-theme.md" target="_blank" style="color:var(--cyan)">mermaid-theme</a></td><td style="padding:3px 8px">Mermaid 统一主题 · 全局变量 · 颜色语义 · 暗色标准</td></tr>'
        + '</table>'
        + '<h3 style="color:var(--color-accent);font-size:.78rem;margin:12px 0 6px">安全与配置 (2)</h3>'
        + '<table style="width:100%;font-size:.68rem;color:var(--text-muted);border-collapse:collapse">'
        + '<tr><td style="padding:3px 8px"><a href="../rules/security-guardrails.md" target="_blank" style="color:var(--cyan)">security-guardrails</a></td><td style="padding:3px 8px">认证不可绕过 · 密钥不落盘 · 输入必校验 · 禁止魔法数字</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/rui-claude.md" target="_blank" style="color:var(--cyan)">rui-claude</a></td><td style="padding:3px 8px">.claude/ 目录同步策略 · 健康分析标准</td></tr>'
        + '</table>'
        + '<h3 style="color:var(--color-accent);font-size:.78rem;margin:12px 0 6px">自改进 (1)</h3>'
        + '<table style="width:100%;font-size:.68rem;color:var(--text-muted);border-collapse:collapse">'
        + '<tr><td style="padding:3px 8px"><a href="../rules/self-improve.md" target="_blank" style="color:var(--cyan)">self-improve</a></td><td style="padding:3px 8px">D0-D7 诊断 · 经验技能化 · 记忆压缩注入 · E1-E4 效果评估</td></tr>'
        + '</table>'
        + '<h3 style="color:var(--color-accent);font-size:.78rem;margin:12px 0 6px">设计与质量 (6)</h3>'
        + '<table style="width:100%;font-size:.68rem;color:var(--text-muted);border-collapse:collapse">'
        + '<tr><td style="padding:3px 8px"><a href="../rules/architecture-principles.md" target="_blank" style="color:var(--cyan)">architecture-principles</a></td><td style="padding:3px 8px">架构宪法 · 内核与扩展边界 · 配置 API · 可健康检测的架构基线</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/design-principles.md" target="_blank" style="color:var(--cyan)">design-principles</a></td><td style="padding:3px 8px">9 条工程原则 · SRP/高内聚/低耦合/DIP/OCP/ISP/DRY/YAGNI/组合 · 每 Agent/Skill 审查基准</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/code-paradigm.md" target="_blank" style="color:var(--cyan)">code-paradigm</a></td><td style="padding:3px 8px">代码编程范式 · 模块/函数/错误/导入/常量 · 可 grep 验证的硬约束</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/agent-handoff.md" target="_blank" style="color:var(--cyan)">agent-handoff</a></td><td style="padding:3px 8px">Agent 交接契约 · from/to/deliverable/acceptance · 5 对契约 + 5 阻断标识</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/doc-quality.md" target="_blank" style="color:var(--cyan)">doc-quality</a></td><td style="padding:3px 8px">A/B/C/D 证据等级 · 统一模版 · 禁止 TBD/占位符 · 退化三信号检测</td></tr>'
        + '<tr><td style="padding:3px 8px"><a href="../rules/knowledge-graph-ownership.md" target="_blank" style="color:var(--cyan)">knowledge-graph-ownership</a></td><td style="padding:3px 8px">KG 所有权模型 · 单点写入多点只读 · 三方耦合解耦</td></tr>'
        + '</table>'
    },
    refs: {
      title: '&#128279; 参考入口 — 项目元信息（4 文档 + 1 拓扑 + 6 监控）',
      content: '<p style="color:var(--text-secondary);margin-bottom:12px">项目最重要的入口文档、角色拓扑与监控仪表板。所有外部触发与系统状态都在此汇总。</p>'
        + '<h3 style="color:var(--color-accent);font-size:.78rem;margin:12px 0 6px">文档入口 (4)</h3>'
        + '<table style="width:100%;font-size:.7rem;color:var(--text-muted);border-collapse:collapse">'
        + '<tr><td style="padding:6px 8px;font-weight:600;color:var(--text-primary)"><a href="../README.md" target="_blank" style="color:var(--cyan)">README.md</a></td><td style="padding:6px 8px">项目总览 — 系统全景图 · 管线架构 · 快速开始 · 命令参考 · 领域语言（20 术语）</td></tr>'
        + '<tr style="background:rgba(255,255,255,.02)"><td style="padding:6px 8px;font-weight:600;color:var(--text-primary)"><a href="../CLAUDE.md" target="_blank" style="color:var(--cyan)">CLAUDE.md</a></td><td style="padding:6px 8px">AI 项目指令 — 四大铁律 · 项目画像 · 不可妥协底线 · 退化三因与四层防御</td></tr>'
        + '<tr><td style="padding:6px 8px;font-weight:600;color:var(--text-primary)"><a href="../skills/rui/formulas.md" target="_blank" style="color:var(--cyan)">formulas.md</a></td><td style="padding:6px 8px">故事文档公式 — 生成模板与公式定义（通用元素 · 故事主线 · 补充文档）</td></tr>'
        + '<tr style="background:rgba(255,255,255,.02)"><td style="padding:6px 8px;font-weight:600;color:var(--text-primary)"><a href="../skills/rui-bot/SKILL.md" target="_blank" style="color:var(--cyan)">企微 Bot 配置</a></td><td style="padding:6px 8px">Webhook 配置 — 消息格式定义 · 通知触发条件 · 发送演示</td></tr>'
        + '</table>'
        + '<h3 style="color:var(--color-accent);font-size:.78rem;margin:12px 0 6px">角色与拓扑 (1)</h3>'
        + '<table style="width:100%;font-size:.7rem;color:var(--text-muted);border-collapse:collapse">'
        + '<tr><td style="padding:6px 8px;font-weight:600;color:var(--text-primary)"><a href="../agents/AGENT.md" target="_blank" style="color:var(--cyan)">AGENT.md</a></td><td style="padding:6px 8px">Agent 角色拓扑总纲 — 行为纪律 · 设计原则 · 执行准则 · ADR · 多 Agent 协作模式</td></tr>'
        + '</table>'
        + '<h3 style="color:var(--color-accent);font-size:.78rem;margin:12px 0 6px">监控与报告 (6)</h3>'
        + '<table style="width:100%;font-size:.7rem;color:var(--text-muted);border-collapse:collapse">'
        ...
        + '<tr style="background:rgba(255,255,255,.02)"><td style="padding:6px 8px;font-weight:600;color:var(--text-primary)">🧬 自改进分析（实时面板）</td><td style="padding:6px 8px">按日/周/月/全景四视角展示健康趋势 · D0-D7 诊断 · 等级分布 · 分支健康对比（<a href="#" onclick="event.preventDefault();window.PanelHub&amp;&amp;window.PanelHub.open(\'selfimprove\')" style="color:var(--cyan)">打开面板</a>）</td></tr>'
        + '<tr><td style="padding:6px 8px;font-weight:600;color:var(--text-primary)">🔔 通知中心（实时面板）</td><td style="padding:6px 8px">健康检查 / 自循环巡检 / 趋势扫描三类通知统一汇总 · 按类型筛选 · 企微推送记录（<a href="#" onclick="event.preventDefault();window.PanelHub&amp;&amp;window.PanelHub.open(\'notify\')" style="color:var(--cyan)">打开面板</a>）</td></tr>'
        + '</table>'
    }
  };
