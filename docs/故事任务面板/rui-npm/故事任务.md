...
| Story 5: 账号级包管理 | FP12/FP13/FP14 | [场景 5](场景-5-账号级包管理/场景-5-账号级包管理.md) | my-packages（认证 → registry API → 表格/JSON）· deprecate（认证 → 所有权验证 → npm deprecate）· unpublish（认证 → 所有权验证 → 安全警告 → 确认 → npm unpublish） | ✓ |
