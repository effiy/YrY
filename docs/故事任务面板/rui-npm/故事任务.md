...
> | v1.2.0 | 2026-06-06 | deepseek-v4-pro | 🌿 feat/rui-npm | 📎 [CLAUDE.md](../../../CLAUDE.md) |
...
{"version":"1.1.0","date":"2026-06-06","trigger":"/rui update","change":"文档基线优化：补齐 4 场景 ×3 辅助文件（知识图谱/测试面板/审查），新增知识图谱.json，丰富场景文档（图谱定位/情感目标/成功感知/数据流全景/安全考量），更新 docs/index.html 导航链接"},
...
| 图层 | 本故事节点 | 上游 | 下游 |
|------|-----------|------|------|
| 领域层 | story: rui-npm | YrY 技能体系 (part_of) | contains → 四个场景 |
| 结构层 | 9 子命令 · 7 场景 · 3 核心文件 | 领域层 maps_to | 内容层 |
...
| 7 | CDN 服务（unpkg/jsDelivr/esm.sh）URL 格式变更 | 风险 | L | L | CDN URL 为字符串模板生成，格式变更时只需更新模板；各 CDN 有稳定 URL 模式 | FP11 |

**约束**：分支隔离（feat/rui-npm）· 源码唯一入口（rui code 管线）· 测试先行（Gate A）· 逐模块 P0 清零

**产出**：故事任务.md · 场景-N-<slug>.md（×4）· plan.html · 知识图谱.json · 知识图谱.html · skills/rui-npm/SKILL.md · help.mjs · rui-npm.mjs · 每场景：架构图.html · 知识图谱.html · 测试面板.html · 演示.html · 审查.html · 计划清单.html
...
|-------------|-----|---------|-------------|:--:|
| Story 1: 包搜索与发现 + CDN 引用 | FP1, FP11 | [场景 1](场景-1-包搜索与发现/场景-1-包搜索与发现.md) | npm search → JSON 解析 → 排序 → 表格/JSON 输出 · 降级处理 + CDN 三源引用（unpkg/jsDelivr/esm.sh） | ✓ |
| Story 2: 包安装与版本管理 | FP2/FP3/FP4 | [场景 2](场景-2-包安装与版本管理/场景-2-包安装与版本管理.md) | install（前置校验 → npm install → 确认版本）· update（版本对比）· list（扁平化） | ✓ |
| Story 3: 本地发布与 npx | FP7/FP8 | [场景 3](场景-3-本地发布与npx使用/场景-3-本地发布与npx使用.md) | publish（路径验证 → 登录检查 → 生成 package.json → 冲突检测 → npm publish）· npx 流式执行 | ✓ |
| Story 4: 包信息审计与卸载 | FP5/FP6/FP9 | [场景 4](场景-4-包信息审计与卸载/场景-4-包信息审计与卸载.md) | info（npm view → 分区展示）· audit（按 severity 分组 → 修复建议）· uninstall（前置校验 → 清理） | ✓ |

| 配套文件 | 路径 | 说明 |
|---------|------|------|
| 计划总览 | [plan.html](plan.html) | 故事级计划总览 |
| 知识图谱 JSON | [知识图谱.json](知识图谱.json) | 结构化知识层数据（4 domains + 5 flows + 节点+边） |
| 知识图谱 HTML | [知识图谱.html](知识图谱.html) | Cytoscape.js 可视化 |
| 技能规约 | [SKILL.md](../../../skills/rui-npm/SKILL.md) | 命令面 + 操作规约 + 7 场景 |
| 可执行脚本 | [rui-npm.mjs](../../../skills/rui-npm/rui-npm.mjs) | 9 子命令统一入口 |
| 帮助脚本 | [help.mjs](../../../skills/rui-npm/help.mjs) | 7 场景 + 参数文档 |
| 测试套件 | [rui-npm.test.mjs](../../../tests/skills/rui-npm.test.mjs) | 77 项基线完整性测试 |
| Demo 演示 | [rui-npm.html](../../../demos/rui-npm.html) | 三面板交互演示（全景/演练/命令参考） |

## §R 关联故事

| 故事 | 关系 | 说明 |
|------|------|------|
| yry-arch | 依赖 | rui-npm 作为第 7 技能纳入 YrY 模块拓扑 |
| yry-self-test | 依赖 | rui-npm 纳入自检测试覆盖范围（77 项测试） |

---

> **回溯链**
>
> - 需求来源：本故事由 `/rui doc` 从需求描述生成——用户需要统一入口管理 npm 包的增删改查和本地发布/npx 使用。需求分析基线来源于对 npm CLI 现有流程的观察和痛点识别。
> - 基线内容：
>   - 命令族设计基于 npm CLI 子命令矩阵（search/install/update/list/info/uninstall/publish/audit）+ npx
>   - 场景拆分基于包管理生命周期：发现 → 获取 → 管理 → 清理
>   - 文档公式：[F.story.task](../../skills/rui/formulas.md#fstorytask--project-故事任务-meta--storyn) — 故事任务公式
> - 源码基线：[rui-npm.mjs](../../skills/rui-npm/rui-npm.mjs) — 9 个子命令处理函数
> - 证据级别：FP# 和 AC# 基于源码实现验证（证据级别 A）。风险假设基于 npm 生态经验推导（证据级别 B）。
>
> **证据标注说明**：FP1–FP9 全部有对应的 `cmd*` 函数实现，可通过 `node skills/rui-npm/rui-npm.mjs <cmd>` 直接验证。子命令数量（9）和场景数量（4）基于规约文件枚举（证据级别 A）。

### 变更记录

| 日期 | 版本 | 变更内容 | 触发 | 证据 |
|------|------|---------|------|------|
| 2026-06-06 | 1.2.0 | 场景 1 加强：并入 CDN 引用发现（FP11）— Story 1 扩展为「包搜索与发现 + CDN 引用」，新增 CDN 用户操作/FP/SC/AC/风险，§7 跨文档索引更新 | `/rui update` | rui-npm.mjs:502-546 cmdCdn 实现 |
| 2026-06-06 | 1.1.0 | 文档基线优化 v2：场景文档补齐 §2 测试源码清单/依赖图/效果验证 + §3-§4 完整模板表 + 回溯链/变更记录 + 导航链接；故事任务 §7 跨文档索引扩展为 FP→场景映射表 | `/rui update` | 对比 yry-arch 场景文档 §2-§4 结构完整性 |
| 2026-06-06 | 1.0.1 | 文档基线优化 v1：补齐 4 场景 ×3 辅助文件（知识图谱/测试面板/审查），新增知识图谱.json，丰富场景文档（图谱定位/情感目标/成功感知/数据流全景/安全考量），更新 docs/index.html 导航链接 | `/rui update` | 对比 yry-arch 每场景 7 文件结构 |
| 2026-06-05 | 1.0.0 | 初始故事基线：新增 rui-npm 技能，管理个人 npm packages 的增删改查 + 本地发布/npx 使用 | `/rui doc` | 需求分析 + npm CLI 现有流程 |
