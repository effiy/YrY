...
> **导航**: [02-用户使用场景 →](./02-rui-story-用户使用场景.md)
...
| §1 Story 2 — 单故事详情查看 | show 子命令需求 | 05-测试用例评审 | AC4–AC5 的测试用例 | 待生成 |
| §1 Story 3 — 文档同步委托 | sync 子命令需求 | 05-测试用例评审 | AC6–AC7 的测试用例 | 待生成 |
| §2 FP5 — 状态判定 | 六状态模型 | 05-测试用例评审 | 状态判定覆盖测试 | 待生成 |
| §2 R2 — sync 委托 | 委托同步程序 | 05-测试用例评审 | AC6–AC7 的委托验证 | 待生成 |
| §3 SC1–SC4 — 全部成功标准 | 用户可感知的度量目标 | 09-自改进复盘 | 逐条对比 SC# 目标值 vs 实测值 | 待生成 |
| §5 全部 AC# | 验收标准定义 | 09-自改进复盘 | AC 最终通过率与未达项分析 | 待生成 |
| §6 风险与假设 | 5 条风险/假设 | 09-自改进复盘 | 回顾风险命中率与假设验证结果 | 待生成 |
...
| 2026-05-17 | v1.1 强化基线贯穿力 | 双基线模型升级 — 去代码耦合、加外部参考应用、加 §L 自改进循环 | [02-用户使用场景](./02-rui-story-用户使用场景.md) 同步升级为代码无关基线 |
