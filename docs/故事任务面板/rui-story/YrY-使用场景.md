...
> **来源引用**: 从 `skills/rui-story/SKILL.md` 命令族全景 + `rui-story.mjs` 命令处理器反推。证据 Level B + 源码路径。2026-05-22 通过 /rui update 追加状态管理和指标采集场景。
...
- 🎯 覆盖全部 12 条命令的用户操作路径，含正常/空/错误三种状态
- 🔒 明确数据边界：查询不读本地、写入有确认、破坏性操作先展示后执行、状态转移可审计
- ⚡ 远端优先体验 — 用户无需关心本地文件系统即可了解全局进度
- 📊 从概览到详情的渐进式信息披露 — 概览 → 列表 → 单故事详情 → 指标采集
- 🔄 状态闭环管理 — 状态转移校验 → 执行 → 审计，形成 任务→设计→实施→测试→报告→改进→新任务 闭环
...
flowchart TD
    ...
    Q1 -->|"管理状态转移"| S10["场景 J: 状态转移管理"]
    Q1 -->|"采集执行指标"| S11["场景 K: 指标采集"]

    classDef entry fill:#fff3e0,stroke:#e65100;
    classDef scene fill:#e3f2fd,stroke:#1565c0;
    class S1,S2,S3,S4,S5,S6,S7,S8,S9,S10,S11 scene;
```
...
| 角色 | 触发条件 | 核心目标 |
|------|---------|---------|
| 管线执行者 | 执行 `/rui-story status check` 或 `/rui-story status transition` | 验证和执行故事状态转移，形成 任务→设计→实施→测试→报告→改进→新任务 闭环 |

```mermaid
flowchart LR
    A["输入 status check<br/>--from=设计 --to=实施"] --> B["加载状态机定义"]
    B --> C{"转移合法?"}
    C -->|"是"| D["输出合法 + 可执行命令"]
    C -->|"否"| E["输出非法 + 原因 + 合法列表"]
    D --> F["用户执行 status transition"]
    F --> G["再次校验合法性"]
    G --> H["更新 rui-state.json"]
    H --> I["追加 audit-log.jsonl"]
    I --> J["输出转移完成确认"]
```

| # | 步骤 | 输入 | 系统响应 | 异常分支 |
|---|------|------|---------|---------|
| 1 | 校验转移 | `--from=设计 --to=实施` | 检查状态机定义中是否存在该转移 | 状态枚举不存在 → 提示合法值 |
| 2 | 执行转移 | `--from=设计 --to=实施 --reason="基线就绪"` | 更新 rui-state.json + 追加审计日志 | 转移非法 → 阻断并提示 |
| 3 | 仪表板 | `status dashboard` | 扫描所有故事目录 → 聚合状态/阻断/时间 | 无故事 → 空状态提示 |
...
| 角色 | 触发条件 | 核心目标 |
|------|---------|---------|
| 项目维护者 | 执行 `/rui-story collect story` 或 `/rui-story collect all` 或 `/rui-story collect anomalies` | 量化管线健康度，检测 D0-D7 退化信号 |

```mermaid
flowchart LR
    A["输入 collect story <name>"] --> B["读取 .memory/execution-memory.jsonl"]
    B --> C{"文件存在?"}
    C -->|"是"| D["按 story 筛选记录"]
    C -->|"否"| E["提示无数据"]
    D --> F["计算阶段耗时/轮次/P0数量"]
    ...
    H["输入 collect anomalies"] --> I["读取全部执行记忆"]
    I --> J["D0-D7 模式匹配"]
    J --> K{"检测到异常?"}
    K -->|"是"| L["输出异常列表含诊断级别"]
    K -->|"否"| M["输出健康声明"]
...
| # | 步骤 | 输入 | 系统响应 | 异常分支 |
|---|------|------|---------|---------|
| 1 | 单故事指标 | `collect story rui-story` | 读取 execution-memory.jsonl → 计算指标 | 文件不存在 → 提示 |
| 2 | 全局汇总 | `collect all` | 扫描所有故事 → 聚合计算 | 无数据 → 空状态提示 |
| 3 | 异常检测 | `collect anomalies` | D0-D7 模式匹配全部记录 | 无异常 → 健康声明 |
...
| J 状态转移管理 | FP13, FP14, FP19, FP20 | AC13–AC17 | §1, §4 | TC-N10, TC-E04 | 待生成 | 闭环：任务→设计→实施→测试→报告→改进→新任务 |
| K 指标采集 | FP16, FP17, FP18 | AC18–AC20 | §1, §8 | TC-N11, TC-E05 | 待生成 | D0-D7 异常检测 |
...
| 3 | FP 全覆盖 | ✅ 20 个 FP 全部覆盖 |
| 4 | 异常分支明确 | ✅ 每场景含 Token 缺失/API 不可达/不存在分支 |
| 5 | 无技术术语 | ✅ 已扫描，无代码路径/API 端点/组件名 |
| 6 | 每场景含空状态与错误恢复 | ✅ Token 缺失/远端不可达/目录不存在/无数据/文件缺失 |
...
| 管线执行者 | 执行状态转移并追踪审计 | 安全可控，有据可查 | 状态变更无记录 | 转移确认 + 审计日志已写入 | J |
| 项目维护者 | 采集指标并检测异常 | 量化健康，提前预警 | 退化信号难以发现 | 指标输出 + 异常列表或健康声明 | K |
...
> | 日期 | 变更 | 触发 | 证据 |
> |------|------|------|------|
> | 2026-05-22 | 追加场景 J 状态转移管理 + 场景 K 指标采集；更新场景覆盖矩阵/评审清单/体验基线/场景→API映射 | /rui update rui-story | skills/rui-story/SKILL.md §状态转移 §指标采集 |
