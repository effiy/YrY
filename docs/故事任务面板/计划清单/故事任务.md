...
| Story 2 FP9–FP12 | 清单勾选·折叠面板·标签页·风险展开 | 场景-3-清单交互组件实现/index.md | 全部交互组件可用，localStorage 持久化正常 | ✓ 已覆盖 |
| Story 3 FP13–FP16 | 验证报告·风险热力图·趋势图·分数卡 | 场景-4-验证报告与健康面板集成/index.md | 可视化组件正确渲染，数据与源一致 | ✓ 已覆盖 |
| Story 4 FP17–FP20 | 批量生成·增量检测·自循环·摘要报告 | 场景-5-批量生成与自循环机制/index.md | 全流程可运行，生成摘要准确 | ✓ 已覆盖 |
