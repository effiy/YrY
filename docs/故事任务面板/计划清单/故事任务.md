...
> | v1.0.0 | 2026-06-13 | deepseek-v4-pro | 🌿 feat/yry-checklist | 📎 [CLAUDE.md](../../../CLAUDE.md) |
...
本故事将计划清单生成能力拆解为五层管线：模板架构 → 数据提取 → 组件交互 → 验证集成 → 批量自循环。每层是一份独立可验证的工程制品，合起来构成从 markdown 源到可交互清单页面的完整自动化链路。
...
flowchart TD
    ...
    RENDER -.->|"CDN 加载链"| CDN["shared.css + theme.css<br/>yry-checklist.css<br/>shared.js"]:::cdn
...
优先级 **P0**。范围边界：定义计划清单页面的完整 HTML 骨架、CSS 类体系、CDN 加载链。不涉及具体数据填充逻辑和交互 JS。依赖：CDN 资源（shared.css、theme.css、yry-checklist.css、shared.js）可用。
...
- **FP3 CDN 加载链规范** — 定义 Category B 页面的 CDN 资源加载顺序：shared.css → theme.css → yry-checklist.css → shared.js。输入：CDN 资源清单。输出：`<head>` 中的 `<link>` / `<script>` 标签序列。阻断：加载顺序错误导致样式覆盖失效。
...
- 具体场景的数据填充逻辑 — 属于 Story 2（数据提取）范畴
- 交互 JS 逻辑实现 — 属于 Story 3（组件交互）范畴
...
<a id="s-1-story-2"></a>
### Story 2: markdown 解析与数据提取管线

作为 内容生产者，我想要 一份自动化的数据提取管线——从场景 index.md 中解析 §0–§4 章节的结构化数据，以便 计划清单页面的内容由 markdown 源驱动，而非手工维护 HTML。

优先级 **P0**。范围边界：章节拆分、表格解析、mermaid 块提取、元数据抽取。不涉及 HTML 渲染和组件交互。依赖：场景 index.md 按标准 §0–§4 结构编写。
...
flowchart LR
    ...
    subgraph 解析["解析层"]
        SPLIT["章节拆分<br/>§0 §1 §2 §3 §4"]:::parse
        TABLE["表格解析<br/>步骤表 · 风险表 · AC表"]:::parse
        MERMAID["mermaid 提取<br/>效果示意 · 数据流"]:::parse
        META["元数据抽取<br/>版本 · 日期 · 标题"]:::parse
    end

    subgraph 输出["结构化输出"]
        CTX["Context 对象<br/>{ story, scene, sections,<br/>  steps, risks, metrics,<br/>  deliverables }"]:::out
    end
...
| # | 操作 | 触发条件 | 操作步骤 | 预期结果 |
|---|------|---------|---------|---------|
| 1 | 提取章节内容 | 运行 `/rui-html <story>` | 读取 index.md → 按 §0–§4 分割 → 返回各章节 markdown | 获得 5 个章节的结构化文本 |
| 2 | 解析步骤表 | 执行 extractor 脚本 | 定位步骤表格 → 逐行解析列（编号·标题·Owner·工期·优先级·产出）→ 输出步骤对象数组 | 步骤数组含全部必需字段 |
| 3 | 提取验收标准 | 执行 extractor 脚本 | 识别 `step-criteria` 块 → 提取 `<ul>` 列表项 → 关联到对应步骤 | 每条验收标准关联到正确步骤 |
| 4 | 解析风险矩阵 | 执行 extractor 脚本 | 定位风险表 → 解析影响×概率 → 分类为高/中/低 | 风险对象含等级·类别·缓解措施 |
...
**P0**
- **FP5 章节拆分引擎** — 将 index.md 按 `## §0` / `## §1` / `## §2` / `## §3` / `## §4` 标题拆分为独立章节块。输入：index.md 文件内容。输出：`{ §0, §1, §2, §3, §4 }` 章节映射。阻断：任一必需章节缺失。
- **FP6 表格解析器** — 将 markdown 表格（步骤表、验收标准表、风险表、AC 表）解析为结构化 JSON 数组。输入：表格 markdown 文本。输出：JSON 对象数组，每行一个对象，列名为 key。阻断：表格格式异常导致解析失败。
- **FP7 元数据抽取器** — 从 index.md 首行和元数据行提取版本号、日期、场景标题。输入：index.md 文件内容。输出：`{ version, date, title, sceneNum }` 对象。阻断：版本号或标题缺失。
- **FP8 Context 对象组装** — 将 FP5–FP7 的产出组装为统一的 Context 对象，供给模板渲染使用。输入：FP5 + FP6 + FP7 的产出。输出：完整 Context JSON 对象。阻断：Context 对象缺少模板 Token 对应的字段。

##### 业务规则

| R# | 描述 | 校验方式 | 证据级别 |
|----|------|---------|---------|
| R5 | 章节拆分必须识别全部 5 个标准章节（§0–§4） | 检查拆分结果中 key 数量 = 5 | A |
| R6 | 表格解析必须保留列名的大小写和特殊字符 | 对比解析结果与源表格列名 | B |
| R7 | Context 对象必须包含模板所需的全部 Token 字段 | 与 Token 变量表逐字段交叉校验 | A |
| R8 | 缺失章节生成占位提示，不阻断整体流程 | 模拟缺失 §3 → 验证不抛异常 | B |

##### 数据约束

| 约束 | 类型 | 范围/格式 | 来源 |
|------|------|----------|------|
| 章节标识符 | 正则 | `^## §[0-4]` | index.md 文档规范 |
| 表格列分隔符 | 字符 | `\|` | markdown GFM 规范 |
| Context JSON 深度 | 整数 | ≤ 4 层嵌套 | 模板渲染性能约束 |
| 元数据日期格式 | 字符串 | `YYYY-MM-DD` ISO 格式 | 文档质量标准 |

#### §3 成功标准

**P0**
- **SC5** 章节拆分正确识别标准 index.md 的全部 5 个章节。度量：用架构场景的 index.md 测试。目标：5/5 章节识别。→ FP5
- **SC6** 表格解析器正确解析含 10+ 行的复杂步骤表。度量：解析参考场景的步骤表，逐行对比字段。目标：字段准确率 100%。→ FP6
- **SC7** Context 对象覆盖全部模板 Token。度量：提取 Context 的所有 key，与 Token 变量表比对。目标：覆盖率 100%。→ FP8
...
**范围外**
- 非标准章节结构（如 §5+ 自定义章节）— 忽略不报错
- markdown 内联格式（粗体/斜体/代码）的 HTML 转换 — 由模板渲染层处理
- 故事任务.md 的完整解析 — 仅提取元数据

#### §5 AC

| AC# | Given | When | Then | 门禁 |
|-----|-------|------|------|------|
| AC5 | 标准格式的 index.md 文件 | 运行章节拆分 | 返回 5 个章节的结构化数据 | Gate A |
| AC6 | 含合并单元格的复杂表格 | 运行表格解析 | 正确识别合并单元格并填充值 | Gate A |
| AC7 | 缺少 §3 章节的 index.md | 运行数据提取 | §3 标注为缺失，其他章节正常提取 | Gate A |
| AC8 | 元数据行格式异常 | 运行元数据抽取 | 返回错误提示，标注异常字段 | Gate B |

#### §6 风险与假设

**高影响风险**
- **markdown 格式漂移** (M·M → FP5) — 场景作者未按 §0–§4 规范编写，导致章节拆分失败。缓解：提取前先验证章节标题格式；不匹配时输出告警和预期格式。

**中影响风险**
- **表格嵌套复杂** (M·M → FP6) — markdown 表格内含代码块或多行内容时解析边界模糊。缓解：按 GFM 规范严格解析；复杂嵌套标注为需人工确认。

**假设**
- 所有场景 index.md 遵循 §0–§4 标准章节结构
- 表格使用标准 GFM 格式（`| col | col |` 分隔）
...
优先级 **P1**。范围边界：验证报告摘要、健康 KPI 卡片、风险矩阵热力图、趋势迷你图、分数卡。不涉及数据采集逻辑和评分算法本身。依赖：Story 2 的数据提取结果含 §3 测试报告和 §4 自改进数据。
...
（注：此处为 Story 5，复用 §1 的 Story 编号体系）

作为 系统运维者，我想要 计划清单页面支持批量生成和自动重新生成——`/rui-html <story>` 一次生成所有场景的清单、markdown 变更触发 HTML 自动更新，以便 文档与实现始终保持同步，无需手工逐个页面维护。

优先级 **P1**。范围边界：批量生成（全场景 × 全类型）、增量更新（按 mtime 检测过期文件）、自循环调度（30 分钟间隔）。不涉及 CI/CD 集成和 Webhook 触发。依赖：Story 1–4 全部完成。
...
| R21 | 计划清单为 Category B 文档，CDN 加载链为 shared.css → theme.css → yry-checklist.css | 逐 HTML 验证 CDN link 标签顺序 | A |
...
| Story 2 FP5–FP8 | 章节拆分·表格解析·元数据抽取·Context 组装 | 场景-2-markdown解析与数据提取管线/index.md | 数据提取管线端到端可运行 | ✓ 已覆盖 |
| Story 3 FP9–FP12 | 清单勾选·折叠面板·标签页·风险展开 | 场景-3-清单交互组件实现/index.md | 全部交互组件可用，localStorage 持久化正常 | ✓ 已覆盖 |
| Story 4 FP13–FP16 | 验证报告·风险热力图·趋势图·分数卡 | 场景-4-验证报告与健康面板集成/index.md | 可视化组件正确渲染，数据与源一致 | ✓ 已覆盖 |
| Story 5 FP17–FP20 | 批量生成·增量检测·自循环·摘要报告 | 场景-5-批量生成与自循环机制/index.md | 全流程可运行，生成摘要准确 | ✓ 已覆盖 |
...
| `yry-cdn` | 资源依赖 | 计划清单页面依赖 yry-cdn 提供的 shared.css、theme.css、yry-checklist.css 和 shared.js。CDN 资源的稳定性直接影响所有清单页面的渲染质量。 |
...
> - 角色契约：[代码实现](../../../agents/code-implementer.md) — 负责模板引擎和提取器的代码实现
> - 治理约束：[文档生成约束](../../../rules/doc-generation.md) · [架构图规范](../../../rules/architecture-diagram.md) · [知识图谱规范](../../../rules/knowledge-graph.md)
> - CDN 资源：[yry-cdn 项目](../../../cdn/) — shared.css · theme.css · yry-checklist.css · shared.js
>
> **证据标注说明**：本 Story 文档中 FP1–FP4 的断言基于 rui-html 技能规约的显式定义（证据级别 A）。FP5–FP20 为新增功能点的需求定义（证据级别 B）。业务规则 R1–R21 源自项目约束和最佳实践分析。

### 变更记录

| 日期 | 版本 | 变更内容 | 触发 | 证据 |
|------|------|---------|------|------|
| 2026-06-13 | 1.0.0 | 初始化，从 rui-html 计划清单生成需求创建：Story 1 模板架构 + Story 2 数据提取 + Story 3 组件交互 + Story 4 验证集成 + Story 5 批量自循环 | 用户请求：计划清单生成技能 | rui-html SKILL.md 分析 + 架构场景参考 |
