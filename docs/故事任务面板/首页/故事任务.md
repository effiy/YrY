...
YrY 的文档资产分布在 19 技能、5 故事、29 场景、9 Agent、16 规则中——但没有一个统一的入口页面将这些资产按层级聚合展示。`docs/index.html` 作为文档中心首页，承担着"项目资产目录"的角色：新成员从这里导航到任一模块，老成员从这里快速定位到任一场景。
...
- 🏠 **统一入口** — 无论新成员还是老成员，从 `docs/index.html` 可在一页内定位到项目全部资产：12 依赖/框架、19 技能、5 故事、29 场景、9 Agent、16 规则
...
优先级 **P0**。范围边界：覆盖本项目的全部能力模块（19 技能）、5 故事目录、29 场景目录、9 Agent 角色、16 治理规则、12 依赖/框架。不涉及各模块内部实现细节，不涉及外部系统。依赖：系统规约文件（skills/*/SKILL.md · agents/*.md · rules/*.md · docs/故事任务面板/）。
...
flowchart TD
    subgraph 输入["数据源"]
        SKILL["19 技能规约<br/>skills/*/SKILL.md"]:::src
        STORY["5 故事目录<br/>docs/故事任务面板/*/"]:::src
        AGENT["9 Agent 角色<br/>agents/*.md"]:::src
        RULE["16 治理规则<br/>rules/*.md"]:::src
...
| FP# | 功能点 | 描述 | 优先级 |
|-----|--------|------|--------|
| FP1 | 项目资产扫描 | 遍历 skills/ · agents/ · rules/ · docs/故事任务面板/ 目录，提取全部模块的路径和元数据 | P0 |
...
flowchart TD
    subgraph 渲染["页面渲染层"]
        ...
        CSS["CDN 主题加载<br/>theme.css + shared.css"]:::render
...
| FP9 | CDN 主题加载 | yry-cdn-lib theme.css + shared.css 引用，双主题系统（Mono + System） | P0 |
...
| 故事 | 关系 | 说明 |
|------|------|------|
| [系统架构知识固化](../架构/故事任务.md) | 上游 — 供给模块拓扑数据 | 本首页的 Layer 2–5 模块信息来自架构编目 |
| [CDN 共享前端资源库](../../../cdn/故事任务面板/cdn/故事任务.md) | 上游 — 供给主题和组件库 | 本首页的 CSS 主题和 JS 工具来自 yry-cdn-lib |
| [自主测试方案](../自测/故事任务.md) | 下游 — 消费首页链接 | 自检中心从首页交叉导航进入 |
| [自改进闭环](../自改进/故事任务.md) | 下游 — 消费首页面板 | 自改进面板从首页 Panel hub 打开 |

---

> **回溯链**
>
> - 需求来源：YrY 文档资产分散在 19 技能、5 故事、29 场景中，缺少统一入口页面。本故事由 `/rui-init` 的 arch 步骤触发，初始化时自动生成 `docs/index.html` 作为文档中心首页。
