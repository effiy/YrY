...
| 诊断 | 触发状态 | 证据 | 基线引用 |
|------|---------|------|---------|
| D3 复杂度增长 | 未触发 | 评分公式：Σ(维度得分 × 权重) / Σ权重 | `rules/architecture-diagram.md` |
| D4 流程退化 | 未触发 | 校验报告 + 漂移趋势 → 仪表板 | `rules/code-pipeline.md` |
| D6 文档过时 | 未触发 | 趋势图可视化文档健康变化 | `rules/doc-quality.md` |
