...
| 文件 | 写入方式 | 维护方 | 用途 |
|------|---------|--------|------|
| `.memory/rui-state.json` | 覆盖 | rui 管线 | 当前阶段 · 阻断标识 · 三步交付状态 |
| `.memory/execution-memory.jsonl` | 追加 | rui 管线 | 阶段切换轨迹 · 质量问题 · bad cases |
| `.improvement/proposals.jsonl` | 追加 | self-improve 引擎 | 自改进提案 · 效果评估 |
...
| 步骤 | 技能规约 | 降级 |
|------|---------|------|
| 追加日志 | [wework-bot — hook-log](./skills/wework-bot/SKILL.md#hook-log追加日志不发送) | 不降级 |
| 文档同步 | [import-docs — hook 触发器](./skills/import-docs/SKILL.md#hook-触发器) | `no-token` 缺 `API_X_TOKEN` 跳过推送，仍标记 |
| 企微通知 | [wework-bot — hook-notify](./skills/wework-bot/SKILL.md#hook-notify实际发送) | 缺 webhook 跳过，仍标记 |
| 闭合检查 | [delivery-gate](./rules/delivery-gate.md) | 任一步未标记 → 阻断 |
