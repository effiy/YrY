...
> 模板系统为 `/rui doc` 管线提供可复用的起点。每个模板文件使用 `{{PLACEHOLDER}}` 语法标记可替换值。
...
5. **测试基础设施与实际匹配** — `templates/tests/` 是 `tests/` 的直接镜像
