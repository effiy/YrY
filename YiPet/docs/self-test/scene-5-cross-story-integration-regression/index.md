...
(`docs/arch/` and `docs/self-test/`) and the docs home (`docs/data.js`)
...
graph LR
  ...
  A -->|href| C[docs/self-test/scene-1]
  B -->|previewHref| D[index.md exists?]
  C -->|previewHref| E[index.md exists?]
  D --> F{✅}
  E --> F
  G[arch scene §2] -->|cites| H[self-test scene]
  H -->|cites| G
```
...
## Step 2: data.js self-test scene hrefs resolve
**Action**: for each `scenes` item in `self-test-scenes` section, `ls docs/<href>`
**Expected**: every href points to an existing index.md
**File**: `docs/data.js`
...
## Step 4: arch scenes cross-link to self-test where claimed
**Action**: grep `docs/self-test/` in `docs/arch/scene-5-trust-boundary-security-surface/index.md`
**Expected**: at least one cross-link to a self-test scene
...
| `docs/self-test/scene-*/index.md` | files | 6 self-test scenes |
| `CLAUDE.md` | file | Guidance table links to docs/arch/ + docs/self-test/ |
...
| 2 | ✅ | All 6 self-test scene hrefs in data.js resolve to index.md |
| 3 | ✅ | All 6 footerLinks resolve (CLAUDE.md, README.md, arch, self-test, CONTRIBUTING.md, CHANGELOG.md) |
| 4 | ✅ | arch scene 5 references `docs/self-test/` in §4 Limitations |
...
## Edge Cases Found
- The dashboard's `panelHub.urls` keys `arch` and `self-test` map to
