...
**What this scene demonstrates**: After running `/rui-init`, verify
every artifact is present and well-formed. This is the gate that
confirms the pipeline actually produced a coherent baseline.

**Why it matters**: A half-emitted init (missing scenes, broken
data.js) silently leaves the project in a worse state than no init
at all — future docs reference missing files. This check catches that.

```mermaid
graph LR
  A[1. CLAUDE.md] --> B[2. README.md]
