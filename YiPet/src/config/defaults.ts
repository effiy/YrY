...
export interface PetSizeLimits {
  min: number;
  max: number;
}
export interface ChatSizeLimits {
  minWidth: number;
  maxWidth: number;
  minHeight: number;
  maxHeight: number;
  ...
  constants: {
    ...
    URLS: {
      ...
      isSystemPage(url: string | null | undefined): boolean {
        ...
        return (
          url.startsWith(this.CHROME_PROTOCOL) ||
    ...
    ERROR_MESSAGES: {
      ...
      RETRY_FAILED: 'Still failed after multiple attempts',
    },
    SUCCESS_MESSAGES: {
      SHOWN: 'Shown',
      HIDDEN: 'Hidden',
      COLOR_CHANGED: 'Color changed',
      COLOR_SET: 'Color theme set',
