...
import type { ContextChangeEntry, SessionItem } from '../types';
...
export interface ContextChangesDeps {
  ...
  updateSessionMeta: (
    key: string,
    meta: { pageContent?: string; tags?: string[] },
  ) => Promise<void>;
}
...
export function useContextChanges(deps: ContextChangesDeps) {
  ...
  async function applyContextChange(path: string, content: string) {
    ...
    await deps.updateSessionMeta(s.id, { pageContent: newPageContent, tags });
  }
  ...
  async function undoLastContextChange(path?: string) {
    ...
    await deps.updateSessionMeta(s.id, {
  ...
  async function addContextFile(path: string) {
    ...
    tags.push(ctxTag);
    await deps.updateSessionMeta(s.id, { tags });
  }
