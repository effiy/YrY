...
<template>
  <dialog v-if="s.saveToKnowledgeVisible" class="modal-overlay" open @click.self="s.saveToKnowledgeVisible = false">
    <div class="modal-content"><div class="modal-header"><span>Save to YiKnowledge</span><button @click="s.saveToKnowledgeVisible = false">✕</button></div><div class="modal-body"><p>Save to knowledge — port in progress</p></div></div>
  </dialog>
</template>
...
.modal-content { background: var(--bg-elevated, #1e1a3b); border-radius: 12px; min-width: 400px; border: 1px solid rgba(var(--primary-rgb, 99, 102, 241), 0.3); color: var(--text-primary, #f5f3ff); }
