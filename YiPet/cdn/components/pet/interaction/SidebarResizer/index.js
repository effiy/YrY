...
;(function () {
  ...
  const TEMPLATE_PATH = 'cdn/components/pet/interaction/SidebarResizer/index.html'
  const TEMPLATE_ID = '#yi-pet-sidebar-resizer-template'
  let templateCache = ''
  ...
  async function loadTemplate () {
    ...
    try {
      ...
      if (dh && typeof dh.loadHtmlTemplate === 'function') {
        templateCache = await dh.loadHtmlTemplate(TEMPLATE_PATH, TEMPLATE_ID, 'Failed to load SidebarResizer template')
      }
    } catch (_) {}
   ...
   * 从模板创建 resizer 元素
   */
  function createFromTemplate (templateHtml) {
  ...
  function createResizer () {
    if (templateCache) {
      try { return createFromTemplate(templateCache) } catch (_) {}
    }
    return createLegacy()
  }
